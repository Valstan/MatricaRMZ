...
import { buildLookupHighlightParts, searchLookupOptionsTiered } from '@matricarmz/shared';
...
export function SearchSelect(props: {
  ...
  const tiered = useMemo(() => searchLookupOptionsTiered(props.options, query), [props.options, query]);
  const filtered = useMemo(() => [...tiered.primary, ...tiered.similar], [tiered]);
  const similarStartIdx = tiered.primary.length > 0 || tiered.similar.length === 0 ? -1 : 0;
            ...
            >
              ...
              <div ref={listRef} style={{ maxHeight: 260, overflowY: 'auto' }}>
                ...
                {filtered.map((o, idx) => {
                  ...
                  const parts = query.trim() ? buildLookupHighlightParts(o.label, query) : [{ text: o.label, matched: false }];
                  return (
                    <React.Fragment key={o.id}>
                      {idx === similarStartIdx && (
                        <div style={{ padding: '6px 12px', fontSize: 12, color: '#92400e', background: '#fef3c7' }}>
                      ...
                      >
                        <div style={{ fontWeight: 700, color: '#111827' }}>
                          {parts.map((part, partIdx) =>
                            part.matched ? (
                              <mark key={partIdx} style={{ background: '#fde68a', color: 'inherit', padding: 0 }}>
                                {part.text}
                              </mark>
                            ) : (
