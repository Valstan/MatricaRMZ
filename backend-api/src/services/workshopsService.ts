...
import { syncFromWorkshop } from './warehouseLocationsService.js';
...
export async function upsertWorkshop(args: {
  ...
  try {
    ...
    // Mirror into the centralized warehouse_locations registry (phase 2.1).
    // Failure here is non-fatal — workshop write already succeeded.
    try {
      await syncFromWorkshop({
        ...
        name,
        isActive: args.isActive ?? true,
        sortOrder: Math.trunc(Number(args.displayOrder ?? 0)),
        deleted: false,
      });
    } catch (mirrorErr) {
      console.warn('[workshopsService.upsert] warehouseLocations sync failed:', mirrorErr);
    }
    return { ok: true, id };
  } catch (e) {
    return { ok: false, error: String(e) };
...
export async function deleteWorkshop(args: { id: string }): Promise<Result<{ id: string }>> {
  try {
    ...
    // Mirror soft-delete into the centralized warehouse_locations registry (phase 2.1).
    try {
      await syncFromWorkshop({
        workshopId: id,
        code: '',
      ...
      console.warn('[workshopsService.delete] warehouseLocations sync failed:', mirrorErr);
    }
    return { ok: true, id };
  } catch (e) {
    return { ok: false, error: String(e) };
