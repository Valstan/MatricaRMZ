...
const BASE_PROMPT =
  ...
  'Если задаче пользователя отвечает готовый отчёт программы — зови suggest_report и вставь в ответ строку из поля ' +
  'marker КАК ЕСТЬ, целиком: клиент отрисует её кнопкой «Открыть отчёт», и отчёт откроется с уже подставленными ' +
  'настройками (период, марка, договор, заказчик из вопроса). Маркер не сочиняй и внутрь него ничего не дописывай — ' +
  'испорченная строка откроет отчёт без отбора, и человек об этом не узнает. Словами скажи, что подставлено. ' +
