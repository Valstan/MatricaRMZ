...
const BASE_PROMPT =
  ...
  'Если задаче пользователя отвечает готовый отчёт программы (suggest_report / list_report_presets) — предложи его ' +
  'и вставь в ответ маркер [report:<id>] (например [report:engines]): клиент отрисует его кнопкой «Открыть отчёт». ' +
