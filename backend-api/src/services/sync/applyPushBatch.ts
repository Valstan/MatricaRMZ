...
export async function applyPushBatch(
  ...
  const result = await db.transaction(async (tx) => {
        ...
        const written = await tx
          ...
          .values(
            rows.map((r) => ({
              ...
              hasOwnNumber: r.has_own_number ?? null,
              hasOwnNumberOverride: r.has_own_number_override ?? null,
              selected: r.selected,
    ...
    // CardDrafts (owner-private working snapshots; no sharing — only the creator may write)
    {
      const raw = grouped.get(SyncTableName.CardDrafts) ?? [];
      const parsedAll = parseRows(SyncTableName.CardDrafts, raw, cardDraftRowSchema);
      if (parsedAll.length > 0 && actorId) {
        const rows = await filterStaleBySeqOrUpdatedAt(cardDrafts, parsedAll, SyncTableName.CardDrafts);
        const ids = rows.map((r) => r.id);
        const existing =
          ids.length === 0
            ? []
            : await tx.select().from(cardDrafts).where(inArray(cardDrafts.id, ids as any)).limit(50_000);
        const existingMap = new Map<string, any>();
        for (const e of existing as any[]) existingMap.set(String(e.id), e);
        ...
        for (const r of rows) {
          const cur = existingMap.get(String(r.id));
          if (!cur) {
            // New draft: owner is always the creator (actor), regardless of client payload.
            allowed.push({ ...r, owner_user_id: actorId });
            continue;
          }
          const ownerOk = String(cur.ownerUserId ?? '') === actorId;
          if (!ownerOk && !actorIsAdmin) throw new Error('sync_policy_denied: card_draft_owner');
          allowed.push({ ...r, owner_user_id: String(cur.ownerUserId ?? actorId) });
        }
          ...
          await tx
            ...
            .values(
              allowed.map((r) => ({
                ...
                cardId: r.card_id as any,
                kind: r.kind,
                title: r.title ?? null,
                payloadJson: r.payload_json ?? null,
                baseUpdatedAt: r.base_updated_at ?? null,
                createdAt: r.created_at,
                updatedAt: r.updated_at,
                deletedAt: r.deleted_at ?? null,
                syncStatus: 'synced',
    ...
    // проставляет строке last_server_seq, а без него инкрементальный pull
    // (`last_server_seq > since`) не отдаёт строку НИКОГДА (в SQL `NULL > n` — не TRUE).
    // С 17.09.2026 тот же штамп ставит и сам журнал (`stampRowSeq` в ledgerService) —
    // для писателей, идущих мимо applyPushBatch; здесь номер ложится повторно тем же значением.
    //
