...
import { isAiEnabled } from './ai/common.js';
...
function getReadonlyPool(): Pool | null {
  ...
  if (!user || !password || !database) {
    ...
    // logInfo вместо logError: AI отключён означает что эти env не нужны.
    // Если оператор включит AI без настройки RO-credentials — лог поможет понять
    // что нужно настроить AI_DB_RO_USER/PASSWORD, но это не «критичная ошибка».
    logInfo('ai chat learning disabled: missing read-only DB env (set AI_DB_RO_USER / AI_DB_RO_PASSWORD to enable)', {
      missingUser: !user,
...
export function startAiAgentChatLearningService() {
  ...
  // Если глобальный AI-рубильник выключен — не запускаем learning вообще.
  // Это устраняет периодические критичные ошибки "missing read-only DB env"
  // в админке, когда AI всё равно отключён (например, из-за geo-block).
  if (!isAiEnabled()) {
    logInfo('ai chat learning service skipped: AI_ENABLED=false');
    return;
  }
