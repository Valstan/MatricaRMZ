...
const CLIENT_PATTERNS: Array<{ re: RegExp; info: MatchInfo }> = [
  ...
  {
    // Телеметрия апдейтера: существующий клиент скачал полный installer вместо дельты.
    // Свежие установки телеметрию не шлют → это чистый сигнал «дельта не сработала».
    // (Строки method=delta этому паттерну не соответствуют — садятся только в дневной лог.)
    re: /update-applied method=full/i,
    info: {
      code: 'client.update.full_download',
      title: 'Клиент скачал полный installer (дельта не сработала)',
      category: 'network',
      severity: 'warn',
    },
  },
