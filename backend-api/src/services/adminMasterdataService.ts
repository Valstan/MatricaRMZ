...
function parseBooleanJson(raw: string | null | undefined): boolean {
...
async function isEngineEntityType(entityTypeId: string): Promise<boolean> {
  const rows = await db
...
async function recordEngineStatusAudit(args: {
  ...
  const from = parseBooleanJson(args.fromRaw);
  const to = parseBooleanJson(args.toRaw);
  if (from === to) return;
  // Серверная запись sync-таблицы audit_log → единый ledger-путь (last_server_seq), иначе
  // запись не доезжает до клиентов инкрементальным pull'ом и висит pending без seq.
  ...
  await insertChangeLog(
    ...
    auditId,
    {
      ...
      action: 'engine_status_change',
      entity_id: args.entityId,
      table_name: SyncTableName.Entities,
      payload_json: JSON.stringify({
        engineId: args.entityId,
        code: args.code,
        label: STATUS_LABELS[args.code] ?? args.code,
        from,
        to,
      }),
      created_at: args.ts,
...
export async function setEntityAttribute(
  ...
  if (existing[0]) {
    ...
    await insertChangeLog(SyncTableName.Entities, entityId, payload, actor, options.allowSyncConflicts ? { allowSyncConflicts: true } : {});
  }

  if (ENGINE_STATUS_CODE_SET.has(code) && (await isEngineEntityType(String(e[0].typeId)))) {
    await recordEngineStatusAudit({
      actor,
      entityId,
      code: code as StatusCode,
      fromRaw: existing[0]?.valueJson == null ? null : String(existing[0].valueJson),
      toRaw: payloadJson,
      ts,
    });
  }
...
export async function softDeleteEntity(actor: Actor, entityId: string, options: { allowSyncConflicts?: boolean } = {}) {
  ...
  if (incoming.length > 0) {
    ...
    return {
      ok: false as const,
      error:
        `Нельзя удалить: на запись ссылаются активные сущности (${breakdown}). ` +
        `Сначала отвяжите связи или используйте «Удалить вместе со ссылками».`,
    };
  }
