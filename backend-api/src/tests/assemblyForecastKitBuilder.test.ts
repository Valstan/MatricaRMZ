...
describe('buildAssemblyForecastKits — Фаза 3: подстановка запасного варианта при отсутствии основного', () => {
  ...
  // Фаза 3b: позиции с вариантами едут в кит целиком (для пулинга в симуляции).
  it('Фаза 3b: kit.positions содержит варианты — основной первым, запасные по убыванию стока', () => {
    const result = buildAssemblyForecastKits({
      headerRows: [header({ id: 'bom-sub', engineBrandId: 'brand-a' })],
      lineRows: positionLines,
      nomenclatureById: positionNoms,
    ...
    const positions = result.kits[0]!.positions ?? [];
    expect(positions).toHaveLength(1);
    expect(positions[0]!.positionKey).toBe('piston');
    // Основной первым, затем запасные по убыванию текущего остатка (c=10 раньше b=3).
    expect(positions[0]!.options.map((o) => o.nomenclatureId)).toEqual(['piston-def', 'piston-c', 'piston-b']);
    expect(positions[0]!.options.every((o) => o.qtyPerEngine === 6)).toBe(true);
  });

  it('Фаза 3b: позиции-одиночки и строки без positionKey НЕ создают positions', () => {
    const result = buildAssemblyForecastKits({
      headerRows: [header({ id: 'bom-solo', engineBrandId: 'brand-a' })],
      lineRows: [
        line({ bomId: 'bom-solo', componentNomenclatureId: 'piston-def', componentType: 'piston', positionKey: 'solo', isDefaultOption: true }),
        line({ bomId: 'bom-solo', componentNomenclatureId: 'piston-b', componentType: 'piston' }),
      ],
      nomenclatureById: positionNoms,
      brandLabels,
      stockByNomenclatureId: new Map(),
    });
    expect(result.kits[0]!.positions).toBeUndefined();
    expect(result.kits[0]!.parts).toHaveLength(2);
  });

  it('Фаза 3b: positions есть и без stock-мапы (порядок запасных стабилен по строкам BOM)', () => {
    const result = buildAssemblyForecastKits({
      headerRows: [header({ id: 'bom-sub', engineBrandId: 'brand-a' })],
      lineRows: positionLines,
      nomenclatureById: positionNoms,
