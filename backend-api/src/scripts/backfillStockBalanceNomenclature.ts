...
async function main() {
  ...
  const orphanRows = await db
    .select({
      ...
      warehouseLocationId: erpRegStockBalance.warehouseLocationId,
  ...
  for (const row of orphanRows) {
    ...
    if (!nomenclatureId) {
      orphaned++;
      console.log(`  ORPHAN: balance ${row.id} (part_card_id=${partCardId}, warehouse_location_id=${row.warehouseLocationId ?? 'null'}, qty=${row.qty}) — no matching nomenclature`);
      continue;
    }
    if (!isDryRun) {
      await db
