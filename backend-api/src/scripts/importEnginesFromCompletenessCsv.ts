...
// Phase 3.7 WS2: directory-native. Parts are directory_parts rows indexed by name key.
async function loadExistingPartByKey(): Promise<Map<string, { id: string }>> {
...
async function main() {
  ...
  const existingPartByKey = await loadExistingPartByKey();
  let createdParts = 0;
  let updatedParts = 0;
  let cleanedParts = 0;
  ...
  for (const desiredPart of parsed.parts.values()) {
    ...
    try {
      ...
      if (!rec) {
        const created = await createDirectoryPart({ name: desiredPart.name });
        let partId = '';
        if (created.ok) partId = created.part.id;
          ...
          else throw new Error(`Не удалось создать деталь: ${desiredPart.name} (${created.error})`);
        }
        rec = { id: partId };
        existingPartByKey.set(desiredPart.key, rec);
        createdParts += 1;
      }
      // directory-native: brand-links live in directory_parts.brand_links_json.
      const cur = await getWarehouseNomenclaturePartSpec({ nomenclatureId: rec.id });
      if (!cur.ok) throw new Error(`Не удалось получить спецификацию детали ${rec.id}: ${cur.error}`);
      const baseSpec = cur.spec ?? { code: null, templateId: null, dimensions: [], brandLinks: [] };
      const currentByBrandId = new Map<string, PartSpecBrandLink>();
      for (const link of baseSpec.brandLinks) {
        const brandId = String(link.engineBrandId || '').trim();
        if (brandId && !currentByBrandId.has(brandId)) currentByBrandId.set(brandId, link);
      }

      const desiredBrandIdsForPart = new Set<string>();
      const nextLinks: PartSpecBrandLink[] = [];
      for (const [brandKey, qty] of desiredPart.qtyByBrandKey.entries()) {
        ...
        const desiredAssembly = cleanCell(desiredPart.assemblyByBrandKey.get(brandKey) || existingLink?.assemblyUnitNumber || '');
        if (!desiredAssembly) {
          // no assembly to set — preserve any existing link untouched
          if (existingLink) nextLinks.push(existingLink);
          continue;
        }
        nextLinks.push({ id: existingLink?.id ?? randomUUID(), engineBrandId: brandId, assemblyUnitNumber: desiredAssembly, quantity: qty });
      }
      // brand-links present but no longer desired are dropped (not carried into nextLinks).
      for (const [brandId] of currentByBrandId.entries()) {
        if (!desiredBrandIdsForPart.has(brandId)) cleanedParts += 1;
      }
      const up = await upsertWarehouseNomenclaturePartSpec({ nomenclatureId: rec.id, spec: { ...baseSpec, brandLinks: nextLinks } });
      if (!up.ok) throw new Error(`Не удалось сохранить спецификацию детали ${rec.id}: ${up.error}`);
      updatedParts += 1;
    } catch (error) {
  ...
  for (const [number, engineId] of engineIdByNumber.entries()) {
    ...
    const del = await detachIncomingLinksAndSoftDeleteEntity(actor, engineId);
