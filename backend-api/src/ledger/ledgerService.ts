...
export function signAndAppendDetailed(
  ...
  // Atomic seq-allocate + append: prevents two processes signing against the same
  // lastSeq and emitting duplicate seqs (encryption already done above, off-lock).
  const { signed, block } = ledger.signAndAppend(encryptedPayloads, keys.privateKeyPem, keys.publicKeyPem);
