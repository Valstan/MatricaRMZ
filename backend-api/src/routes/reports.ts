...
const TABLES: TableSpec[] = [
  ...
  },
  {
    ...
    // заметок, файлы с чужого Верстака. Админ здесь ничего не выигрывает — он
  ...
  },
  {
    name: 'chat_messages',
    label: 'Чат: сообщения',
    // Per-user private DMs — admin-only (the dedicated /chat export gate is
    // ChatExport, also admin-only). The builder has no row-ownership scoping, so
    // ChatUse (held by every operator) must NOT reach it. (security-hardening H2)
    permission: [PermissionCode.ChatExport],
    table: chatMessages,
    columns: [
  ...
  },
  {
    name: 'notes',
    label: 'Заметки',
    // Per-owner private notes (title + bodyJson) — admin-only. The builder has no
    // ownerUserId/note_shares scoping, so ReportsView (held by every operator)
    // must NOT reach it. (security-hardening H3)
    permission: [PermissionCode.AdminUsersManage],
    table: notes,
    columns: [
...
export function allowedForTable(perms: Record<string, boolean>, table: TableSpec): boolean {
...
// without this a non-admin with any *.view could dump those rows. Drop sensitive
// attribute rows unless the caller is an admin (who legitimately runs payroll
// reports). (security-hardening-2026-06 H4)
export function redactReportRows(
  meta: TableSpec,
  rows: any[],
  perms: Record<string, boolean>,
): any[] {
  if (meta.name === 'attribute_values' && perms[PermissionCode.AdminUsersManage] !== true) {
    return rows.filter((r) => !isHiddenAttributeName(r?.attributeName));
  }
  return rows;
}
  ...
  try {
    ...
    for (const t of parsed.data.tables) {
      ...
      tables.push({
        ...
        rows: redactReportRows(meta, rows as any[], perms),
      });
    }
