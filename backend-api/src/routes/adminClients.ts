...
adminClientsRouter.get('/clients', async (_req, res) => {
  try {
    ...
    const fullNames = await resolveLoginsToFullNames(rows.map((r) => r.lastUsername));
    return res.json({
      ok: true,
      rows: rows.map((r) => {
        const login = (r.lastUsername ?? '').trim();
        return {
          clientId: r.clientId,
          ...
          lastUsername: r.lastUsername ?? null,
          lastFullName: login ? (fullNames[login.toLowerCase()] ?? null) : null,
...
adminClientsRouter.post('/clients/:clientId/sync-request', async (req, res) => {
  ...
  // The `reinstall` command is consumed only by the external watchdog. Issuing it
  // to a client that predates the watchdog rollout leaves it stuck unacked in the
  // client's single sync-request slot (the client skips it; nothing clears it).
  // Refuse it unless the client reports a watchdog-carrying build.
  if (parsed.data.type === 'reinstall') {
    const settings = await getOrCreateClientSettings(clientId).catch(() => null);
    if (!settings || !clientHasWatchdog(settings.lastVersion)) {
      return res.status(409).json({
        ok: false,
        error: 'У клиента ещё нет watchdog (старая версия) — команда переустановки зависла бы. Дождитесь обновления клиента.',
      });
    }
  }
