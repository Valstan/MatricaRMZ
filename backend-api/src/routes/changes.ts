...
changesRouter.get('/', async (req, res) => {
  try {
    ...
    const rows = await db
      .select()
      .from(changeRequests)
      // RBAC #474 M4: user-change requests (admin_user) have their own approval
      // queue/UI — keep them out of the generic entity-edit "Изменения" module.
      .where(and(eq(changeRequests.status, status), ne(changeRequests.tableName, 'admin_user')))
