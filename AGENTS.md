...
**ADR-0007 «тактика напрямую, знание через курацию»:** любой sibling-репо можно **читать read-only напрямую** (`../<project>/`) для тактических фактов — API-контракт, форматы, docs, handoff соседа. **Соседние репо не синхронизируются** (мандат владельца 2026-08-04): ни `fetch`, ни `pull`, ни `checkout` в чужих репо — только чтение того, что лежит локально (незакоммиченное чужое тоже видно и свежее закоммиченного). Писать/коммитить в чужой репо нельзя; прочитанное — не контракт (зависимость от чужого API = интеграция → письмо в мозг). Знание/директивы/переносимые уроки — по-прежнему через mailbox.
...
- **brain → MatricaRMZ:** brain пишет в `../brain_matrica/mailboxes/MatricaRMZ/from-brain/*.md`. Мы читаем **двухканально, без синхронизации чужого репо** (мандат владельца 2026-08-04): локально `../brain_matrica/mailboxes/MatricaRMZ/from-brain/*.md` (как есть, без `pull`) + через GitHub API/веб `main` того же проекта (`mailbox/to-brain/*.md`), без clone/fetch/pull. Свежесть — по истории именно этого пути, набор = объединение источников (детали — в `/start` §0).
...
- **Правки файлов, ветки, коммиты, PR, авто-мерж** — авто, без переспрашивания. **Подтверждение = зелёные гейты:** build `shared`+`ledger` → `corepack pnpm -r typecheck` + `corepack pnpm -F @matricarmz/electron-app typecheck:test` + `lint` → `corepack pnpm -r test` → **CDP e2e-smoke** (`verifier-electron`, skill `verify`) при UI-правках → CI зелёный. Прогонять перед мержем; красный гейт = стоп, чиню, не мержу.
  - Тестовый гейт расширен с одного `backend-api` до всего монорепо 2026-08-22: правила проекта живут не только в бэкенде — например, тест-сторож «оператору не показывают служебный код» (`electron-app/src/main/services/reports/humanLabels.guard.test.ts`) при прежнем гейте не запускался бы вовсе.
  ...
  - **Известный флейк, не считать красным гейтом с первого раза:** `android-app/src/core/boot.test.ts` → «без логина SyncManager честно отказывает» падает под параллельной нагрузкой `-r test` (`expected true to be false`), а в одиночном прогоне зелёный — 22.08.2026 проверено: 3 из 3 одиночных зелёные, 1 из 2 полных красный. Красный **только этого** теста — перезапусти его отдельно (`corepack pnpm -F @matricarmz/android-app exec vitest run src/core/boot.test.ts`); зелёный в одиночку = гейт пройден. Любой другой красный — настоящий.
...
8a. **APK планшета — качать локально, класть на прод через `scp`** (не забывать: без этого шага планшеты не обновятся). Самообновление Android читает свежайший `.apk` из `<updatesDir>/android/` (`/opt/matricarmz/updates/android/`) и **версию берёт из имени файла** (`extractVersionFromFileName` в `updateDispatcherService.ts`, старший по `compareAppVersion`), поэтому имя обязано содержать номер выпуска. Размер и sha256 сервер считает сам, `latest.yml`/blockmap у Android нет, рестарт не нужен — кэш инвалидируется по mtime.
   ...
   ```
   > ⚠️ **Не гонять `gh release download` на самом проде** — к GitHub CDN оттуда воспроизводимо валится TLS-таймаут (memory `prod-gh-release-download-tls-timeout`). Каталог `updates/` принадлежит `valstan`, так что `scp` идёт без `sudo`. Проверка: `curl -fsSk "https://127.0.0.1/dispatcher/update-plan?platform=android&current=<прежняя>"` → план с новой версией (а не `up-to-date`).
