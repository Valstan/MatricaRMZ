...
- [`docs/PENDING_FOLLOWUPS.md`](docs/PENDING_FOLLOWUPS.md) — **только открытые** задачи/техдолги/отложенные (🔴 / ⏳ / 🟡 / 🟢) + метки старения. Завершённое сюда **не кладём** (выпиливается при закрытии). **Читать только если задача про open issues.**
  - **Одна дверь для дат** ([#152](../brain_matrica/cross-project-ideas/ideas/152-a-date-promised-outside-the-alarm-clock-is-not-a-date.md)): дата, названная в письме, в мандате, в теле PR или в разговоре с владельцем, **в том же ходе** попадает в §«📅 Календарь ротаций и дедлайнов» этого файла. Не попала — это не дата, а намерение: будильника нет, и наступит она незамеченной. Календарь — единственное место, где даты живут (в `SESSION_HANDOFF.md` их не держим — он перезаписывается каждой сессией). Проверка на замыкание — сгрепать дату обратно из календаря сразу после записи, а не понадеяться, что вписал.
...
**Одна память на экосистему (brain D-097, [ADR-0013](../brain_matrica/adr/0013-memory-layout-index-plus-chunk-files.md), 2026-09-21):**

- **Читать индекс → файл, никогда реестр целиком.** Память Мозга разложена как индекс + файл на запись (`cross-project-ideas/GOTCHAS.md` — индекс, тела — `gotchas/G<N>.md`; `REFERENCE.md` — индекс, тела — `reference/R<N>.md`; идеи — `INDEX.md` + `ideas/NNN-*.md`). Поиск по всей памяти одной командой, без подключений: `python ../brain_matrica/.claude/scripts/brain_find.py "слова симптома или темы"` — печатает лучшие строки индексов и путь к файлу записи (2–5 КБ, читать целиком). Это тот же consult-library рефлекс, только шире: перед долгой отладкой знакомо выглядящей проблемы и перед вводом нового — сначала `brain_find.py`, потом свой `docs/GOTCHAS.md`. Не нашёл с первого запроса — это письмо Мозгу, не молчание.
- **Долговременное и переносимое — только у Мозга.** У нас локально живёт оперативное (handoff, PENDING, карта кода, профиль машины) и специфичное для стека/домена. Переносимая находка уходит письмом (#009); у себя остаётся **одна строка-указатель** на `G<N>`/`#NNN`, не копия текста (#087: правка в одной из копий — расхождение, не лечение). Писать в память Мозга нельзя — только письмом; он раскладывает и отвечает номером записи.
- **Свой реестр > 50 КБ — на ту же раскладку** (индекс + файл на запись), потолок держит гейт. Текущее состояние и порядок раскладки — [`docs/plans/memory-index-plus-chunks-2026-09.md`](docs/plans/memory-index-plus-chunks-2026-09.md).
- **Чистка раз в месяц** вместе с ротацией PENDING: дубли слить, устаревшее пометить, противоречие старого новому — снять.
...
- **Правки файлов, ветки, коммиты, PR, авто-мерж** — авто, без переспрашивания. **Подтверждение = зелёные гейты:** build `shared`+`ledger` → `corepack pnpm -r typecheck` + `corepack pnpm -F @matricarmz/electron-app typecheck:test` + `lint` → `corepack pnpm -r test` → **CDP e2e-smoke** (`verifier-electron`, skill `verify`) при UI-правках → CI зелёный. Прогонять перед мержем; красный гейт = стоп, чиню, не мержу.
  ...
  - **Гейт гоняет CI, а не память агента** ([#125](../brain_matrica/cross-project-ideas/ideas/125-a-new-gate-masks-an-older-red-one.md)). Гейт, который агент прогнал у себя и отчитался словами, — не гейт: он невоспроизводим и его зелёность нечем предъявить. Каждый гейт из списка выше обязан иметь свой шаг в `.github/workflows/`. **Единственное именованное исключение — CDP e2e-smoke** (`verifier-electron`): он требует живого Electron с дисплеем и гоняется руками на машине разработчика; поэтому прогон смоука называется в теле PR явной строкой «смоук прогнан / не гнался, почему», а не подразумевается. Новый гейт заводится сразу шагом CI — иначе он молча маскирует более старый красный.
- **Фича на 3+ PR, вводящая новую границу, получает финальный проход по всей ветке** ([#166](../brain_matrica/cross-project-ideas/ideas/166-per-task-review-is-blind-to-seams-between-tasks.md)). Новая граница — это роль, право, арендатор, демо-режим, режим доступа: то, что обязано держать по всему периметру, а не в одном экране. По-PR-ное ревью смотрит куски и слепо к швам между ними: каждый PR зелёный, а граница в сборе дырявая. Перед включением такой фичи — один сквозной проход по диффу всей серии целиком (`git diff <база>..HEAD`), с вопросом «через какую ещё дверь входят в это право». Свежий пример цены — **M135** (право «всем админам» циклом + строка синка без серверного гейта, класс brain [G363](../brain_matrica/cross-project-ideas/GOTCHAS.md#g363)): три PR порознь были зелёные.
...
4. On prod server: `bash scripts/prod-ops/deploy-backend.sh` — скрипт сам делает `git pull --ff-only`, забирает артефакт `backend-dist` последней успешной сборки `main`, проверяет его во временном каталоге, раскладывает `dist` трёх пакетов + миграции и перезапускает primary → health → secondary (#912). **Прод ничего не собирает и ничего не устанавливает.**
   > **Откат встроен (с 18.09).** Прежний `dist` уходит в `.deploy-backup/<стамп>/` до замены; при отказе health primary скрипт сам возвращает его и перезапускает. Хранятся последние 3 копии (`MATRICA_DEPLOY_BACKUP_KEEP`). Ручной откат — `cp -a .deploy-backup/<стамп>/<путь> <путь>` + рестарт юнитов. До этого отката не было вовсе, а единственный путь назад — повторный выкат прошлого артефакта — живёт `retention-days: 14`.
   > **Неустановленная рантаймовая зависимость останавливает выкат ДО замены `dist`:** скрипт сверяет `dependencies` бэкенда с `backend-api/node_modules/` и называет недостающее. Ставить — фильтрованно (см. предупреждение ниже), затем повторить выкат.
   > Окно ожидания: primary — 120 с, secondary — 60 с (`MATRICA_DEPLOY_PRIMARY_WAIT` / `_SECONDARY_WAIT`). Меньше 90 с у primary брать нельзя — см. M100 в шаге 10.
   > ⚠️ **`corepack pnpm install` в корне на проде — не делать.** Он вернёт сборочную обвязку клиента, снятую 15.09.2026 (+574 МБ на диске, где свободно 4 ГБ). Если релиз добавил **рантаймовую** зависимость бэкенда (новая строка в `dependencies` у `backend-api`) — только тогда и только фильтрованно: `corepack pnpm install --filter "@matricarmz/backend-api..."`, после чего прогнать `node scripts/prod-ops/prune-virtual-store.mjs` (см. `scripts/prod-ops/README.md`). Голый `pnpm` вместо `corepack pnpm` не использовать никогда — на боксе он новее пиннутого и работает с другим стором (GOTCHAS **M133**).
   > ⚠️ **Если фильтрованный `pnpm install` виснет на VPS** (на `added N-1/N` или зомби-процессом) — это флаки-сеть + бесполезная закачка electron-бинаря; гнать `env ELECTRON_SKIP_BINARY_DOWNLOAD=1 npm_config_fetch_timeout=45000 npm_config_fetch_retries=10 corepack pnpm install` (GOTCHAS **M16**). Для code-only релиза (lockfile не менялся) install можно вовсе пропустить — только build серверных пакетов. Застрявшие install'ы убивать по PID, НЕ `pkill -f 'corepack pnpm install'` (самоматчит ssh-команду).
   > ⚠️ **Сборки на проде нет вовсе** (исторически собирались ТОЛЬКО серверные пакеты) (`shared` / `backend-api` / `web-admin`): backend-сервис запускается из **скомпилированного** `backend-api/dist/index.js` (systemd `ExecStart=node …/dist/index.js`), поэтому их build обязателен. **Electron-КЛИЕНТ (`.exe`) собирается GitHub Actions** (workflow «Release Electron Windows», триггерится тегом на шаге 3) и на прод приходит **готовым артефактом** — только скачивается (шаг 7), **никогда не собирается на проде**. **Не запускать на проде нефильтрованный `pnpm build` / `pnpm -r build`** — он соберёт и `electron-app` (клиент); всегда явный `-F` на серверные пакеты.
...
10. Restart services: `sudo systemctl restart matricarmz-backend-primary.service matricarmz-backend-secondary.service`. Verify with `curl -fsk https://127.0.0.1/health` (should report new version).
    > ⚠️ **Приёмка рестарта — по каждому порту отдельно, не через nginx** (GOTCHAS **M100**). `https://127.0.0.1/health` идёт в upstream из двух инстансов и отвечает `200`, пока жив хотя бы один — упавший primary (а с ним все фоновые джобы) этой проверкой не виден. Проверять `http://127.0.0.1:3001/health` и `:3002/health` порознь. Primary биндится **~50 с** (поднимает фоновую обвязку), secondary — за секунды; окно ожидания брать не меньше 90 с, а `systemctl is-active` за готовность не считать: он печатает `active` до бинда.
