...
4. On prod server: `bash scripts/prod-ops/deploy-backend.sh` — скрипт сам делает `git pull --ff-only`, забирает артефакт `backend-dist` последней успешной сборки `main`, проверяет его во временном каталоге, раскладывает `dist` трёх пакетов + миграции + статику админки `web-admin/dist` (с 21.09; архив без неё отвергается до сноса) и перезапускает primary → health → secondary (#912). **Прод ничего не собирает и ничего не устанавливает.**
   > **Откат встроен (с 18.09).** Прежний `dist` уходит в `.deploy-backup/<стамп>/` до замены; при отказе health primary скрипт сам возвращает его и перезапускает. Хранятся последние 3 копии (`MATRICA_DEPLOY_BACKUP_KEEP`). **Откат — команда скрипта** (с 21.09, brain #324/G373): `bash scripts/prod-ops/deploy-backend.sh rollback [<стамп>]` возвращает копию (по умолчанию последнюю) и перезапускает primary → health → secondary; `… snapshot` кладёт копию текущего `dist` без рестарта. Репетиция отката без смены версии — `snapshot` → `rollback`; приёмка #324 без прогона отката не закрывается. До 18.09 отката не было вовсе, а единственный путь назад — повторный выкат прошлого артефакта — живёт `retention-days: 14`. Артефакты, собранные до 21.09 (без `web-admin/dist`), скрипт отвергает — назад только через `rollback`.
