...
export type EngineListItem = {
  ...
  /**
   * Дата ЛЮБОЙ последней операции по двигателю — акта комплектности, дефектовки, наряда,
   * движения склада, а не только истории ремонта. Тем и отличается от соседей:
   * `lastHistoryAt` и `lastSheetAt` считаются по строкам истории ремонта и этапам работ,
   * и двигатель, по которому вчера провели дефектовку, по ним выглядел бы «без движения».
   * Нужна индикатору срока ремонта: карточка, по которой давно ничего не делали, красным
   * не горит (`COUNTDOWN_STALE_DAYS`) — там не просрочка, а незакрытый учёт.
   */
