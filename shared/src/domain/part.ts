...
export type PartSpecBrandLink = {
  ...
  inDefectAct?: boolean;
  // Владелец 22.09.2026: у части деталей бывает СВОЙ номер, набитый на детали. Флаг ставится
  // здесь — в комплекте марки — и наследуется листами всех двигателей этой марки (переопределить
  // можно в конкретном листе). В бланке дефектовки поле под номер рисуется только у отмеченных:
  // у остальных печатается просто строка, и весь список умещается на лист.
