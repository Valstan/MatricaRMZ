...
/**
 * Блоки подписей наряда — свои на каждый вид формы (2026-07-31).
 *
 * Наряд на сборку: двухфазная процедура «Выдача наряда» (ПДО выдал → ОТК согласовал →
 * нач. цеха принял в работу) и «Завершение наряда» (нач. цеха сдал → ОТК принял → ПДО
 * закрыл). Гриф «Утверждаю» (директор) — отдельный, печатается над шапкой.
 *
 * Простой наряд (обычный / ремонт / изготовление): двухфазной процедуры нет — бригада
 * расписывается в своей таблице, а внизу листа согласуют нормировщик, кадры, ОТК и мастер
 * цеха. Блоки `issue`/`completion` при этом сохраняются с ПУСТЫМИ `defaultCaptions`: сами
 * собой они больше не печатаются, но уже подписанные наряды не осиротеют — их слоты
 * по-прежнему находятся по blockId и выводятся на лист.
 */
  ...
  return [
    {
      ...
      title: 'Согласовано',
      dateLineLabel: 'Дата выполнения',
      defaultCaptions: ['Специалист по нормированию', 'Специалист по кадрам', 'ОТК', 'Мастер цеха'],
    },
    { id: 'issue', title: 'Выдача наряда', dateLineLabel: 'Дата выдачи', defaultCaptions: [] },
    { id: 'completion', title: 'Завершение наряда', dateLineLabel: 'Дата выполнения', defaultCaptions: [] },
  ];
}
