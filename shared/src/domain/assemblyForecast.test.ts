...
describe('assemblyForecast', () => {
  ...
  it('requiredParts получают sourceWarehouseId при наличии warehouseStockBins', () => {
    const stock = new Map<string, number>([
      ['p1', 2],
      ['p2', 2],
    ]);
    const warehouseStockBins = new Map([
      ['p1', [{ warehouseId: 'w-shop-1', warehouseLabel: 'Цех №1', qty: 2 }]],
      ['p2', [{ warehouseId: 'w-shop-2', warehouseLabel: 'Цех №2', qty: 2 }]],
    ]);
    const kits = mergeBrandKits([
      { partId: 'p1', brandId: 'b1', brandLabel: 'B1', partName: 'Гильза', article: '', qtyPerEngine: 1 },
      { partId: 'p2', brandId: 'b1', brandLabel: 'B1', partName: 'Поршень', article: '', qtyPerEngine: 1 },
    ...
    const row = res.rows.find((r) => r.status === 'ok');
    expect(row?.requiredParts?.find((p) => p.partId === 'p1')?.sourceWarehouseId).toBe('w-shop-1');
    expect(row?.requiredParts?.find((p) => p.partId === 'p2')?.sourceWarehouseId).toBe('w-shop-2');
  });

  it('requiredParts split на несколько строк когда деталь идёт с разных складов', () => {
    const stock = new Map<string, number>([['p1', 3]]);
    const warehouseStockBins = new Map([
      [
        'p1',
        [
          { warehouseId: 'w-shop-1', warehouseLabel: 'Цех №1', qty: 1 },
          { warehouseId: 'w-shop-2', warehouseLabel: 'Цех №2', qty: 2 },
        ],
      ],
    ]);
    const kits = mergeBrandKits([
      { partId: 'p1', brandId: 'b1', brandLabel: 'B1', partName: 'Гильза', article: '', qtyPerEngine: 3 },
    ]);
    const res = computeAssemblyForecast({
      horizonDays: 1,
      targetEnginesPerDay: 1,
    ...
    const row = res.rows.find((r) => r.status === 'ok');
    const p1Rows = (row?.requiredParts ?? []).filter((p) => p.partId === 'p1');
    expect(p1Rows.length).toBeGreaterThanOrEqual(2);
    const totalQty = p1Rows.reduce((s, p) => s + p.qty, 0);
    expect(totalQty).toBe(3);
    const warehouses = new Set(p1Rows.map((p) => p.sourceWarehouseId));
    expect(warehouses.has('w-shop-1')).toBe(true);
    expect(warehouses.has('w-shop-2')).toBe(true);
  });

  it('variantKey стабилен независимо от split по складам', () => {
    const kits = mergeBrandKits([
      { partId: 'p1', brandId: 'b1', brandLabel: 'B1', partName: 'Гильза', article: '', qtyPerEngine: 2 },
    ]);
    const stock = new Map<string, number>([['p1', 2]]);
    const oneWarehouse = computeAssemblyForecast({
      horizonDays: 1,
      targetEnginesPerDay: 1,
      warehouseId: null,
      kits,
      stockByNomenclatureId: stock,
      warehouseStockBins: new Map([['p1', [{ warehouseId: 'w1', warehouseLabel: 'Цех A', qty: 2 }]]]),
      incomingLines: [],
    });
    const twoWarehouses = computeAssemblyForecast({
      horizonDays: 1,
      targetEnginesPerDay: 1,
      warehouseId: null,
      kits,
      stockByNomenclatureId: new Map<string, number>([['p1', 2]]),
      warehouseStockBins: new Map([
        [
          'p1',
          [
            { warehouseId: 'w1', warehouseLabel: 'Цех A', qty: 1 },
            { warehouseId: 'w2', warehouseLabel: 'Цех B', qty: 1 },
          ],
        ],
      ]),
      incomingLines: [],
    });
    expect(oneWarehouse.rows[0]?.variantKey).toBe(twoWarehouses.rows[0]?.variantKey);
  });
