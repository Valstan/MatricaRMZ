...
// ── Единицы измерения 1С → справочник программы ─────────────────────────────────
//
// 1С сокращает единицы иначе, чем наш справочник («пар» ↔ «пара», «компл» ↔
// «комплект», «пог. м» ↔ «м»). Возвращаем каноническое название для поиска в
// справочнике; неизвестные — как есть (лягут без единицы, поправимо в карточке).

const UNIT_1C_SYNONYMS: Record<string, string> = {
  'пар': 'пара',
  'компл': 'комплект',
  'к-т': 'комплект',
  'пог м': 'м',
...
export function resolve1cUnit(unit: string): string {
  const raw = String(unit ?? '').trim().replace(/\.+$/, '');
  const key = normalizeLookupCompact(raw);
  return UNIT_1C_SYNONYMS[key] ?? raw;
}

// ── Тематическая группа для заводимой номенклатуры ──────────────────────────────
//
// В файле 1С нет номенклатурных групп (отчёт группирует только по складам),
// поэтому раздел определяется по ключевым словам названия. Названия групп —
// существующие группы справочника программы; правила упорядочены: первое
// сработавшее побеждает (например, «щётка стартера» — электрооборудование,
// а просто «щётка» — инструмент). Не распознали — без группы, оператор разложит.

export const STOCK_1C_GROUP_RULES: Array<{ group: string; re: RegExp }> = [
  { group: 'Электрооборудование и КИП', re: /генератор|стартер|реле|свеч[аи]|датчик|кабел|провод(?!ок)|лампа|аккумулятор|изолент|клемм|предохранител|выключател|розетк|светильник|кабельн/i },
  { group: 'Подшипники', re: /подшипник/i },
  { group: 'Инструмент и оснастка', re: /сверло|плашка|метчик|фреза|резец|зенковк|развертк|надфил|напильник|микрометр|индикатор|калибр|штангенциркул|державк|твердосплавн|алмаз|шл\.?\s*круг|шл\.?\s*шкурка|круг.*(отрезн|лепестков|шлифовальн|обдир|абразивн|\d+[.,]\d+\s*22[.,]23|bf)|заклепочник|ключ\s|отвертк|молоток|зубило|керн|бор-?фрез|полотно|ножовочн|щетк[аи](?!.*(стартер|генератор))|электрод/i },
  { group: 'Крепёжные изделия', re: /болт|гайк|винт|шайб|шпильк|заклепк|саморез|гвозд|шплинт|хомут|дюбел/i },
  { group: 'РТИ и уплотнения', re: /кольцо|манжет|сальник|прокладк|уплотнен|рукав|ремень|ремни|шланг|викель|асбест|паронит|сальников|резинов|рти/i },
  { group: 'Запасные части', re: /вкладыш|гильза|поршн|поршень|клапан|коллектор|втулк|валик|распылител|насос|фильтр|патрубок|ремкомплект|р\/к|форсунк|тнвд|турбо|венец|крышк|палец|головк|термостат|маховик|картер|шатун|полумуфт|плунжер|секци[яи]|гидрораспределител|крестовин|штуцер|заглушк|колпач[оё]к|привод|вал\s|шестерн/i },
  { group: 'Закупка · Расходные материалы', re: /эмаль|грунт|краск|герметик|ацетон|ксилол|керосин|растворител|смазк|масло|лента|перчатк|ветошь|очистител|wd-?40|lavr|старт\b|антифриз|тосол|литол|солидол|клей|скотч|мыло|салфетк|обтирочн|канал|пакет|мешк[иа]/i },
  { group: 'Закупка · Материалы и сырьё', re: /лист\s|проволок|труба|трубк|бумаг|войлок|картон|шнур|круг\s+(алюм|ст|латун|бронз|д16|ф?\d)|пруток|полоса|уголок|швеллер|сетк|стеклоткан|фторопласт|капролон|текстолит|жесть|припой|олово|баббит/i },
];

export function classify1cGroup(name: string): string | null {
  const n = String(name ?? '');
  for (const rule of STOCK_1C_GROUP_RULES) {
    if (rule.re.test(n)) return rule.group;
  }
  return null;
}
