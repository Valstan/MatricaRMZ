...
/**
 * Второе исключение из правила «нет подписи — прочерк». Ставится в колонке, которая называет
 * САМ ДОКУМЕНТ («Заявка», «№ двигателя»): прочерк там читается как «ячейку забыли заполнить»,
 * тогда как документ существует и просто не пронумерован. Идентификатора не заменяет ничем —
 * именно попытка «хоть как-то опознать строку» и рождала в этих колонках UUID и его огрызки.
 *
 * Двигатели пришли к этому же тексту раньше и держат свою копию в `options.ts`
 * (`UNKNOWN_ENGINE_NUMBER_LABEL`) — при следующей уборке она сюда и переедет.
 */
export const HUMAN_LABEL_NO_NUMBER = '(без номера)';
...
export const REPORT_TOTAL_LABELS: Record<string, string> = {
  ...
  workSheetRows: 'Строк этапов работ, шт.',
