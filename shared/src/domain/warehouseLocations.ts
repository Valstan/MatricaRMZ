...
export type SystemWarehouseLocation = (typeof SYSTEM_WAREHOUSE_LOCATIONS)[number];

export function isSystemWarehouseLocation(warehouseId: string | null | undefined): warehouseId is SystemWarehouseLocation {
  if (!warehouseId) return false;
  return (SYSTEM_WAREHOUSE_LOCATIONS as readonly string[]).includes(String(warehouseId).trim());
}
