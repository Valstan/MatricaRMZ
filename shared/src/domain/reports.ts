...
export type ReportOptionSource =
  ...
  /** Виды работ для этапов: value — код вида (по нему этап работ себя и называет), подпись — имя. */
  | 'work_sheet_types';
...
export const ENGINES_LIST_REPORT_COLUMNS: ReportColumn[] = [
  ...
  // Этапы работ (15.09.2026): на каком узле двигатель по последней строке этапа работ.
  { key: 'lastSheetNode', label: 'Последний этап работ' },
  { key: 'lastSheetAt', label: 'Дата этапа работ', kind: 'date' },
];
...
export const REPORT_PRESET_DEFINITIONS: ReportPresetDefinition[] = [
  ...
  },
  {
    ...
    filters: [
      ...
      // Порядок — это порядок ступеней каскада на экране: сперва заказчик, он сужает контракты,
      // они вместе — марки. Переставишь — оператор выбирает марку из полного справочника.
      {
        type: 'multi_select',
        key: 'counterpartyIds',
        label: 'Заказчики',
        optionsSource: 'counterparties',
        labelHint: 'Первая ступень отбора: контракты и марки ниже сузятся до выбранных заказчиков. Пусто — все заказчики.',
      },
      {
        type: 'multi_select',
        key: 'contractIds',
        ...
        labelHint: 'Только контракты выбранных заказчиков. Заказчики не выбраны — все контракты.',
      },
      {
        type: 'multi_select',
        key: 'brandIds',
        ...
        labelHint: 'Только марки, встречающиеся в отобранных контрактах и у отобранных заказчиков. Ничего не выбрано — все марки.',
      },
      {
        type: 'select',
        key: 'engineState',
