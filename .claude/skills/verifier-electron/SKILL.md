...
Цель: дать `/verify` рабочий локальный handle для Electron-приложения, чтобы Claude мог запустить app, дойти до карточки двигателя и снять observations.

## Когда использовать

- `verify` встретил UI-change в `electron-app/` и cold-start ищет handle (`.claude/skills/verifier-electron/`).
...
.\.claude\skills\verifier-electron\scripts\setup-env.ps1
.\.claude\skills\verifier-electron\scripts\setup-db.ps1            # -Reset чтобы пересоздать
.\.claude\skills\verifier-electron\scripts\migrate-and-seed.ps1
```
...
```powershell
.\.claude\skills\verifier-electron\scripts\start-backend.ps1
.\.claude\skills\verifier-electron\scripts\start-electron.ps1 -Cdp     # -Cdp включает CDP (порт 9222)
```
...
Stop:
```powershell
.\.claude\skills\verifier-electron\scripts\stop.ps1
```

## Driving в живом UI (для Claude в `/verify`)
...
node .\.claude\skills\verifier-electron\scripts\cdp-drive.mjs
```
...
## CDP-драйв: грабли (выучено 2026-09-12, смоук приказов о ценах)

- **Три модала между логином и меню.** После входа на обновлённом клиенте стоит окно «Что нового» (кнопка «За работу!»), а следом — «Восстановление несохранённых карточек» с черновиками прошлых прогонов (по кнопке «Отклонить» на каждый). Пока они открыты, клики по меню уходят в никуда, а драйвер видит «экран не открылся». Закрывать оба в начале прогона; образец — `cdp-service-price-orders.mjs`.
- **`verify` не видит группу «Снабжение».** У него `admin`, но доступ по разделам ограничен (`TEST-DEPT`), и `nomenclatureUpsert` отвечает «Нет доступа к разделу „Склад“». Для экранов снабжения/склада логиниться `valstan` / `valstan-dev` (суперадмин, разделы не гейтятся); `auth.logout({})` — с объектом, без него вызов отвергается и сессия остаётся прежней.
- **Сущность, созданную мостом, сервер увидит только после `sync.run()`.** Серверная логика (например, перенос цены приказа в карточку) отвечает «Сущность не найдена», пока реплика не отправлена; и наоборот, изменение, сделанное сервером, приезжает в `admin.entities.get` только после следующего `sync.run()`.
- **Поле формы искать через `<label>` → соседний блок**, а не «первый видимый input рядом с текстом»: `FormField` кладёт подпись в `<label>`, а поиск по `div/span` с тем же текстом находит заголовок другого блока и печатает не туда.

## CDP-драйв: грабли (выучено 2026-09-12, смоук пакета владельца 12.09)

- **Состояние списка роумится между прогонами.** Ступени фильтра (`facetFields`) остаются включёнными с прошлого раза, и «щёлкнуть по ступени» во втором прогоне её ВЫКЛЮЧАЕТ — драйвер видит пустой список вариантов и краснеет на ровном месте. Включать только если вариантов ещё нет: сначала `document.querySelector('[data-facet-value^="<id>:"]')`, и лишь потом клик по `[data-facet-field="<id>"]`.
- **Зацепки фильтра — `data-facet-*`, а не текст кнопок.** `[data-facet-toggle]` (кнопка «Фильтры»), `[data-facet-field=<id>]` (ступень), `[data-facet-value=<id>:<значение>]` (вариант со счётчиком), `[data-facet-reset]` (сброс). Поиск вариантов по подписи ловит одноимённые кнопки других панелей.
- **Произвольный путь файлу подсунуть нельзя, и это правильно.** `files:upload` принимает только путь, выданный диалогом (`consumeIssuedPath`), а `files:uploadBlob` в десктопной сборке не зарегистрирован (он планшетный). Фикстуру файла заводить через REST стенда: `POST /auth/login` → `POST /files/upload` (`{name, mime, dataBase64}`) → `POST /files/:id/preview`; образец — `cdp-owner-batch-0912.mjs`.
- **Группа меню у сотрудников — «Персонал и доступ», не «Справочники»** (`GROUP_LABELS` в `shared/domain/uiSections.ts`). Подписи групп брать оттуда, а не по памяти.
- **После `auth.logout({})` форма входа появляется не сразу**: оверлеи («Что нового», модал черновиков) могут перекрывать её и перехватывать клики. Ждать поле пароля в цикле и периодически закрывать оверлеи, а не один раз в начале.

## CDP-драйв: грабли (выучено 2026-09-13, смоук витрины заготовок)

- **Мостовой `auth.login` НЕ переводит рендерер с экрана входа.** Сессия заводится, `auth.status()` честно отвечает `superadmin`, а на экране по-прежнему форма логина — рендерер мост не слушает. Дальше драйвер ищет пункт меню среди кнопок формы и рапортует «пункт меню не найден», хотя пункт есть. **Логиниться формой:** native value-setter в поля + клик «Войти», и только если поле пароля на экране есть (на втором прогоне сессия уже жива).
- **Левая панель меню бывает скрыта** (кнопка «МЕНЮ»), и тогда групп нет в DOM вовсе — симптом тот же «группы не найдено». Сначала раскрыть панель, потом группу (подпись свёрнутой начинается с `▸`).
- **Смоук обязан быть идемпотентным, иначе второй прогон краснеет на ровном месте.** Вкладка с прошлого раза остаётся открытой со СВОИМ состоянием: набранный поиск фильтрует список, а данные устарели — правка через мост окно не уведомляет (события шлёт UI, а не IPC). Приводить уже открытый экран в чистое состояние (очистить поиск, нажать «Обновить») вместо того, чтобы рассчитывать на свежий переход из меню.
- **Фикстуру за собой убирать.** Стенд переживает прогоны, и смоуковые шаблоны копятся, попадая в следующие ассерты как «лишние карточки».

## CDP-драйв: грабли (выучено 2026-09-15, смоук счётчиков списков)

- **Строки таблицы считать без `td[colspan]`.** Пустое состояние «Ничего не найдено» и спейсеры `VirtualTable` — одна ячейка с colspan; на пустом списке контрактов драйвер принял её за первую строку данных.
- **У сгруппированной номенклатуры таблицы нет, пока группа свёрнута** — ждать `[data-list-count]` (он над списком всегда), а для проверки строк сначала раскрыть первую `section > button`.
- **Вкладки живут (keep-alive): `[data-list-count]` и `table.list-table` есть у всех открытых экранов.** Искать только в активной панели `.v3-tab-pane[data-pane-active="1"]`, иначе счётчик «Двигателей» прочитается на экране «Контрактов».
- **Подсказка `SearchSelect`/`EntityReferenceField` — `div[data-idx]` с `onClick`**, не `[role=option]`; выбирать её `opt.click()` после набора номера в поле (плейсхолдер поля — зацепка). Иначе форма отвечает «Выберите двигатель», хотя список показал совпадение.
- **Стенд после новой миграции надо мигрировать вручную:** бэкенд стенда `db:migrate` при старте не делает; без этого REST новой таблицы отвечает `relation does not exist`. Команда — `env $(grep -v '^#' backend-api/.env.dev | tr -d '
' | grep '=' | xargs) corepack pnpm -F @matricarmz/backend-api db:migrate`.
- **Субагент, поднимающий стенд, обязан запускать процессы откреплённо** — прямой `pnpm dev`/`start-*.ps1` в его Bash висит до таймаута, и агент отваливается «no progress for 600s».

## CDP-драйв: грабли (выучено 2026-09-14, смоук номера двигателя в акте)

- **Деталь на стенде больше не завести через `/parts`** — API выведено из эксплуатации (HTTP 410). Позиция номенклатуры это ЗЕРКАЛО карточки-источника, поэтому цепочка такая: `admin.entities.create(<id типа part>)` → `setAttr('name')` → **`sync.run()`** → `warehouse.nomenclatureUpsert({ name, code, directoryKind:'part', directoryRefId:<id сущности>, groupId, unitId, specJson:{templateId} })`. Пропуск `sync.run()` даёт «Источник part:… не найден или удален»: карточка ещё лежит в реплике. Сервер отказывает по одному полю за раз (артикул → directoryKind → directoryRefId → группа → шаблон), так что собирать вызов лучше сразу целиком. Шаблон обязан быть с `itemTypeCode === 'part'`. Образец — `cdp-engine-act-number.mjs`.
- **Поле карточки двигателя — не пара `label`+`input`.** Шапка карточки строится из `mainFieldItems` (подпись и значение — соседи в строке), поэтому поиск по `<label>` ничего не находит. Брать элемент с ТОЧНОЙ подписью и подниматься по предкам до первого `input`.
- **Строки марки досыпаются в лист не мгновенно.** После открытия карточки brand-resync подтягивает новые детали марки асинхронно и сохраняет лист; ассерт по строкам делать после `waitFor` на появление нужной строки, а не после фиксированной паузы.
- **Вложенные шаблонные строки в драйвере — источник `SyntaxError: missing ) after argument list`.** Выражение, которое само собирается из шаблона и потом подставляется в другой шаблон, рвётся на экранировании молча. Длинные выражения собирать конкатенацией массива строк.

## CDP-драйв: грабли (выучено 2026-09-17, смоук кнопок «Провести …»)

- **Фикстуру для панели ставить ДО открытия карточки.** `RepairChecklistPanel` читает лист один раз при монтировании и дальше живёт своим `answers`; строка, положенная мостом при открытой карточке, до неё не доедет, и кнопка законно ответит «в акте комплектности нет строк». Первый прогон на этом и дал 8 FAIL при исправном коде. Порядок: выбрать двигатель → завести фикстуру мостом → открыть карточку.
- **Предусловие прогона — шаг, а не `warn`.** Драйвер видел «строк в листе 0», печатал предупреждение и шёл дальше — восемь красных шагов вместо одного честного «нечего проверять». Если прогон без условия бессмыслен, ассертить условие (`step`), а не упоминать его.
- **Строки марки приезжают не всем.** Заметка «на стенде строки приезжают ресинком марки» верна только для марок с деталями; у марки без деталей лист остаётся пустым. Считать наличие строк свойством стенда нельзя — только заводить самим и убирать за собой (метка в `part_name`, уборка фильтрует по ней, чужие строки не трогает).

## CDP-драйв: грабли (выучено 2026-09-17, смоук рейтинга пикеров)

- **Порядок списка на стенде не видно на двух сотрудниках.** Справочник стенда мал; смоук про СОРТИРОВКУ обязан сам завести дюжину помеченных карточек и снять их в уборке. Имена — синтетические: репозиторий публичный, настоящих ФИО в отслеживаемых файлах быть не может, и в вывод печатать нужно НОМЕРА позиций в алфавите, а не подписи.
- **Проверяй тот путь, которым ходит оператор, — мышь.** Смоук поймал то, чего не увидели ни типы, ни линт, ни сторож: у строки выпадающего списка был свой обработчик мимо общей воронки выбора, и счётчик рос с клавиатуры, но молчал на клике. Если у действия два пути (мышь и клавиатура), смоук обязан пройти обоими — и сначала мышью.
- **Эффект успевает отработать раньше, чем данные приедут.** Шаблон листа уже есть, `answers` ещё пусты — и «одноразовая» автоподстановка тратит свой раз на пустышку. Живой прогон это видит, юниты — нет (GOTCHAS M139).
- **Состояние в `localStorage` — тоже фикстура.** Прежнее значение ключа сохранить в начале, вернуть в конце И перезагрузить рендерер: модульный синглтон хука держит копию в памяти окна и без перезагрузки продолжит жить со своей.

## Файлы skill'а

```
.claude/skills/verifier-electron/
...
└── scripts/
    ...
    ├── cdp-owner-batch-0912.mjs   (смоук: модалка неверного пароля, ступени сотрудников, кэш превью)
    ├── cdp-report-templates-showcase.mjs (смоук: витрина заготовок — три источника, описание, открытие с настройками)
    ├── cdp-bom-card-simplified.mjs  (смоук: упрощённая карточка BOM — таблица, разделы по типу, запасной вариант, норма → сервер; сам засевает фикстуру)
    ├── cdp-bom-brand-group-bind.mjs (смоук: «+ марки группы…» в BOM и блок BOM в карточке группы; заводит вторую марку и группу, убирает за собой)
    ├── cdp-engine-act-number.mjs   (смоук: номер двигателя в шапке акта и в «№ на детали» обоих картеров; сам заводит детали, убирает за собой)
    ├── cdp-nomenclature-parent-position.mjs (смоук: обобщённая позиция — правила сервера, карточки родителя/варианта, «Свернуть варианты», «Свод по обобщённым»; сам заводит тип/группу/шаблонные строки, убирает за собой)
    ├── cdp-list-counts.mjs        (смоук: «Всего · Показано» и колонка «№» на Двигателях, Контрактах, Номенклатуре; сужение поиском)
    ├── cdp-work-sheets.mjs        (смоук: «Этапы работ» — вкладки узлов, строка обкатки диалогом → «Отремонтирован» датой строки, правка без дублей; убирает за собой; стенду нужна миграция 0097)
