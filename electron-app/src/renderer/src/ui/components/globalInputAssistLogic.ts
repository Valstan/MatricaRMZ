import { rankLookupOptions } from '@matricarmz/shared';
...
export function buildAssistPopupItems(args: {
  ...
  if (slotsLeft > 0 && dbOptions?.length) {
    const available = dbOptions.filter((option) => !items.some((item) => item.value === option));
    // Shared tiered matcher (#035): exact/prefix/compact-substring/token-AND +
    // RU<->EN layout, ranked by relevance — a strict upgrade over naive includes
    // for as-you-type suggestions. Empty query keeps the original order (preview).
    const candidates = value ? rankLookupOptions(available.map((o) => ({ id: o, label: o })), value).map((o) => o.id) : available;
