...
export function TwoColumnList<T>(props: {
  ...
  useLayoutEffect(() => {
    const recalc = () => {
      ...
      // Never render more columns than there are items: with 0/1 items a wide
      // window would otherwise split into multiple EMPTY tables (e.g. two
      // "Изменений нет" panes on the journal). Bound columns by item count so
      // empty/low-data screens collapse to a single table with its empty-state.
      const itemBound = Math.max(1, props.items.length);
      const columns = clampInt(Math.min(fitColumns, itemBound), 1, maxColumns);
