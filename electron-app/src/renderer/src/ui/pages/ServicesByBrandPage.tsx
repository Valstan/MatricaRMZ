      ...
      <div style={{ display: 'grid', gap: 8, gridTemplateColumns: 'minmax(280px, 360px) minmax(220px, 1fr) auto auto auto', alignItems: 'center' }}>
        ...
        <label style={{ display: 'grid', gap: 4 }}>
          ...
          >
            ...
            <option value="bound">Только привязанные к марке</option>
            <option value="universal">Универсальные (без марок)</option>
            <option value="other">Привязанные к другим маркам</option>
          </select>
        </label>
        {props.canEdit ? (
          ...
          <table className="list-table">
            <thead>
              <tr>
                ...
                <th style={{ textAlign: 'left' }}>Услуга</th>
                <th style={{ textAlign: 'left' }}>Ед.</th>
                <th style={{ textAlign: 'right' }}>Цена</th>
                <th style={{ textAlign: 'left' }}>Привязка к маркам</th>
                <th />
              </tr>
            </thead>
            <tbody>
                ...
                filteredServices.map((s) => {
                  ...
                  return (
                    <tr key={s.id} style={s.dirty ? { background: 'rgba(245, 158, 11, 0.08)' } : undefined}>
                      ...
                      <td style={{ verticalAlign: 'top' }}>{s.name}</td>
