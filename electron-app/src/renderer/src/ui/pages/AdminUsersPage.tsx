...
import { permAdminOnly, permGroupRu, permTitleRu, systemRoleTitleRu, SYSTEM_ROLE_CATALOG } from '@matricarmz/shared';
...
export function AdminUsersPage(props: { canManageUsers: boolean; me?: { id: string; role: string; username: string } | null }) {
  ...
  async function loadUserRequests() {
    const r = await window.matrica.admin.users.changeRequestsList();
    setUserRequests(r?.ok && Array.isArray(r.requests) ? r.requests : []);
  }

  function describeUserRequest(req: any): string {
    const data = req?.payload?.data ?? {};
    const parts: string[] = [];
    if (data.login !== undefined) parts.push(`логин «${String(data.login)}»`);
    if (data.fullName !== undefined && data.fullName) parts.push(`ФИО «${String(data.fullName)}»`);
    if (data.role !== undefined) parts.push(`роль «${systemRoleTitleRu(String(data.role))}»`);
    if (data.accessEnabled !== undefined) parts.push(data.accessEnabled ? 'доступ включён' : 'доступ выключен');
    if (data.passwordHash !== undefined) parts.push('новый пароль');
    const kind = req?.payload?.kind === 'create' ? 'Создание' : 'Изменение';
    return `${kind}: ${parts.join(', ') || '—'}`;
  }

  function userMutationStatus(r: any, okMsg: string): string {
    if (!r?.ok) return `Ошибка: ${r?.error ?? 'unknown'}`;
    if (r.mode === 'requested') return String(r.message ?? 'Изменения отправлены на одобрение.');
    return okMsg;
  }
  ...
  const canApprovePending = meRole === 'superadmin';
    ...
    <div>
      ...
      {canManageUsers && (
        <div style={{ border: '1px solid #e5e7eb', borderRadius: 12, padding: 12 }}>
          ...
          {meRole === 'superadmin' && userRequests.length > 0 && (
            <div style={{ marginTop: 12, border: '1px solid #fde68a', background: '#fffbeb', borderRadius: 12, padding: 12 }}>
              <strong>Заявки на изменение пользователей ({userRequests.length})</strong>
              <div style={{ marginTop: 10, display: 'grid', gap: 10 }}>
                {userRequests.map((req) => (
                  <div
                    key={req.id}
                    style={{ border: '1px solid #fcd34d', borderRadius: 10, padding: 10, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}
                  >
                    <div style={{ flex: 1, minWidth: 200 }}>
                      <div style={{ fontSize: 14, color: '#111827' }}>{describeUserRequest(req)}</div>
                      <div style={{ fontSize: 12, color: '#92400e' }}>от {String(req.changeAuthorUsername ?? '')}</div>
                    </div>
                    <Button
                      onClick={async () => {
                        setStatus('Применение заявки...');
                        const r = await window.matrica.admin.users.changeRequestsDecide({ id: req.id, action: 'approve' });
                        setStatus(r?.ok ? 'Заявка применена' : `Ошибка: ${r?.error ?? 'unknown'}`);
                        await refreshUsers();
                      }}
                    >
                      Одобрить
                    ...
                    <Button
                      variant="ghost"
                      onClick={async () => {
                        setStatus('Отклонение заявки...');
                        const r = await window.matrica.admin.users.changeRequestsDecide({ id: req.id, action: 'reject' });
                        setStatus(r?.ok ? 'Заявка отклонена' : `Ошибка: ${r?.error ?? 'unknown'}`);
                        await refreshUsers();
                      }}
                    >
                      Отклонить
            ...
            <div style={{ border: '1px solid #f3f4f6', borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 8 }}>
                ...
                <select
                  ...
                  style={{ padding: '8px 10px', borderRadius: 10, border: '1px solid #d1d5db' }}
                >
                  {SYSTEM_ROLE_CATALOG.filter((r) => r.key !== 'pending' && r.key !== 'superadmin').map((r) => (
                    <option
                      key={r.key}
                      value={r.key}
                      disabled={(r.key === 'employee' && !canCreateEmployee) || (r.key === 'admin' && !canCreateAdmin)}
                    >
                      {r.titleRu}
                    </option>
                  ))}
                ...
                <Button
                  onClick={async () => {
                    ...
                    setStatus(userMutationStatus(r, 'Пользователь создан'));
                    if (r.ok) {
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 140px', gap: 10, alignItems: 'center' }}>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        const r = await window.matrica.admin.users.update(selectedUserId, { login: next });
                        setStatus(userMutationStatus(r, 'Логин обновлён'));
                        await refreshUsers();
                    ...
                    <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: '#111827', fontSize: 14 }}>
                      ...
                      <select
                        ...
                        onChange={async (e) => {
                          ...
                          setStatus(userMutationStatus(r, 'Роль обновлена'));
                          await refreshUsers();
                          await openUser(selectedUserId);
                        }}
                        ...
                        style={{ padding: '8px 10px', borderRadius: 10, border: '1px solid #d1d5db' }}
                      >
                        {SYSTEM_ROLE_CATALOG.filter((r) => r.key !== 'pending').map((r) => (
                          <option
                            key={r.key}
                            value={r.key}
                            disabled={
                              (r.key === 'employee' && !canCreateEmployee) ||
                              (r.key === 'admin' && !canCreateAdmin) ||
                              r.key === 'superadmin'
                            }
                          >
                            {r.titleRu}
                    ...
                    <label style={{ display: 'flex', gap: 8, alignItems: 'center', color: '#111827', fontSize: 14 }}>
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          setStatus(userMutationStatus(r, 'Активность обновлена'));
                          await refreshUsers();
                        }}
                        disabled={!canEditRoleOrAccess}
                      />
                            ...
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  ...
                                  const r = await window.matrica.admin.users.deleteCancel(selectedUserId);
              ...
              {!userPerms ? (
                ...
                <>
                  <div style={{ marginTop: 10, display: 'flex', gap: 10, alignItems: 'center' }}>
                    ...
                    <div style={{ color: '#6b7280', fontSize: 12, whiteSpace: 'nowrap', display: 'flex', gap: 8, alignItems: 'center' }}>
                      <span>
                        Пользователь:{' '}
                    ...
                    <table className="list-table">
                      <thead>
                        <tr style={{ background: 'linear-gradient(135deg, #0891b2 0%, #0e7490 120%)', color: '#fff' }}>
                          <th data-col-kind="name" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 10 }}>perm</th>
                          <th data-col-kind="name" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 10 }}>кто → кому</th>
                          <th style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 10 }}>срок</th>
                          <th data-col-kind="flag" title="статус" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 10 }}>статус</th>
                      ...
                      <tbody>
                        {delegations
                          ...
                          .map((d) => {
                            ...
                            return (
                              <tr key={d.id}>
                                <td data-col-kind="name" style={{ borderBottom: '1px solid #f3f4f6', padding: 10, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: 12 }}>
                                  {d.permCode}
                                </td>
                                <td data-col-kind="name" style={{ borderBottom: '1px solid #f3f4f6', padding: 10 }}>{fromU} → {toU}</td>
                                <td style={{ borderBottom: '1px solid #f3f4f6', padding: 10 }}>
                                  {formatMoscowDate(d.startsAt)} — {formatMoscowDate(d.endsAt)}
                                </td>
                                <td data-col-kind="flag" style={{ borderBottom: '1px solid #f3f4f6', padding: 10 }}>{state}</td>
