...
export function ServicesPage(props: {
  ...
  const contextColumns = useMemo(
    () => [
      ...
      { title: 'Цена', value: (row: Row) => formatPrice(row.price) },
    ],
    [],
  );
  ...
  function printRows(items: Row[]) {
    printRowsPreview({
      title: items.length > 1 ? `Выделенные услуги (${items.length})` : `Услуга: ${items[0]?.displayName || '(без названия)'}`,
      sectionTitle: 'Список услуг',
      rows: items,
  ...
  async function deleteRows(ids: string[]) {
    ...
    const message = buildDeleteConfirmMessage({
      ...
      selectedManyLabel: 'выделенные услуги',
      singleLabel: 'услугу',
    });
