...
export function MasterdataPage(props: {
  ...
  function normalizeLookupBaseCode(baseCodeRaw: string): string {
    ...
    // 'workshop' намеренно не редиректится на legacy workshop_ref — канон цеха в
    // directory_workshops, link-поля резолвятся в refreshLinkOptions.
    return baseCode;
  }
  ...
  async function loadBrandParts(brandId: string) {
    ...
    const r = await listAllPartSpecs({ engineBrandId: brandId }).catch(() => ({ ok: false as const, error: 'unknown' }));
    if (!r.ok) {
  ...
  async function updateBrandParts(nextIds: string[]) {
    ...
    for (const partId of toAdd) {
      ...
      const up = await upsertPartSpecBrandLink({
        partId,
    ...
    for (const partId of toRemove) {
      ...
      const del = await deletePartSpecBrandLink({ partId, linkId: String(current.id) });
      if (!del.ok) {
  ...
  async function refreshLinkOptions(defsForType: AttrDefRow[]) {
    ...
    // Канон «цеха» — таблица directory_workshops, а не EAV entity-type. Легаси-коды
    // 'workshop'/'workshop_ref' резолвим на канон (см. docs/PROJECT_STATE.md).
    let workshopCanon: { id: string; label: string }[] | null = null;
    for (const d of defsForType) {
      if (d.dataType !== 'link') continue;
      const targetCode = getLinkTargetTypeCode(d);
      if (!targetCode) continue;
      if (targetCode === 'workshop' || targetCode === 'workshop_ref') {
        if (!workshopCanon) {
          try {
            const wr = await window.matrica.workshops.list({ activeOnly: true });
            workshopCanon = wr?.ok ? wr.rows.map((w) => ({ id: String(w.id), label: String(w.name) })) : [];
          } catch {
            workshopCanon = [];
          }
        }
        map[d.code] = workshopCanon;
        continue;
      }
                        ...
                        <tbody>
                          {deleteDialog.links.slice(0, 10).map((l, idx) => (
                            <tr key={`${l.fromEntityId}:${l.attributeDefId}:${idx}`}>
                              ...
                              <td data-col-kind="name" style={{ borderBottom: '1px solid #f3f4f6', padding: 10 }}>{l.attributeName}</td>
                    ...
                    <table className="list-table list-table--catalog">
                      <thead>
                        <tr style={{ background: 'linear-gradient(135deg, #db2777 0%, #9d174d 120%)', color: '#fff' }}>
                          ...
                          <th data-col-kind="flag" title="Обяз." style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 10 }}>Обяз.</th>
                      ...
                      <tbody>
                        {visibleDefs.map((d) => (
                          <tr key={d.id}>
                            ...
                            <td data-col-kind="flag" style={{ borderBottom: '1px solid #f3f4f6', padding: 10 }}>{d.isRequired ? 'да' : 'нет'}</td>
                            {canUseDangerActions && (
