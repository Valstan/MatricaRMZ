...
export function ScreenEditorPage(props: {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- deps left byte-for-byte as on main (this branch is behaviour-neutral). The rule asks for targetIds/removeLink/removeBlocks/duplicateBlocks/nudgeBlocks — plain function declarations recreated every render, so listing them would detach/re-attach the window listener on every render (undo/redo are NOT flagged: the rule already treats them as stable because their bodies touch only setSpec and historyRef). Most of them survive a stale closure: removeLink/removeBlocks/nudgeBlocks mutate only through setSpec/mutateSpec functional updaters, and targetIds reads just selection/multiIds, both of which are deps. KNOWN GAP (pre-existing on main, NOT fixed here; tracked in docs/PENDING_FOLLOWUPS.md «Ctrl+D в конструкторе экранов дублирует по старым координатам»): duplicateBlocks reads spec.blocks straight from the render closure while the deps track only spec.blocks.length, so a drag (geometry changes, count does not) leaves the listener subscribed with the pre-drag spec — Ctrl+D then creates the copy at the pre-drag x+24/y+24, and the UI_SPEC_MAX_BLOCKS guard counts that same stale array. Depending on spec.blocks instead of spec.blocks.length would fix it, but that is a behaviour change and belongs to the follow-up PR. snapToGrid is a superfluous pre-existing dep, kept only to preserve main's re-subscribe timing: nothing reachable from this handler reads it (nudgeBlocks applies raw dx/dy; snapping lives in patchBlockGeometry/handleBlockMove, which the keyboard path never calls)
