...
export function ScreenEditorPage(props: {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- the rule asks for targetIds/removeLink/removeBlocks/duplicateBlocks/nudgeBlocks — plain function declarations recreated every render, so listing them would detach/re-attach the window listener on every render (undo/redo are NOT flagged: the rule already treats them as stable because their bodies touch only setSpec and historyRef). They survive a stale closure: removeLink/removeBlocks/nudgeBlocks mutate only through setSpec/mutateSpec functional updaters, targetIds reads just selection/multiIds, and duplicateBlocks reads spec.blocks — all of which are deps. spec.blocks (not .length) is required: duplicateBlocks copies at b.x + 24 / b.y + 24, so a drag must re-subscribe the listener or Ctrl+D would duplicate at pre-drag coordinates. snapToGrid is a superfluous pre-existing dep: nothing reachable from this handler reads it (nudgeBlocks applies raw dx/dy; snapping lives in patchBlockGeometry/handleBlockMove, which the keyboard path never calls)
  }, [selection, multiIds, spec.blocks, snapToGrid]);
