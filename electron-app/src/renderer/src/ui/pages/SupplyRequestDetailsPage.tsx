...
export function SupplyRequestDetailsPage(props: {
   ...
   * row does not exist yet; the first save materializes it (number assigned then). */
  initialPayload?: SupplyRequestPayload;
  canEdit: boolean;
  canSign: boolean;
  canApprove: boolean;
  canAccept: boolean;
  ...
  async function loadLinkLists() {
    ...
    for (const it of payloadRef.current?.items ?? []) {
      ...
      // Phase 3 Stage F: part fields come from directory_parts (part id == nomenclature id),
      // not parts.get — name from the part-spec (every part has a directory row), unit from the
      // nomenclature row (same source the refKind='nomenclature' branch below already uses).
      const sp = await window.matrica.warehouse.nomenclaturePartSpecGet({ nomenclatureId: pid });
      if (!sp?.ok || !sp.name) continue;
      knownIds.add(pid);
      const partName = String(sp.name ?? '').trim() || `Деталь ${pid.slice(0, 8)}…`;
      const nl = await window.matrica.warehouse.nomenclatureList({ id: pid, limit: 1 }).catch(() => null);
      const unit = nl?.ok && nl.rows?.[0]
        ? String((nl.rows[0] as WarehouseNomenclatureListItem).unitName ?? '').trim()
        : '';
      items.push({
        ...buildSearchOption({
          id: pid,
  ...
  async function enrichUnitIfMissing(optionId: string) {
    ...
    try {
      ...
      if (opt.refKind === 'nomenclature' || opt.refKind === 'part') {
        // Phase 3 Stage F: parts resolve unit from the nomenclature row (part id ==
        // nomenclature id), the same path as nomenclature — no parts.get.
  ...
  async function saveNow(next: SupplyRequestPayload) {
    ...
    try {
      ...
      if (r.requestNumber && next.requestNumber !== r.requestNumber) {
        const materialized = { ...next, requestNumber: r.requestNumber };
        setPayload(materialized);
        payloadRef.current = materialized;
        lastSavedJson.current = JSON.stringify(materialized);
      } else {
    ...
    [
      {
        ...
        defaultOrder: 10,
        label: 'Номер закупки',
        value: payload.requestNumber || 'новая',
        render: <div style={{ fontWeight: 700 }}>{payload.requestNumber?.trim() || 'новая'}</div>,
      },
      {
        code: 'compiled_at',
        defaultOrder: 20,
          ...
          <table className="list-table list-table--single-mode work-order-table supply-request-items-table">
            ...
            <thead>
              <tr>
                ...
                <th style={rightHeaderStyle} data-col-kind="num" title="Пришло">Пришло</th>
                <th style={rightHeaderStyle} data-col-kind="num" title="Недовезли">Недовезли</th>
                ...
                return (
                  <React.Fragment key={itemRowKeysRef.current[idx] ?? `supply-request-item-${idx}`}>
                    ...
                    >
                      ...
                      <td data-col-kind="num" style={rightCellStyle}>{remaining}</td>
                      <td>
