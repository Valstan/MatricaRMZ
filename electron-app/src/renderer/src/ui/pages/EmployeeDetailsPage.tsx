...
export function EmployeeDetailsPage(props: {
  ...
  const [workshops, setWorkshops] = useState<Option[]>([]);
  const [workshopsStatus, setWorkshopsStatus] = useState('');
  ...
  useEffect(() => {
    ...
    const desired = [
      ...
      // Цех — справочник directory_workshops (не EAV-entity), поэтому text-атрибут с UUID цеха; UI резолвит имя по workshops.list().
      { code: 'workshop_id', name: 'Цех', dataType: 'text', sortOrder: 105 },
    ];
  ...
  async function loadWorkshops() {
    try {
      setWorkshopsStatus('Загрузка списка цехов…');
      const r = await window.matrica.workshops.list();
      const rows = r && (r as any).ok && Array.isArray((r as any).rows) ? (r as any).rows : [];
      const opts = (rows as any[]).map((w) => ({ id: String(w.id), label: String(w.name ?? w.id) }));
      opts.sort((a, b) => a.label.localeCompare(b.label, 'ru'));
      setWorkshops(opts);
      setWorkshopsStatus('');
    } catch (e) {
      setWorkshopsStatus(`Ошибка: ${String(e)}`);
    }
  }
  ...
  /**
   * Должность хранится свободным текстом (атрибут `role`), а выпадающий список — из
   * справочника `position_ref`. Чтобы они не расходились, при сохранении карточки
   * заводим должность в справочник, если её там ещё нет (кейс-нечувствительно, ё→е).
   */
  async function ensurePositionInDirectory(label: string): Promise<void> {
    if (!props.canEdit) return;
    const clean = label.trim();
    if (!clean) return;
    const key = clean.toLowerCase().replace(/ё/g, 'е');
    const exists = positionOptions.some((o) => o.label.trim().toLowerCase().replace(/ё/g, 'е') === key);
    if (exists) return;
  ...
  useEffect(() => {
    ...
    const vWorkshop = attrs.workshop_id;
    ...
    setWorkshopId(typeof vWorkshop === 'string' && vWorkshop.trim() ? vWorkshop : null);
  ...
  useEffect(() => {
    ...
    props.registerCardCloseActions({
      ...
      copyToNew: async () => {
        ...
        if (workshopId) await window.matrica.employees.setAttr(r.id, 'workshop_id', workshopId);
        dirtyRef.current = false;
      },
    });
    return () => {
  ...
  const workshopLabel = workshopOptions.find((w) => w.id === workshopId)?.label ?? null;
      ...
      },
      {
        ...
        render: (
          <SearchSelectWithCreate
            ...
            disabled={!props.canEdit}
            placeholder={workshopsStatus || 'Выберите цех'}
            canCreate={false}
