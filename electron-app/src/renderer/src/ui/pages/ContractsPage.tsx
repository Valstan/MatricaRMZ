...
export function ContractsPage(props: {
      ...
      const details = await Promise.all(
        (listRaw as any[]).map(async (row: any) => {
          try {
            ...
            return {
              id: String(row.id),
              number: row.displayName ? String(row.displayName) : '(без номера)',
              internalNumber: '',
              counterparty: '—',
              searchText: row.searchText ? String(row.searchText) : '',
              dueDateMs: null,
  ...
  async function deleteRows(ids: string[]) {
    ...
    for (const id of ids) {
      ...
      if (!r.ok) {
        failed += 1;
        if (!firstError && r.error) firstError = String(r.error);
      }
    }
    setStatus(
      failed > 0 && firstError
        ...
        {props.canCreate && (
          <Button
            onClick={async () => {
              ...
              try {
                // Deferred-create: open an empty card on a client id; the row materializes on the
                // first save (ContractDetailsPage passes fallbackTypeId). Opened and abandoned →
                // no empty contract ghost synced to everyone.
