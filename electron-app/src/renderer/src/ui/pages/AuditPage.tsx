...
export function AuditPage(props: { audit: AuditItem[]; onRefresh: () => Promise<void> }) {
  ...
  const tableHeader = (
    <thead>
      <tr style={{ background: 'linear-gradient(135deg, #0891b2 0%, #0e7490 120%)', color: '#fff' }}>
        <th data-col-kind="date" title="Время" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 8, cursor: 'pointer' }} onClick={() => onSort('createdAt')}>
          Время {sortArrow(listState.sortKey as SortKey, listState.sortDir, 'createdAt')}
        </th>
        <th data-col-kind="name" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 8, cursor: 'pointer' }} onClick={() => onSort('actor')}>
          Логин {sortArrow(listState.sortKey as SortKey, listState.sortDir, 'actor')}
        </th>
        <th data-col-kind="text" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 8, cursor: 'pointer' }} onClick={() => onSort('action')}>
          Действие {sortArrow(listState.sortKey as SortKey, listState.sortDir, 'action')}
        </th>
        <th data-col-kind="name" style={{ textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.25)', padding: 8, cursor: 'pointer' }} onClick={() => onSort('section')}>
  ...
  function renderAuditCells(a: AuditItem) {
    return (
      <>
        <td data-col-kind="date" style={{ borderBottom: '1px solid #f3f4f6', padding: 8 }}>{formatMoscowDateTime(a.createdAt)}</td>
        <td data-col-kind="name" style={{ borderBottom: '1px solid #f3f4f6', padding: 8 }}>{a.actor}</td>
        <td data-col-kind="text" style={{ borderBottom: '1px solid #f3f4f6', padding: 8 }}>
          {actionRu(a)}
        </td>
        <td data-col-kind="name" style={{ borderBottom: '1px solid #f3f4f6', padding: 8 }}>{contextRu(a)}</td>
      </>
    );
  }
    ...
    return (
      <div style={{ border: '1px solid #e5e7eb', overflow: 'clip' }}>
        <table className="list-table">
          ...
          <tbody>
            {items.map((a) => (
              <tr key={a.id}>{renderAuditCells(a)}</tr>
            ))}
            {items.length === 0 && (
              <tr>
      ...
      <div ref={containerRef} onScroll={onScroll} style={{ marginTop: 8, flex: '1 1 auto', minHeight: 0, overflow: 'auto' }}>
        {twoCol ? (
          ...
          <VirtualTable
            ...
            count={sorted.length}
            header={tableHeader}
            renderCells={(i) => renderAuditCells(sorted[i]!)}
