 ...
 *
 * Это ровно те два узла, на которых стоит `overflow: auto` в `shellV3.css`, — панель
 * списочной вкладки и тело карточки. Список раньше содержал `v3-split-list` (снесён
 * вместе со сплит-раскладкой) и `mx-data-scroll` (пометка, которую так никто и не
 * проставил), то есть живым из трёх был только `v3-card-body`: прокрутка ЛЮБОГО
 * списка не считалась данными и хром не убирала.
 *
 * `v3-tab-content-menu` сюда НЕ входит и в нём нет опечатки: токены сравниваются
 * целиком (`classList` даёт отдельные значения), поэтому `v3-tab-content` его не
 * зацепит — и правильно, вкладка МЕНЮ это хром, а не данные.
 */
export const DATA_SCROLL_CLASSES: readonly string[] = ['v3-tab-content', 'v3-card-body'];
