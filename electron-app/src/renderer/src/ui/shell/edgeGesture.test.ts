...
describe('решение на отпускании', () => {
  ...
  it('флик считается и когда pointerup повторяет координаты последнего движения', () => {
    // Ровно то, что шлёт браузер: up приходит позже, но в той же точке. Если считать
    // скорость по этой паре, она нулевая — быстрый короткий свайп молча откатывался бы.
    const g = leftDrawer();
    g.start({ x: 0, y: 0, t: 0 });
    g.move({ x: 20, y: 1, t: 10 });
    g.move({ x: 70, y: 2, t: 30 });
    const r = g.end({ x: 70, y: 2, t: 95 });
    expect(r.open).toBe(true);
  });
