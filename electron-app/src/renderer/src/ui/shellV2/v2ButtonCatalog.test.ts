...
describe('buildV2Buttons — планшетное операторское меню', () => {
  ...
  it('в режиме «Планшет» остаются только разделы пресета', () => {
    ...
    const navIds = allItems(b).filter((id) => !['theme','tablet_mode','decor','palette','search','basket','column_mode','sync','chat_link','notes_link','desktop_shortcut','program_feedback','chat','ai_chat','user'].includes(id));
