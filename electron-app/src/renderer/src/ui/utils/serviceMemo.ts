...
export function buildServiceMemoSections(args: {
  ...
  const intro = `
    ...
    по указанным двигателям получен авансовый платёж, а срок ремонта по контракту, который идёт
    с даты поступления двигателя на завод, подходит к завершению.</p>`;
  const rows = engines
    .map(
      (e, i) => `<tr>
