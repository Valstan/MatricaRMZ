...
/**
 * Блок подписей payroll-отчёта. ФИО предзаполняются из `report.payrollSignatures`
 * (бригада + начальник цеха / нормировщик / начальник ОК); пустые — оператор подписывает ручкой.
 */
export function payrollSignaturesBlockHtml(report: PreviewOk): string {
  const sig = report.payrollSignatures ?? null;
  const row = (label: string, name: string) =>
    `  <div class="payroll-sig-row"><span>${escapeHtml(label)}</span><span class="payroll-sig-line">${
      name ? `<span class="payroll-sig-name">${escapeHtml(name)}</span>` : ''
    }</span></div>`;
  return [
    '<div class="payroll-sig">',
    row('Подпись сотрудника участника бригады', sig?.crewMember ?? ''),
    row('Подпись начальника цеха', sig?.workshopHead ?? ''),
    row('Подпись Специалист по нормированию', sig?.normingSpecialist ?? ''),
    row('Подпись начальник отдела кадров', sig?.hrHead ?? ''),
    '</div>',
  ].join('\n');
}
...
.payroll-sig-line{flex:1;border-bottom:1px solid #0f172a;min-height:18px;display:flex;align-items:flex-end;justify-content:flex-end}
.payroll-sig-name{font-size:12px;color:#475569;padding:0 6px 1px}
...
export function renderWorkOrderPayrollFullHtml(report: PreviewOk): string {
  ...
  // Подписи перенесены сюда из карточки наряда (v1.18.3): бригада / начальник цеха /
  // нормировщик / отдел кадров. ФИО предзаполняются из report.payrollSignatures.
  const signatures = payrollSignaturesBlockHtml(report);
