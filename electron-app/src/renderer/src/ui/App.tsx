...
export function App() {
  ...
  function closeV2Card(card: { kind: TabId; entityId: string }) {
    ...
    const doAfter = () => {
      setV2CardEpoch((e) => e + 1);
      dispatchTabs({ type: 'CLOSE', id: closingId });
      // Сосед слева у карточки — другая карточка, список (карточка встаёт справа от списка,
      // из которого открыта) или служебная вкладка. В последнем случае (закрыли САМУЮ ЛЕВУЮ
      // карточку) остаёмся среди карточек, и лишь когда их не осталось — возвращаемся на
      // родительский список, а не бросаем оператора в чат.
      if (nextFocus?.kind === 'card') {
        reopenV2Card(nextFocus.cardKind as TabId, nextFocus.entityId);
        return;
      }
      if (nextFocus?.kind === 'list') {
        setTabState(nextFocus.tabId as TabId);
        return;
      }
