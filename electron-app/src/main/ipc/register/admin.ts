...
import {
  ...
  adminListUserChangeRequests,
  adminDecideUserChangeRequest,
...
export function registerAdminIpc(ctx: IpcContext) {
  ...
  ipcMain.handle('admin:entities:get', async (_e, id: string, fallbackTypeId?: string) => {
    const gate = await requirePermOrResult(ctx, 'masterdata.view');
    if (!gate.ok) throw new Error('permission denied: masterdata.view');
    return getEntityDetails(ctx.dataDb(), id, fallbackTypeId);
  });
  ipcMain.handle('admin:entities:setAttr', async (_e, entityId: string, code: string, value: unknown, fallbackTypeId?: string) => {
    if (isViewMode(ctx)) return viewModeWriteError();
    const gate = await requirePermOrResult(ctx, 'masterdata.edit');
    if (!gate.ok) return gate;
    return setEntityAttribute(ctx.dataDb(), entityId, code, value, fallbackTypeId);
  });
  ipcMain.handle(
  ...
  // RBAC #474 M4: user-change approval queue.
  ipcMain.handle('admin:users:changeRequestsList', async () => {
    await requirePermOrThrow(ctx, 'admin.users.manage');
    return adminListUserChangeRequests(ctx.sysDb, ctx.mgr.getApiBaseUrl());
  });
  ipcMain.handle(
    'admin:users:changeRequestsDecide',
    async (_e, args: { id: string; action: 'approve' | 'reject'; note?: string }) => {
      if (isViewMode(ctx)) return viewModeWriteError() as any;
      await requirePermOrThrow(ctx, 'admin.users.manage');
      return adminDecideUserChangeRequest(ctx.sysDb, ctx.mgr.getApiBaseUrl(), args.id, args.action, args.note);
    },
  );
  ipcMain.handle(
  ...
  ipcMain.handle(
    'admin:users:sectionAccessSetOne',
    async (_e, userId: string, sectionId: string, level: 'viewer' | 'editor' | null) => {
      if (isViewMode(ctx)) return viewModeWriteError() as any;
      await requirePermOrThrow(ctx, 'admin.users.manage');
      return adminSetSectionAccessOne(ctx.sysDb, ctx.mgr.getApiBaseUrl(), userId, sectionId, level);
    },
  );
