...
// Последняя «активность» от renderer (input-based active-time). Едет доп. query-параметром на
// уже идущем 60-сек heartbeat'е — без отдельных запросов. Кумулятив за локальный день клиента.
let reportedActivity: { activeDate: string; activeMs: number } | null = null;
export function setReportedActivity(v: { activeDate?: unknown; activeMs?: unknown } | null): void {
  const activeDate = String(v?.activeDate ?? '').trim();
  const activeMs = Math.trunc(Number(v?.activeMs ?? NaN));
  reportedActivity =
    /^\d{4}-\d{2}-\d{2}$/.test(activeDate) && Number.isFinite(activeMs) && activeMs >= 0
      ? { activeDate, activeMs }
      : null;
}
...
export async function applyRemoteClientSettings(args: {
  ...
  try {
    ...
    // Watchdog-targeted commands (e.g. `reinstall`) are consumed by the external
    // watchdog process, not the app. Leave them in the request slot — never ack,
    // clear, or run a sync for them, or the watchdog (≈15-min cadence) would never
    // observe a command the app (60-sec cadence) raced to consume first.
    const isWatchdogCommand = syncReqType === 'reinstall';
    if (syncReqId && syncReqType && !isWatchdogCommand) {
