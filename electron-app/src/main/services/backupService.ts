...
// Downloaded nightly backups are full plaintext SQLite snapshots of the whole DB,
// cached here only for the duration of a "view backup" session. They are deleted on
// exit and swept on startup so a plaintext copy of the entire database never lingers
// in userData (stolen-laptop threat — security-hardening-2026-06, Phase 2).
...
export async function nightlyBackupDownload(
  ...
  try {
    ...
    const dir = join(userDataDir, BACKUP_CACHE_DIR);
...
// Delete a single downloaded backup snapshot (best-effort). Call after its readonly
// handle is closed (on exit from view mode / before opening a different snapshot).
export async function deleteBackupCacheFile(backupPath: string): Promise<void> {
  try {
    if (!backupPath) return;
    // Safety: only ever delete inside the managed backup_cache dir, regardless of how
    // backupPath was supplied (e.g. a future enterLocal with a user-picked path).
    if (basename(dirname(resolve(backupPath))) !== BACKUP_CACHE_DIR) return;
    await fsp.rm(backupPath, { force: true });
  } catch {
    // best-effort: a transient Windows lock right after close() is non-fatal; the
    // startup sweep will remove anything left behind.
  }
}

// Remove the whole backup-cache dir (best-effort). Call on startup to clear snapshots
// left over from a previous session that exited uncleanly while viewing a backup.
export async function sweepBackupCache(userDataDir: string): Promise<void> {
  try {
    await fsp.rm(join(userDataDir, BACKUP_CACHE_DIR), { recursive: true, force: true });
  } catch {
    // best-effort
  }
}
