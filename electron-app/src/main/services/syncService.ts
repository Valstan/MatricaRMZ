...
function getUpsertChunkSize(rows: Array<Record<string, unknown>>, bindLimit = getSyncSqlLimits().maxBindParams) {
  ...
  return Math.max(1, Math.min(MAX_UPSERT_CHUNK_ROWS, Math.floor(bindLimit / keyCount)));
}
  ...
  for (let i = 0; i < rows.length; ) {
    ...
    try {
      ...
      if (chunkSize > legacyChunkSize && /too many (sql )?variables/i.test(String(e))) {
        logSync(`upsert chunk fallback to legacy bind limit chunkSize=${chunkSize}->${legacyChunkSize}`);
...
/**
 * Progressive fill (client-perf п.3): на холодном snapshot-пулле UI-критичные таблицы
 * (EAV-ядро + ERP-склад) качаются первыми, объёмные не-UI таблицы — в хвосте.
 * Стабильная партиция registry-порядка, поэтому topological-зависимости сохраняются:
 * deps хвостовых таблиц (chat_reads←chat_messages, note_shares←notes) остаются внутри
 * хвоста в прежнем относительном порядке, deps ERP-таблиц (entities, erp_nomenclature)
 * — в головной части.
 */
const FULL_STATE_DEFERRED_TABLES = new Set<string>([
  'audit_log',
  'chat_messages',
  'chat_reads',
  'user_presence',
...
const FULL_STATE_SYNC_TABLES_PRIORITIZED: string[] = [
  ...FULL_STATE_SYNC_TABLES.filter((t) => !FULL_STATE_DEFERRED_TABLES.has(t)),
  ...FULL_STATE_SYNC_TABLES.filter((t) => FULL_STATE_DEFERRED_TABLES.has(t)),
];
...
async function collectPending(db: BetterSQLite3Database) {
  ...
  }
  {
    const pendingDrafts = await db
      .select()
      .from(cardDrafts)
      .where(eq(cardDrafts.syncStatus, pending))
      .limit(limitFor(SyncTableName.CardDrafts));
    const valid: typeof pendingDrafts = [];
    const invalidIds: string[] = [];
    for (const row of pendingDrafts) {
      const syncRow = toSyncRow(SyncTableName.CardDrafts, row);
      const parsed = cardDraftRowSchema.safeParse(syncRow);
      if (parsed.success) {
        valid.push(row);
      } else {
        invalidIds.push(String(row.id));
    ...
    if (invalidIds.length > 0) {
      await markPendingError(db, SyncTableName.CardDrafts, invalidIds);
      logSync(`push drop invalid card_drafts count=${invalidIds.length} ids=${invalidIds.slice(0, 5).join(',')}`);
    }
...
async function applyPulledChanges(
      ...
      case SyncTableName.ErpEngineInventoryLines:
        {
          ...
          groups.erp_engine_inventory_lines.push({
            ...
            hasOwnNumber: optBool(payload.has_own_number),
            hasOwnNumberOverride: optBool(payload.has_own_number_override),
  ...
  if (groups.card_drafts.length > 0) {
    emitApply(SyncTableName.CardDrafts, groups.card_drafts.length);
    await upsertPulledRowsInChunks(db, cardDrafts, groups.card_drafts, cardDrafts.id, {
      ownerUserId: sql`excluded.owner_user_id`,
      ...
      title: sql`excluded.title`,
      payloadJson: sql`excluded.payload_json`,
      baseUpdatedAt: sql`excluded.base_updated_at`,
      updatedAt: sql`excluded.updated_at`,
      lastServerSeq: sql`excluded.last_server_seq`,
      deletedAt: sql`excluded.deleted_at`,
      syncStatus: 'synced',
  ...
  if (groups.erp_engine_inventory_lines.length > 0) {
    ...
    await upsertPulledRowsInChunks(db, erpEngineInventoryLines, groups.erp_engine_inventory_lines, erpEngineInventoryLines.id, {
      ...
      hasOwnNumber: sql`excluded.has_own_number`,
      hasOwnNumberOverride: sql`excluded.has_own_number_override`,
  ...
  if (dedupWarehouseBalances.length > 0) {
    ...
    await upsertPulledRowsInChunks(db, erpRegStockBalance, dedupWarehouseBalances, erpRegStockBalance.id, {
      ...
      warehouseLocationId: sql`excluded.warehouse_location_id`,
  ...
  if (changes.length >= 500) {
    const doneAt = nowMs();
    logSync(
      `apply breakdown rows=${changes.length} totalMs=${doneAt - applyStartedAt} parseMs=${parseDoneAt - applyStartedAt} remapMs=${remapDoneAt - parseDoneAt} dedupLookupMs=${dedupDoneAt - remapDoneAt} upsertMs=${doneAt - dedupDoneAt}`,
    );
  }
}
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      const pullFullState = async () => {
        let totalPulled = 0;
        let serverSeq = 0;
        const tableCount = FULL_STATE_SYNC_TABLES_PRIORITIZED.length;
        for (let tableIdx = 0; tableIdx < tableCount; tableIdx += 1) {
          const table = FULL_STATE_SYNC_TABLES_PRIORITIZED[tableIdx];
          if (!table) continue;
          ...
          logSync(`state ${table} done rows=${tableRows} pages=${tablePages} durMs=${nowMs() - tableStartedAt}`);
          const nextTable = FULL_STATE_SYNC_TABLES_PRIORITIZED[tableIdx + 1];
          if (table && !FULL_STATE_DEFERRED_TABLES.has(table) && (!nextTable || FULL_STATE_DEFERRED_TABLES.has(nextTable))) {
            logSync(`state core tables ready (progressive fill), deferred tail starts`);
            emitSyncProgress('progress', { pulled: totalPulled, coreReady: true, detail: 'основные данные загружены' });
          }
        }
      ...
      // Fresh client (never pulled): state snapshot is strictly cheaper than replaying the
      // whole changelog from seq=0 — it skips superseded row versions (history churn grows
      // unboundedly) and gives per-table progress. Local tables are empty, so the missing
      // delete-ops of the snapshot path cannot leave zombies. Recovery paths below that
      // reset `since` mid-run keep the changelog walk: their local DB may hold stale rows
      // that only changelog delete-ops would remove.
      const coldStart = since === 0;
