...
export async function erpDictionaryList(db: BetterSQLite3Database, apiBaseUrl: string, moduleName: ErpModule) {
  try {
    const r = await httpAuthed(db, apiBaseUrl, `/erp/dictionary/${encodeURIComponent(moduleName)}`, { method: 'GET' });
...
export async function erpDictionaryUpsert(
  ...
  try {
    const r = await httpAuthed(db, apiBaseUrl, `/erp/dictionary/${encodeURIComponent(args.moduleName)}`, {
      method: 'POST',
...
export async function erpCardsList(db: BetterSQLite3Database, apiBaseUrl: string, moduleName: ErpCardModule) {
  try {
    const r = await httpAuthed(db, apiBaseUrl, `/erp/cards/${encodeURIComponent(moduleName)}`, { method: 'GET' });
    if (!r.ok) return { ok: false as const, error: formatHttpError(r) };
...
export async function erpCardsUpsert(
  ...
  try {
    const r = await httpAuthed(db, apiBaseUrl, `/erp/cards/${encodeURIComponent(args.moduleName)}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function erpDocumentsCreate(
  ...
  try {
    const r = await httpAuthed(db, apiBaseUrl, '/erp/documents', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(args),
    });
...
export async function erpDocumentsPost(
  ...
  try {
    const r = await httpAuthed(db, apiBaseUrl, `/erp/documents/${encodeURIComponent(documentId)}/post`, { method: 'POST' });
    if (!r.ok) return { ok: false as const, error: formatHttpError(r) };
    if (!r.json?.ok) return { ok: false as const, error: String(r.json?.error ?? 'unknown') };
