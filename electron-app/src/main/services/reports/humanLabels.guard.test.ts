...
const ID = {
  ...
  /** Детали с таким id нет — висячая ссылка в списке `part_ids` услуги. */
  partMissing: 'c3990000-0000-4000-8000-000000000002',
  partLink: 'd4aa0000-0000-4000-8000-000000000001',
  /** Марки с таким id нет — висячая ссылка в СПИСКЕ марок карточки детали (`engine_brand_ids`). */
  brandMissingInList: 'aa110000-0000-4000-8000-000000000004',
  ...
  /** Цеха с таким id справочник не знает — висячая ссылка в строке этапа работ. */
...
function eavRows(shape: FixtureShape) {
  ...
  const attrs: Record<string, Record<string, unknown>> = {
    ...
    [ID.service]: {
      name: 'Шлифовка коленвала',
      price: 5000,
      // Дырявая: одна из привязанных деталей — висячая ссылка. Прежде фолбэк печатал в колонке
      // «Привязанные детали» сам идентификатор, и оператор читал его в одном ряду с названиями.
      part_ids: shape.leaky ? [ID.part, ID.partMissing] : [ID.part],
    },
    [ID.product]: { name: 'Прокладка ГБЦ', price: 320 },
    [ID.part]: {
      name: 'Коленвал',
      part_number: 'Д245-1005020',
      // Дырявая: вторая ветка «Совместимости» читает СПИСОК марок карточки детали, и марки из
      // списка в справочнике нет. Ветка отдельная от связки ниже — фолбэк в ней свой.
      ...(shape.leaky ? { engine_brand_ids: [ID.brandMissingInList] } : {}),
    },
    // Дырявая: связка «деталь ↔ марка» указывает на марку, которой в справочнике нет вовсе.
    [ID.partLink]: { part_id: ID.part, engine_brand_id: shape.leaky ? ID.brandMissing : ID.brand },
  };
...
function operationRows(shape: FixtureShape): Row[] {
  ...
  return [
    ...
    // Этапы работ (15.09.2026): строка — запись истории ремонта с meta.sheet. Дырявая:
