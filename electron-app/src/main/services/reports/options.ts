...
/**
 * Опции «Виды работ» для фильтра отчёта «Этапы работ». Источник — тот же серверный
 * справочник, что и у экрана: виды работ живут REST-таблицей, в реплике их нет.
 *
 * `value` — КОД вида работ, а не id: этап работ называет свой вид кодом (`sheet.typeCode`),
 * и отчёт отбирает по нему же. Архивные виды включены намеренно — их этапы работ в истории
  ...
  try {
    ...
    for (const raw of rows) {
      ...
      options.push({
        ...
        ...(archived ? { hintText: 'вид работ убран в архив, строки остались' } : {}),
