...
/**
 * Phase 2 (deferred-create): returns a fresh id + empty payload WITHOUT inserting an operations
 * row. `requestNumber: ''` is the «не присвоен» sentinel — the number (date-based Z-…) and the row
 * are materialized on the first save (`updateSupplyRequest` upsert). An abandoned empty card leaves
 * no row. The department is still resolved here so the «нет подразделения» case fails early.
 */
...
): Promise<{ ok: true; requestNumber: string } | { ok: false; error: string }> {
  try {
    const cur = await getSupplyRequestRaw(db, args.id);
    // Phase 2 (deferred-create): no row yet → first save materializes it. Distinguish «not found»
    // (materialize) from a real read error (corrupt metaJson on an existing row → propagate).
    const missing = !cur.ok && cur.error === 'Заявка не найдена';
    if (!cur.ok && !missing) return cur;

    if (!canEditAllDepartments(args.scope?.role ?? '')) {
      const userDepartmentId = args.scope?.userId ? await getUserDepartmentId(db, args.scope.userId).catch(() => null) : null;
      const nextDepartmentId = String(args.payload.departmentId ?? '');
      const currentDepartmentId = cur.ok ? String(cur.payload.departmentId ?? '') : '';
      if (!userDepartmentId || nextDepartmentId !== userDepartmentId || (cur.ok && currentDepartmentId !== userDepartmentId)) {
        return { ok: false as const, error: 'permission denied: supply_requests.edit' };
      }
    }
    ...
    if (missing) {
      ...
      const note = payload.title?.trim() ? `Заявка ${requestNumber}: ${payload.title.trim()}` : `Заявка ${requestNumber}`;
      ...
      await audit(db, args.actor, 'supply_request.create', payload);
      return { ok: true as const, requestNumber };
    }
    ...
    // IMPORTANT: do NOT write audit_log on each update of an existing request — high-level audit
    // is recorded on materialize (above), status transitions, and delete.
    return { ok: true as const, requestNumber: String(args.payload.requestNumber ?? '') };
  } catch (e) {
    return { ok: false as const, error: String(e) };
  }
}
