...
## С чего начинать
- Перед каждой новой сессии запускайте: `corepack pnpm run session:start` (или `pwsh scripts/session-start.ps1`)
  - Скрипт сам приводит репозиторий к актуальному состоянию относительно `origin/main`, показывает новые коммиты и выводит краткий контекст из `docs/README.md`.
