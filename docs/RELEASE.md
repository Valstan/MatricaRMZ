...
### Известные подводные камни сборки

- **pnpm `overrides` на `minimatch` и `brace-expansion`** — не добавляйте. Они форсят свежие версии глоб-библиотек на electron-builder@25.x, и упаковка asar тихо теряет `dist/main/index.js` (sanity-check падает с `Application entry file ... does not exist`). Если security-audit ругается на эти CVE — игнорируйте: они про ReDoS через подозрительные паттерны, а в нашей build-цепочке такие паттерны не передаются. Прецедент: серия неудачных релизов 1.15.0–1.15.4 (см. v1.15.5 welcome).
- **Перед тегом релиза проверьте, что CI-сборка предыдущего тега завершилась успехом.** Если предыдущий release-workflow упал, новый тег без фикса упадёт по той же причине — попадёте в каскад патчей.
- **Артефакты v1.15.4 и ранее в "сломанной серии" — нерабочие.** Если на проде в `/opt/matricarmz/updates/` лежит `*.zip` (portable) вместо `MatricaRMZ-Setup-X.Y.Z.exe` — это знак, что installer не собирался; client electron-updater не подхватит апдейт без `latest.yml + .exe`.
