...
Если в задаче/чате написано **«выпусти релиз»**, выпускаем **только ту часть проекта, в которую внесены изменения**.
Если изменения затронули только один модуль (например, только `electron-app` или только `backend-api`), релизим только его.
Если изменения затрагивают несколько модулей — релизим каждый из них по своим правилам.

Для автоматического режима (когда нужно релизить и клиент, и backend) используем команду:
