# Требования MatricaRMZ (архив)

Этот документ хранится для истории и может быть устаревшим.
Используйте только как справку при необходимости.
...
### 3.3 Доп. уточнения из текста DOCX (важно для UI)
- В контрольном листе есть явные поля с “чекбоксами” (или вариантами выбора):
  - **Комплектность**: “комплектный / некомплект (есть недостающие детали)”
  - **Контроль номерного соответствия**: “соответствует / не соответствует”
  - **Доставка заказчику**: “доставлено” + “претензии: нет / да”
- Есть места для “вложений/файлов”:
  - **Фотофиксация**: отметка + место хранения файла (путь/ссылка)
  - **Акт исследований**: дата/номер + отметка что “оригинал акта получен”
- Есть поля “основание” для закупок (покупка выполнена на основании чего-то) — лучше хранить как `text` + (опц.) ссылку на файл.

---

## 6) Модуль “Контрольный лист ремонта двигателя” (следующий шаг реализации)

### 6.1 Представление данных в UI
Требуется форма, которая покрывает:
- **Заголовочные поля** (даты/номера/реквизиты/текстовые поля)
- **Таблицы**:
  - недостающие детали (агрегаты)
  - несоответствующие детали при сборке
- **Подписи**: “акт составлен (ФИО, должность, подпись)”

### 6.2 Где хранить (MVP)
- **Шаблон чек-листа**: master-data (EAV), чтобы версионировать/менять без миграций.
- **Заполненный чек-лист**: JSON в `operations.metaJson` (автоматически синхронизируется).

### 6.3 Минимальный формат ответов (черновик)
Заполненный чек-лист должен уметь хранить:
- выбранный шаблон
- заполненные значения полей
- табличные строки
- кто/когда заполнил
