...
| M124 | Ответ модели «пустой» или «готовый», а на деле оборван потолком `max_tokens`: раздумья делят бюджет с ответом, HTTP 200, признак конца выбрасывался | AI / LLM |
| M125 | Хэш chat id совпал у двух ботов — «чат общий»; на деле это номер пользователя, и у каждого бота своя личка | Telegram / идентификаторы |
| M126 | После рестарта пары secondary отдаёт 404 на installer и blockmap с `manifest_or_torrent_missing`: прочитал `latest.json` в тот миг, когда primary его переписывал; лечится сам через 60 с | релиз / updater |
| M127 | Новая колонка реплики через `electron-app/drizzle/*.sql` с `ALTER TABLE … ADD COLUMN` роняет клиент в цикл self-heal → «аварийное обновление»: drizzle-журнал гоняется на ВСЕХ базах, `ADD COLUMN` в SQLite не идемпотентен, а без `--> statement-breakpoint` падает даже на свежей | клиент / миграции реплики |
| M128 | Тест краснеет только в параллельном прогоне, в одиночку зелёный («таймингозависимый флейк») — а на деле общий файл: десктопный модуль шимится только для браузерного таргета, в vitest пишет настоящим `node:fs` по фиктивному пути `app.getPath` | android-порт / тесты |
| M129 | Скрипт, подключённый точкой, молча затёр переменную вызывающего — путь пришёл, а внутри пусто | [M129](#m129--подключение-скрипта-точкой-исполняет-его-param-и-молча-затирает-переменные-вызывающего) |
| M130 | Отчёт из функции PowerShell печатается одной строкой вместо списка | [M130](#m130--array-из-функции-powershell-не-разворачивается-в-конвейере-и-отчёт-печатается-одной-строкой) |
| M131 | Воркер убит OOM при обработке картинок, у разработчика всё работает | [M131](#m131--библиотека-картинок-берёт-память-по-числу-ядер-а-не-по-памяти-машины) |
| M132 | Удалил пакеты из `node_modules` — `df` не изменился | [M132](#m132--удаление-пакетов-из-node_modules-не-освобождает-место-блоки-лежат-в-сторе) |
| M133 | `pnpm` на проде ведёт себя не как у всех: чужой стор, переписанный lockfile | [M133](#m133--голый-pnpm-на-боксе-новее-пиннутого-и-смотрит-в-другой-стор) |
| M134 | Данные появляются со второго-третьего нажатия кнопки — «как будто тормозит» | [M134](#m134--занятый-синк-отвечает-результатом-прошлого-прогона-и-показ-опережает-данные) |
| M135 | Право «сняли у всех», а администраторы всё равно правят; клиентский гейт есть, строка уезжает синком | [M135](#m135--право-выдаваемое-поимённо-нельзя-держать-на-цикле-всё-админу-и-на-одном-клиентском-гейте) |
| M136 | В ленте истории ремонта у однотипных записей разные префиксы примечания — выглядит как порча данных | [M136](#m136--переименование-раздела-меняет-текст-только-у-новых-записей-старые-остаются-в-бд-со-старым-словом) |
| M137 | Переключил вкладку — состояние соседней вкладки приехало сюда и затёрло её собственное (свёрнутость, фильтр, скролл) | [M137](#m137--динамический-storagekey-у-uselistuistate-смена-ключа-не-перечитывает-состояние-зато-сразу-записывает-чужое) |
...
## M124 — ответ модели оборван потолком `max_tokens`, а выглядит пустым или готовым

- **Симптом:** разбор логов изредка падает с «Модель не вернула отчёт»; недельная сводка приходит, но конец пересказа странно куцый. Ошибок HTTP нет.
- **Корень:** модели DeepSeek работают в режиме раздумий, и раздумья тратят тот же `max_tokens`, что и ответ. При тесном потолке эндпойнт отвечает 200 с пустым или недописанным содержимым и `stop_reason: max_tokens`, а `llmProvider.ts` этот признак не читал: JSON-путь сливал обрыв с «инструмент не позван», текстовый отдавал обрезок как готовый текст.
- **Замер до правки:** 5 из 53 вызовов за 14 дней ровно в потолке — сводка 2 из 2 (1200), вечерний разбор логов 3 из 10 при 0 из 11 утренних (2048): обрывы ложатся на трудные случаи, средняя доля это прячет.
- **Лечение:** `stop` в каждой строке `llm usage`; `LlmOutputTruncatedError` у `callLlm`/`callLlmJson` (обрезанный `tool_use` за отчёт не принимается); потолки 4000/4096. Контроль-негатив в `llmProvider.test.ts`: без обрыва пустое остаётся пустым. Тул-циклы не тронуты — `PENDING` «Хвосты перевода ИИваныча», п. 4.
- **Считать обрывы:** `journalctl -u matricarmz-backend-primary --since "14 days ago" | grep '"stop":"max_tokens"'`.
- **Поймано:** 2026-09-10, по письму brain (опыт Гоньбы, G334).

## M125 — хэш chat id совпал у двух ботов, а чат не общий

- **Симптом:** `sha256(MATRICA_TELEGRAM_ALERT_CHAT_ID)` совпал с хэшем у соседнего проекта — вывод «чат оповещений общий», решение владельца развести чаты (D-083). У владельца при этом «1 общая группа» с ботом, тревоги приходят в личку.
- **Корень:** номер личного чата в Telegram равен номеру пользователя и осмыслен только в паре «бот — пользователь». Одно значение у двух ботов — две разные лички. Хэш доказал совпадение значения, а не объекта.
- **Проверка до вывода:** `getChat?chat_id=…` → `type` (`private` / `group` / `supergroup` / `channel`). Один вызов на чтение, значение печатать не нужно. `leaveChat` к личному чату неприменим.
- **Попутно:** бот не отвечал владельцу на `/sync_chat_id` ни в группе, ни в личке — отвечает только суперадмину по `telegram_login` карточки, а там было другое имя. Отброшенные сообщения не логировались; с 10.09 — след `telegram bot ignored a message…` с чатом и отправителем.
- **Общая форма:** совпадение идентификатора из локального пространства имён не доказывает совпадения объекта — спроси у владельца пространства имён тип объекта за значением.
- **Поймано:** 2026-09-10, скриншот владельца + `getChat`.

## M126 — secondary после рестарта пары читает `latest.json` в момент его перезаписи primary

- **Симптом:** сразу после `systemctl restart` обоих бэкендов `/updates/file/<exe>.blockmap` через nginx отвечает 404 через раз, `:3001` напрямую — 200, `:3002` — 404 и на blockmap, и на сам `.exe`; в `/updates/status` на `:3002` — `"lastError":"manifest_or_torrent_missing"`, при этом `latest.json` на диске цел и совпадает с установщиком. Через минуту всё зелёное без вмешательства.
- **Корень:** оба инстанса стартуют в одну секунду. Primary на первом скане пересеивает установщик и **переписывает** `latest.json` (12:39:50.363), secondary в scan-only режиме читает тот же файл (12:39:50.300) и застаёт его пустым или недописанным → `manifestRaw` пуст → состояние обнулено. Следующий рескан secondary (интервал 60 с) читает финальный манифест и восстанавливает раздачу.
- **Отличие от M40:** там `stale_manifest` держится **стабильно** и нужен второй рестарт; здесь ошибка `manifest_or_torrent_missing`, только на secondary, и уходит сама к следующему скану.
- **Приёмка:** blockmap проверять на каждом порту порознь и **не раньше, чем через минуту** после старта пары; `curl -sk … x5` через nginx как финальная проба. Без `-k` curl к `https://127.0.0.1` даёт `000` — это не ответ сервера, а отказ на самоподписанном сертификате.
- **Общая форма:** два процесса, поднимаемые одной командой, где один пишет файл, а другой его читает при старте, — гонка по построению; окно закрывает либо атомарная запись (`write tmp` + `rename`), либо порядок старта. Здесь достаточно того, что рескан самолечит.
- **Поймано:** 2026-09-12, выкат 3.27.0.

## M127 — колонка реплики через drizzle-файл: клиент уходит в self-heal и «аварийное обновление»
- **Симптом (стенд 13.09.2026, E3 плана bom-simplify):** после добавления `electron-app/drizzle/0026_*.sql` с `ALTER TABLE erp_nomenclature ADD COLUMN …` клиент не открывает окно: в `matricarmz.log` — `sqlite init failed … Failed to run the query`, затем «DB self-heal LOOP detected … entering emergency update mode». CDP отдаёт браузерный эндпоинт без page-таргетов.
- **Корень (два слоя):** (1) drizzle-мигратор клиента гоняется на **всех** базах, включая долгоживущие, где колонку уже добавила цепочка версий `clientSchemaMigrations.ts` или `alignSchemaWithServer` — `ADD COLUMN` в SQLite без `IF NOT EXISTS`, второй раз падает «duplicate column», транзакция откатывается, `index.ts` перестраивает базу; (2) файл без `--> statement-breakpoint` между операторами падает и на свежей базе — `better-sqlite3` исполняет по одному оператору.
- **Лечение:** колонку реплики добавлять **только** идемпотентными PRAGMA-обёртками: шаг цепочки в `clientSchemaMigrations.ts` (`from/to`, `PRAGMA table_info` → `ALTER` при отсутствии) и дубль в `ensureClientSchemaParity` (`migrate.ts`) для баз, минующих цепочку. Drizzle-файлы реплики — для новых таблиц (`CREATE TABLE IF NOT EXISTS`), с breakpoint'ами между операторами. Правило записано в шапке `migrate.ts` — читать её до того, как класть `.sql`.
- **Смежное:** колонка легаси-таблицы (список `LEGACY_SCHEMA_SNAPSHOT_TABLES`) двигает хеш снимка старым сборкам (<3.5.0), а они на это перестраивают базу; такие колонки прячутся из легаси-снимка через `LEGACY_HIDDEN_COLUMNS` (`diagnosticsSchemaService.ts`).

## M128 — «таймингозависимый флейк» оказался общим файлом: шим стоял только на браузерном таргете
- **Симптом (14.09.2026):** `android-app/src/core/boot.test.ts` → «без логина SyncManager честно отказывает» падает `expected true to be false` в полном прогоне пакета (примерно два раза из трёх на PC79), а в одиночку зелёный 5 из 5. Три недели числился перегрузом параллельного `-r test`.
- **Корень:** `androidShims` подменял `sessionSidecarStore` **только при `target: 'browser'`**. В vitest грузился настоящий десктопный модуль на sync `node:fs`, а `app.getPath` android-шима отдаёт фиктивный `/android/appData` — на Windows это `<диск>:\android\appData\MatricaRMZ\auth-session.json`, ОДИН файл на все параллельные воркеры. Логин любого тест-файла писал туда сессию, а `getSession` в другом файле подхватывал её fallback'ом «БД пуста → sidecar». Изоляция воркеров vitest тут бессильна: канал — файловая система, а не модульное состояние.
- **Диагностика:** флейк «зелёный в одиночку, красный в пачке» с *общим ресурсом вне процесса* выглядит ровно как перегруз. Отличать так: прежде чем списать на тайминг — искать общий файл. `app.getPath` шима отдаёт **не путь, а ключ** (комментарий в `shims/electron.ts`), поэтому любой невышимленный модуль с `node:fs` пишет по нему в корень диска. Проверка одной командой: после прогона тестов посмотреть, не появился ли `<диск>:\android\` (там же копится `userData/matricarmz.log` от невышимленного логгера — безобидно, но по тому же механизму).
- **Лечение:** шимить в vitest всё десктопное, чего на планшете нет (`sessionSidecarStore`, `watchdogHandshakeService`), а не только в браузерном бандле — тест обязан исполнять то же, что планшет. Сторож — `authOverShims.test.ts` «сессия не уезжает в файл: свежая БД не залогинена» (без шима честно краснеет).
## M129 — подключение скрипта точкой исполняет его `param()` и молча затирает переменные вызывающего
- **Симптом (14.09.2026, окно обслуживания):** `matrica-ops.ps1` получил от клиента `-ClientExe "…\MatricaRMZ.exe"`, а вкладка раздачи сообщила «клиент не найден». Ошибки нет, путь просто пуст.
- **Корень:** `. .\lan-share-firewall.ps1 -AsLibrary` выполняет блок `param()` подключаемого скрипта **в области видимости вызывающего**. У того свои `$ClientExe` и `$Port` со своими значениями по умолчанию — они перезаписывают одноимённые переменные вызывающего. Совпадение имён параметров у двух скриптов достаточно, чтобы переданное значение исчезло.
- **Лечение:** свои параметры уносить в отдельные имена (`$OpsClientExe`, `$OpsPort`) **до** первого дот-сорсинга и дальше пользоваться только ими. Само подключение обрывать ключом-библиотекой: `[switch]$AsLibrary` + `if ($AsLibrary) { return }` перед главным потоком, иначе дот-сорсинг ещё и выполнит чужой сценарий целиком (для GUI-скрипта — откроет второе окно).

## M130 — `,$array` из функции PowerShell не разворачивается в конвейере, и отчёт печатается одной строкой
- **Симптом (14.09.2026):** `lan-share-firewall.ps1 -Verify` вывел три строки отчёта склеенными через пробел в одну. Ни ошибки, ни обрезки — просто нечитаемо.
- **Корень:** идиома `,$lines` в конце функции нужна, чтобы массив из ОДНОЙ строки не схлопнулся в скаляр — она оборачивает результат, и обёртку разворачивает выводом ровно один раз. Дальше по конвейеру уезжает **сам массив как один элемент**, и `| ForEach-Object { Write-Host $_ }` печатает его через строковое представление массива.
- **Лечение:** потребителю — `foreach ($line in (Get-Отчёт …)) { … }` вместо `| ForEach-Object`. Присваивание (`$x = Get-Отчёт`), `+=` и передача в параметр `[string[]]` с `,` работают правильно — ломается именно конвейер.

## M131 — библиотека картинок берёт память по числу ядер, а не по памяти машины
- **Симптом (14.09.2026):** воркер бэкенда убит OOM-killer'ом при приёме снимков; на машине разработчика та же правка работает.
- **Корень:** `sharp` держит кэш libvips и пул потоков **по числу ядер**, не спрашивая, сколько памяти есть. На прод-боксе 1536 МБ, снимки по 20 Мп — два-три одновременно, и памяти нет. На разработческой машине это невидимо.
- **Лечение:** сразу при импорте — `sharp.cache(false)`, `sharp.concurrency(1)`, и `limitInputPixels` на разумном потолке. Правило шире `sharp`: у любой библиотеки, выбирающей параллелизм «по железу», целевая машина может быть втрое слабее сборочной.

## M132 — удаление пакетов из `node_modules` не освобождает место: блоки лежат в сторе
- **Симптом (прод, 15.09.2026):** `du -sh node_modules` показывает 975 МБ, но `pnpm store prune` снимает 12 пакетов, а после удаления 341 каталога из `.pnpm` свободное место на диске не меняется вовсе.
- **Корень:** pnpm кладёт файлы в `node_modules/.pnpm` **хардлинками** в глобальный стор. Блоки одни и те же, у инода два имени. `du` по каждому пути отдельно считает их себе, поэтому и стор (934 МБ), и `node_modules` (975 МБ) «весят» почти весь объём — а union у них 993 МБ. Пока на инод ссылается стор, удаление ссылки из `node_modules` освобождает ноль.
- **Диагностика:** `du -shc <стор> <node_modules>` **одной командой** — общий итог считает хардлинки один раз, и разница между итогом и слагаемыми и есть степень разделения. У нас собственных блоков у `node_modules` оказалось 59 МБ из 975.
- **Лечение:** снимать ссылки, потом `corepack pnpm store prune` — только он убирает вторую половину пары. Порядок обратный (сначала prune) бесполезен: стор считает референсным всё, на что ссылается хоть один проект.

## M133 — голый `pnpm` на боксе новее пиннутого и смотрит в другой стор
- **Симптом (прод, 15.09.2026):** `pnpm store path` отвечает `~/.local/share/pnpm/store/v11`, а каталога `v11` не существует; фактический стор проекта — `v10`.
- **Корень:** в `PATH` лежит системный `/usr/bin/pnpm` **12.4.1**, а проект пиннут `packageManager: pnpm@10.26.1` и ходит через corepack. Разные мажоры pnpm — разные версии формата стора; каждый работает со своим. Голый `pnpm install` на такой машине не просто «промахнётся мимо кэша» — он переустановит всё в новый стор и перепишет `pnpm-lock.yaml` под свой формат, а прод-клон после этого не примет `git pull --ff-only` в выкате.
- **Лечение:** на проде — **только `corepack pnpm`**. Проверять до работы с зависимостями: `corepack pnpm store path` (должен указывать на существующий каталог) и `corepack pnpm --version` против `packageManager` в корневом `package.json`.

## M134 — занятый синк отвечает результатом прошлого прогона, и показ опережает данные
- **Симптом (владелец, 15.09.2026):** «Карточка двигателя → Детали и акты → Дефектовка → *Провести дефектовку*. Список выходит, а личные номера экземпляров — нет. Жмёшь второй и третий раз — появляются. Как будто где-то тормозит или тащит с сервера».
- **Корень (два слоя).** (1) `SyncManager.runOnce` при уже идущем синке **не запускал ничего и возвращал `lastResult` ПРОШЛОГО прогона** — то есть на вопрос «синхронизируй и скажи, как прошло» приходило `ok: true` от работы, закончившейся раньше, чем вызывающий вообще что-то изменил. (2) Вызывающий (`RepairChecklistPanel`) этот ответ ещё и выбрасывал (`.catch(() => undefined)`, `ok` не проверялся) и сразу читал **реплику**, где данных с сервера ещё нет; `loadRepairPartData` безусловно писал `setStampedInstances([])` и затирал показанное. Со второго нажатия работало не потому, что «досчиталось», а потому, что успевал доехать ФОНОВЫЙ синк.
- **Диагностика.** Признак класса: «со второго-третьего раза появляется» почти никогда не про скорость — это показ, опережающий данные. Искать не «где медленно», а **кто соврал про готовность**: результат синка выброшен, `ok` не проверен, чтение реплики сразу после записи на сервере. Второй признак — отказ чтения, пишущий пустой список: «данных нет» и «не смогли прочитать» на экране выглядят одинаково.
- **Лечение.** Занятость синка больше не отвечает чужим результатом: обращение во время чужого прогона ждёт его и запускает **свежий** (идущий мог стартовать до изменения, и его результат ничего не доказывает), очередь глубиной один. Вызывающий проверяет `ok` и говорит оператору словами, если обмен не удался. Отказ чтения не стирает показанное.
- **Смежное:** то же правило для любой пары «записали на сервер → читаем реплику»: между ними обязан стоять синк, чей исход **проверен**, а не вызван «на всякий случай».

## M135 — право, выдаваемое поимённо, нельзя держать на цикле «всё админу» и на одном клиентском гейте
- **Симптом (владелец, 15.09.2026):** «доступ к редактированию ведомостей вразнобой — слишком много людей допущено, видимо, администраторы». Право `work_sheets.edit` ни одна операторская роль не давала, но admin / superadmin / легаси `user` получали его молча: `defaultPermissionsForRole` раздаёт ВСЕ коды циклом и вычитает короткий список.
- **Корень (два слоя).** (1) Новый код права по умолчанию попадает в «всё админу» — «выдаётся поимённо» держится только если код явно исключён из цикла (образец — `audit.view`). (2) Строка ведомости — запись `operations` типа `repair_history_entry`, уезжает обычным синком; гейт `work_sheets.edit` стоял только в IPC клиента, а серверная политика (`ledgerAuthz`) видела лишь `operation_type` и требовала `operations.edit` (есть у мастеров), причём admin / `user` обходили requirement вовсе. Отличить строку ведомости от ручной записи можно только по `meta_json` (`entryType: 'sheet'`).
- **Диагностика.** У «поимённого» права проверять три места: цикл ролей в backend `auth/permissions.ts`, IPC клиента и серверный гейт синка. Один зелёный гейт из трёх — не гейт. Для строк, которые пишутся через синк, спросить: «по какому полю сервер узнаёт эту строку?» — если только по `operation_type`, а различие в meta, сервер её не узнаёт.
- **Лечение.** Код исключён из цикла (`= r === 'superadmin'`), сторож в `WorkSheetsPage.guard.test.ts`; `ledgerWriteRequirement` получил `operationMetaJson` и `isWorkSheetRowWrite`; в `ledgerAuthzGuard` — универсальный backstop ДО ветки `!operatorScoped` (`forbidden:work_sheet_row`), права грузятся и для admin, если в батче есть такая строка. Снятие у уже существующих — скрипт `perm:revoke-work-sheets` (явный `allowed=false`, не удаление строк).
- **Смежное:** M123 (гейт по префиксу канала), G29 (необратимое на проде — с подтверждением в том же ходе).

## M136 — переименование раздела меняет текст только у новых записей; старые остаются в БД со старым словом
- **Симптом:** в ленте истории ремонта двигателя (и в колонке «Примечание» конструктора отчётов) у одинаковых по смыслу записей разные префиксы: часть говорит «Ведомость: Обкатка», часть — «Этап работ: Обкатка». Читается как порча данных или как две разные сущности.
- **Корень:** текст примечания **вмерзает в строку при записи** (`workSheetService.ts`, `operations.note`), а не собирается при показе. Переименование C1 («ведомости работ» → «этапы работ», 16.09.2026) поменяло только то, что пишется дальше; строки, лежащие в БД с 15.09, переписать было нечем — миграции note в задаче не было.
- **Диагностика.** Прежде чем чинить «разнобой», спроси: подпись **вычисляется при показе или хранится в строке?** Хранится — значит переименование даёт смешанный словарь навсегда, и это не баг кода. Проверить, кто читает префикс: `grep -rn "Ведомость: "` — на 16.09 читателей нет вовсе (классификация записи идёт по `meta.entryType` / `meta.sheet`, отчёт — по `meta.sheet.typeCode`, гейт синка — по той же meta), так что функционально смешение безопасно.
- **Лечение.** Ничего не лечим в коде. Либо принять разнобой (решение 16.09: принят), либо разовая миграция `operations.note` у записей `repair_history_entry` с `meta.sheet` — замена ведущего «Ведомость: » на «Этап работ: ». Это запись в живые прод-данные, то есть **только с подтверждением владельца в том же ходе** (#025 / G29).
- **Смежное:** тот же класс — любой снимок подписи, положенный в строку: имя цеха в строке этапа работ, скопированная подпись ярлыка Верстака и записи «недавних посещений» (после C1 у операторов ярлык остался «Ведомости работ», пока не переименуют руками).

## M137 — динамический `storageKey` у `useListUiState`: смена ключа не перечитывает состояние, зато сразу записывает чужое
- **Симптом:** два экрана (или две вкладки одного экрана) делят один хук памяти UI и различаются только ключом хранилища. Переключился — и видишь состояние соседа: свёрнутый блок «Сведения» разворачивается там, где его сворачивали, «своё» состояние не возвращается никогда, а после пары переключений оба экрана показывают одно и то же. Выглядит как «память не работает», на деле она работает слишком усердно.
- **Корень:** `useListUiState` (`electron-app/src/renderer/src/ui/hooks/useListBehavior.ts`) читает хранилище **ровно один раз** — в инициализаторе `useState`, — а пишет в `useEffect` с зависимостями `[storageKey, state]`. Значит смена `storageKey` на лету делает две вещи, и обе не те: (1) состояние **не** перечитывается — в руках остаётся состояние прежнего ключа; (2) эффект срабатывает сразу и кладёт **это чужое состояние под новый ключ**. Не пустое чтение, а тихая перезапись: сохранённое значение соседа уничтожено, перезапуск не поможет. Класс общий для любого «ключ хранилища — переменная, чтение однократное, запись по ключу»; на том же хуке стоит `usePersistedScrollTop`, то есть так же ведёт себя и запомненный скролл.
- **Диагностика.** Ищется не на экране, а в коде: шаблонная строка или тернарник в первом аргументе хука (``useListUiState(`card:engine:acts:${actView}:ui`, …)``). Подтверждение — в самом хранилище: посмотреть `sessionStorage` до и после переключения; если под ключом соседа лежит значение первого экрана, дело в записи, а не в чтении. На экране признак — «состояние соседа приехало сюда и не отлипает».
- **Лечение:** **по вызову хука на каждый ключ**, ключи — литералами, а выбор нужного состояния уже в рендере. Правила хуков соблюдены: оба вызова происходят всегда, ветвится только результат. Образец — акт-вкладки карточки двигателя (D1, 17.09.2026): `useListUiState('card:engine:acts:completeness:ui', …)` и `useListUiState('card:engine:acts:defect:ui', …)`, дальше `const sections = isDefectView ? defectSections : completenessSections;`; сторож панели требует оба литерала и запрещает шаблонную строку в ключе. Если ключей столько, что по вызову не расставить, — читать хранилище в эффекте на смену ключа и писать **только после** чтения, но это уже другой хук, и заводить его надо явно.
- **Смежное:** M78 (инлайновый `display` перебивает `hidden` на скрытой вкладке) и M120 (замер в скрытой вкладке) — соседний класс «состояние живёт дольше, чем экран, который его показывает».
