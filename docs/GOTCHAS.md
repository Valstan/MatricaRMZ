...
| M108 | «Проходит примерно половина» бывает не сетью, а вторым потребителем: Telegram `getUpdates` отвечает `409 Conflict` второму поллеру того же токена. Повторы не лечат — это определённый ответ, а не потеря пакета | прод / Telegram |
...
## M108 — «половина запросов теряется» бывает не сетью, а вторым потребителем токена

- **Симптом:** в логе primary регулярно, около 30 раз в сутки, `sync pipeline bot poll failed … telegram HTTP 409: Conflict: terminated by other getUpdates request; make sure that only one bot instance is running`, `"transient":false`, `streak` каждый раз сбрасывается в 1. Между отказами опрос идёт нормально. Выглядит ровно как потеря пакетов из M101 — «то проходит, то нет».
- **Разница, из-за которой это отдельная грабля:** M101 — потерянный SYN, ответа нет вообще, и повтор его лечит. Здесь ответ **есть** и он определённый: у бота уже есть другой `getUpdates`-потребитель, и Telegram отдаёт долгий опрос тому, кто пришёл последним. Повторы не помогают ни разу — сколько ни повторяй, второй потребитель никуда не делся. Класс общий: «проходит примерно половина» — это либо монета сети, либо **делёж канала с кем-то ещё**, и лечения у них противоположные.
- **Что затрагивает, а что нет:** ломается только **входящий** канал — команды боту (кнопки sync-pipeline, `MATRICA_SYNC_PIPELINE_TELEGRAM_ACTIONS_ENABLED`) достаются то нам, то второму потребителю. Исходящие `sendMessage` (алерты, critical-events, ночной отчёт супервизора) идут другим методом и работают. Поэтому «алерты доходят» — не доказательство, что канал цел.
- **Как отличить за минуту:** `409` в тексте ошибки; на боксе при этом ровно один поллер — `systemctl list-units | grep <проект>`, `ps -eo pid,cmd | grep -i telegram`, и опрос включён только у primary (у secondary в логе нет ни одной строки `bot poll`). Один поллер на боксе + устойчивые 409 = потребитель снаружи бокса, то есть тот же токен живёт ещё где-то.
- **Поймано:** 2026-09-06 при суточном наблюдении за журналом в PG. Смежное: пункт про Telegram был закрыт 04.09 по диагнозу M101 — повторы действительно вылечили исходящую сторону, а входящая всё это время оставалась разделённой, и в счётчиках отказов канала это неотличимо, пока не прочитать текст ошибки.
- **Родня:** M101 (тот же симптом, другой корень и противоположное лечение).
