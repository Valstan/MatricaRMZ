...
| M92 | `gh release download` молча отдаёт обрезанный или нулевой файл на крупном ассете | релиз / артефакты |
| M93 | Плановая задача «при входе» отсутствует на всех машинах, а лог твердит `schtasks create failed` | watchdog / Windows |
| M94 | Запущенный `.ps1` исчезает с диска, `powershell` выходит с кодом 21 и пустым выводом | антивирус / инструменты |
| M95 | Сборка APK падает на «X is not exported by shims/…», хотя PR с этой правкой был зелёным | android / гейты |
| M96 | Ротация `MATRICA_JWT_SECRET` в `.env` «не действует»: свежий токен `token_invalid`, старые живут | прод / секреты |
| M97 | Запрос к системным каталогам PG возвращает строки в psql, а в коде массив «пустой» | БД / драйвер |
| M98 | `unattended-upgrades` каждый день рапортует «нечего обновлять», а система отстала на месяцы | прод / обновления |
| M99 | Таблица выглядит пустой/мёртвой в `pg_stat_user_tables`, а в ней десятки тысяч строк | БД / прод-аудит |
| M100 | После рестарта primary `/health` на :3001 не отвечает, `is-active` говорит `active` — выглядит зависшим | прод / рестарт |
| M101 | `api.telegram.org` с прод-бокса «не отвечает»: одиночный `curl` даёт таймаут, а это потеря ~половины SYN на сети хостера — лечится повторами, не сменой IP | прод / Telegram |
| M102 | Голый uuid в `sync_state` — это id аккаунта, не машины: push пишет `client_id = actor.id`, pull — настоящий clientId; «Последняя отправка» у машин всегда пуста | sync / диагностика |
| M103 | «В проекции этого нет» — не доказательство: проверь тем же запросом заведомо присутствующего соседа, иначе отрицательный результат пуст по построению | диагностика / ledger |
| M104 | Блок в цепочке ledger ≠ состоявшаяся транзакция: `appendBlock` публикует блок и индекс раньше проекции и PG, обрыв между ними оставляет «блок-призрак» — 1000 транзакций, которых нет нигде | ledger / целостность |
| M105 | Проекция `state.json` упирается в предел строки V8 (~512 МБ): `JSON.stringify` всего состояния на каждый блок падает `std::bad_alloc`, а не «out of memory»; массовая запись в ledger растит проекцию быстрее, чем блоки | ledger / память |
| M106 | `pgrep -f <имя>` через `ssh host '…'` находит саму ssh-команду: проверка «процесс жив» врёт часами. Проверять по pid-файлу или `pgrep -f '[b]ackfill'` | прод-ops |
| M107 | Смена генератора номеров на живой системе: старый и новый счётчик пишут в одну таблицу с первичным ключом по номеру — отказы записи у половины парка либо строка ниже курсора, невидимая клиенту навсегда. Писателей останавливать | выкат / sync |
| M108 | Периодический опрос без флага «уже выполняется» конфликтует сам с собой: повторы на медленной сети растягивают проход дольше тика, и Telegram отвечает `409 Conflict` нашему же следующему проходу. Ищется по меткам времени, а не по коду | прод / Telegram |
| M109 | Колбэк «сервис готов» ждёт ВСЕ подсистемы, поэтому один сломанный сокет прячет успех остальных: трекер слушает, а строки `tracker listening` нет ни разу — виден только отказ. Частный случай: `udp6` нельзя забиндить на `'0.0.0.0'` (`EINVAL`) | прод / раздача обновлений |
| M110 | `.ps1` с кириллицей без BOM: PowerShell 5.1 читает файл как ANSI, и падение выглядит синтаксической ошибкой или «не найден позиционный параметр» — причина не в коде, а в кодировке | скрипты / Windows |
| M111 | Один и тот же фильтр («Заказчики») отбирает в двух отчётах разные наборы: цепочка «сущность → её связь → значение» сложена в каждом отчёте отдельно и разошлась | отчёты / EAV-провенанс |
| M112 | Фильтр сравнивает одно представление значения, а в данных их два (ссылка и текст); либо фолбэк читает поле, которого в источнике нет вовсе — и «второй путь» жив только в фикстуре теста | отчёты / EAV / фикстуры |
| M113 | Разбивка «всего» на подколонки не покрывает всё множество: остаток молча оседает в соседней колонке. Итоги отчёта сходятся, тесты зелёные, а «всего» больше суммы своих частей — читатель ищет пропавшие строки | отчёты / инварианты |
| M114 | Булев флаг из EAV через `Boolean(value)`: строковое `'false'` — непустая строка, то есть **истина**. Зеркально `value === true` роняет строковое `'true'`. Одно и то же «нет» лежит в базе тремя представлениями | EAV / отчёты |
| M115 | Сырой байт NUL в исходнике: `grep`/`ripgrep` считают файл бинарным и **молча пропускают** — он выпадает из каждого сплошного поиска по коду, и аудит рапортует «чисто» | инструменты / аудит |
| M116 | Лечение «безнадёжная строка уходит в error» не действует: соседняя функция того же файла возвращает её в очередь, проверив НЕ ту причину (схему вместо родителя) — вечный цикл push→skip→error→recover | sync / dead-letter |
| M117 | Счётчик здоровья считает ПОПЫТКИ, а называется «строками»: один залипший клиент даёт тысячи «пропущенных строк» в сутки и держит статус `critical` вечно, пряча настоящие пропуски | диагностика / гейты |
| M118 | Сравнение «список тот же» по подписи, в которую забыли добавить новое поле | списки / кэш |
| M119 | Гейт прав возвращает УСПЕХ, а его читают как «отказ» | права / мост |
| M120 | Замер ширины в скрытой вкладке (нули) и признак, потерянный горячей перезагрузкой | UI / замеры |
| M121 | Счётчик пропусков есть, а следа в журнале нет — беду называют, но найти по ней нечего; плюс дофиксовые снимки врут ещё целое окно после лечения | диагностика / гейты |
| M122 | Родитель создаётся отложенно, а ребёнок сохраняется сам: ребёнок уезжает на сервер раньше родителя («критичная» тревога на каждую новую карточку), брошенная карточка оставляет сироту, а два автосохранения подряд — два ребёнка | sync / отложенное создание |
| M123 | Префиксный гейт раздела захватил запись телеметрии вместе с чтением журнала: сквозной канал отбивается у всех, кто в раздел не входит | права / гейт разделов |
...
- **Лечение:** качать `.exe` **отдельным** вызовом с щедрым таймаутом (не в одной команде с blockmap), после скачивания **всегда сверять размер + sha512 с `latest.yml`** перед scp на прод; при несовпадении — `rm` и перекачать. Общий принцип — brain pool #011 «верь содержимому, не сигналу успеха» (rc 0 ≠ файл целый; ср. M18). Правило «`gh release download` — только локально, на прод через `scp`» — AGENTS.md §Release шаг 8a; локальный download тоже может усечься.

## M30 — backend-скрипт на проде подписал sync-изменения «в никуда» (паразитный ledger, низкие seq)
- **Снято 2026-09-05:** цепочки блоков на диске больше нет (план `ledger-journal-in-pg`), журнал и номера живут в той же PostgreSQL, что и данные — «паразитный ledger» невозможен. Прод-скриптам по-прежнему нужен боевой env, но уже ради `PG*`.
...
## M90 — действие из панели МЕНЮ считает исход от устаревшего Верстака: скрытая keep-alive вкладка держит стейл-колбэки

- **Симптом:** тумблер галстука на вкладке убрал ярлык в корзину, следом «Добавить на Верстак» из контекстного меню кнопки МЕНЮ сказал «добавлен» — а на сервере ярлык остался в корзине; либо наоборот, действие из меню молча возвращает Верстак к состоянию минутной давности. Воспроизводится только CDP-драйвером (он шлёт события в скрытую панель); оператор руками нажимает в видимую.
...
- **Корень:** тест дошёл до функции с бесконечным циклом и не может выйти, поэтому репортер не успевает напечатать ни строчки. Классический источник — поиск «первого подходящего места» с условием, которое при некоторых входах не выполняется НИКОГДА: `for (let row = 0; ; row++) for (let col = 0; col + cells <= cols; col++) …` крутится вечно, если `cells > cols` (внутренний цикл не выполняется ни разу). Поймано на раскладке Верстака: плитка в две ячейки при Верстаке в одну колонку.
...
- **Лечение:** убить зависшие процессы по PID (оборванная команда их не снимает), починить цикл и **оставить тест на этот вход** — он и есть регрессия. В раскладке лечение — зажать запрос по ширине Верстака (`Math.min(cells, cols)`).
- **Поймано:** 2026-08-25, этап C Верстака (`desktopLayoutGrid`).

## M92 — `gh release download` молча отдаёт обрезанный (или нулевой) файл на крупном ассете

- **Симптом:** команда завершается успешно, файл на диске есть — но он в разы меньше ассета. На релизе v3.11.0 APK скачался как **212 КБ** вместо 25 МБ, повторные попытки давали **0 байт**, и ни одна из них не напечатала ошибки. Родственно **M18** (мульти-`--pattern` роняет blockmap): `gh` не проверяет полноту скачанного.
- **Корень:** флаки-сеть на пути к GitHub CDN. Ошибки на этом пути и так известны (на самом проде `gh release download` воспроизводимо валится TLS-таймаутом, см. AGENTS.md §Release шаг 8a) — просто здесь она не падает, а обрывается тихо.
- **Диагностика:** взять эталонный размер у самого GitHub, а не глазами: `gh api repos/Valstan/MatricaRMZ/releases/tags/<tag> --jq '.assets[] | "\(.name) \(.size)"'`. Второй ориентир — соседние версии на проде: `ls -la /opt/matricarmz/updates/android/` (APK стабильно ~25 МБ, инсталлятор ~136 МБ).
- **Лечение:** качать **циклом со сверкой размера**, а не одной командой:
  ```bash
  for i in 1 2 3 4; do rm -f "$D/app-release.apk"; gh release download android-vX.Y.Z --pattern "*.apk" -D "$D" --clobber >/dev/null 2>&1;
    [ "$(stat -c%s "$D/app-release.apk" 2>/dev/null || echo 0)" = "<size из gh api>" ] && break; done
  ```
  Запускать **из каталога репо** — вне git-контекста `gh` вообще отказывается работать (`failed to run git: fatal: not a git repository`) и оставляет пустой каталог, что легко принять за «скачал ноль байт».
- **Цена ошибки:** незамеченный обрезанный APK уезжает `scp`'ом на прод, Диспетчер честно отдаёт его планшетам по имени файла — и парк получает битый установщик. Проверять размер **до** `scp`, а не после.
- **Поймано:** 2026-08-25, релиз v3.11.0, шаг 8a.

## M93 — плановая задача «при входе» отсутствует на всех машинах, а лог твердит `schtasks create failed`

- **Симптом:** `Get-ScheduledTask -TaskPath '\MatricaRMZ\*'` возвращает одну задачу вместо двух — `Watchdog Periodic` есть, `Watchdog Logon` нет. В `matricarmz.log` при каждом старте клиента `watchdog heal: schtasks create failed for MatricaRMZ\Watchdog Logon`, а следом текст в cp866, который в UTF-8-консоли выглядит мусором и потому не читается: «ОШИБКА: Отказано в доступе».
- **Корень:** `schtasks.exe` **не умеет** создать logon-задачу для текущего пользователя без прав администратора. Замер (Windows 11 22631): `/SC ONLOGON` → отказ, `/SC ONLOGON /RU <имя>` в любой форме имени → отказ, `/SC ONSTART` → отказ, `/SC MINUTE /MO 15` тем же вызовом → успех. PowerShell-API это умеет (`New-ScheduledTaskTrigger -AtLogOn -User <имя>` проходит; без `-User` — тоже отказ, потому что это «при входе любого пользователя»), но powershell как дочерний процесс запрещён ADR-0002 как LOLBin-триггер.
- **Диагностика:** не гадать про антивирусный свип — свип уносит задачу разово, а тут ровный ряд неудач. Отделяется одним опытом: создать пробную задачу в **своём** пространстве имён двумя расписаниями (`/SC MINUTE` и `/SC ONLOGON`) и сравнить коды возврата. Разные исходы = ограничение прав, одинаковые = что-то внешнее. Пробы потом снять.
- **Лечение:** автозапуск при входе — **ярлык в папке автозагрузки** (`%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup`), нативным `shell.writeShortcutLink`; прав не требует, powershell не поднимает. Сторож собран `-H=windowsgui`, поэтому консоль при входе не мелькает.
- **Цена ошибки:** самолечение годами делало невозможное и молчало об этом в локальный файл. Отсюда правило: неудача самолечения обязана иметь канал наружу (у нас — `POST /client/watchdog/report`, kind `autostart_broken` → «Критические события»), иначе поломки страховки не видит никто.
- **Поймано:** 2026-08-26 (находка brain про пропавшую задачу на `rmz4val`).

## M94 — запущенный `.ps1` исчезает с диска, `powershell` выходит с кодом 21 и пустым выводом

- **Симптом:** скрипт отработал два-три раза нормально, потом запуск стал давать `exit=21` без единой строки вывода (ни stdout, ни stderr), а сам файл `.ps1` **пропал с диска**. Соседние файлы того же каталога на месте, копия, которую не запускали, — тоже. В журнале Windows (`Application`) ни одной записи.
- **Корень:** Касперский. Скрипт опрашивал `avp.com`, реестр, планировщик и резолвил адрес сервера — для неподписанного PowerShell это готовый профиль поведения; вердикт «созревает» не с первого запуска, поэтому начало выглядит рабочим. Тот же почерк, что уносил каталог приложения и плановые задачи (brain #192, D-044).
- **Диагностика:** разделить «удаляет при запуске» и «удаляет фоновым сканом» — положить файл и НЕ запускать (у нас пролежал спокойно), затем запустить и проверить наличие сразу и через минуту. Пустой вывод при этом легко списать на обрыв пайпа — проверять редиректом в файл, а не через `| head`.
- **Лечение:** сначала исключение антивируса на каталог, потом запуск. Файл держать в git — тогда он восстанавливается `git checkout -- <путь>`, а не переносится заново из источника.
- **Цена ошибки:** инструмент, который настраивает исключения антивируса, сам удаляется антивирусом до того, как исключения внесены. Порядок «инструмент → исключения» не работает; работает только «исключения → инструмент».
- **Поймано:** 2026-08-26, перенос `scripts/client-ops/` (три воспроизведения подряд).

## M95 — сборка APK падает на «X is not exported by shims/…», хотя PR с этой правкой был зелёным

- **Симптом:** `vite build` планшета останавливается на `"<новый экспорт>" is not exported by "src/shims/<модуль>.ts", imported by "../electron-app/src/main/services/<файл>.ts"`. Падение всплывает НЕ в том PR, где появился экспорт, а в следующем — часто в релизном, где менялся `shared/`.
- **Корень:** двойной. (1) Сборка планшета тянет **main-процесс** electron-app: часть модулей подменяют шимы (`android-app/src/shims/`), остальные компилируются как есть — значит новый экспорт в шимованном модуле обязан появиться и в шиме. (2) Гейт `build-apk` на PR слушал только `android-app/**`, `shared/**`, `electron-app/src/renderer/**` — то есть **не тот путь, который его и ломает**. Путь `electron-app/src/main/**` добавлен 2026-08-26.
- **Диагностика:** ошибка называет и шим, и импортирующий файл — читать её буквально, а не искать «поломку сборки». Локальная проверка одной командой: `corepack pnpm -F @matricarmz/android-app build`.
- **Лечение:** дописать в шим no-op с той же сигнатурой и комментарием, ПОЧЕМУ он пустой (на планшете сторожа нет). Пустой экспорт без объяснения читается как забытая реализация.
- **Урок шире случая:** гейт, который не слушает путь своей же поломки, создаёт худший вид ложного зелёного — правка уезжает в main, а краснеет потом чужой PR. Заводя шим или платформенную развилку, сразу проверь, какие пути видит её гейт.
- **Поймано:** 2026-08-26, релиз v3.13.0 (экспорт `reportWatchdogAutostartIfBroken` из PR #694).

## M96 — ротация `MATRICA_JWT_SECRET` в `.env` «не действует»: свежий токен `token_invalid`, старые живут

- **Симптом:** заменил `MATRICA_JWT_SECRET` в `backend-api/.env`, перезапустил оба сервиса — а свежевыпущенный токен не проходит `/auth/me` (`token_invalid`), при этом старые токены продолжают работать. Выглядит как «ротация не подействовала».
- **Корень:** секрет живёт в ДВУХ файлах. Сервисы берут окружение из systemd `EnvironmentFile=/etc/matricarmz/matricarmz.env`; `backend-api/.env` читают только CLI-скрипты (`run-with-backend-env.mjs`, tsx с dotenv). Переменные systemd уже стоят в окружении процесса, dotenv их не перекрывает — правка `.env` для сервисов невидима.
- **Диагностика:** `systemctl cat matricarmz-backend-primary` → строка `EnvironmentFile=`. Состав файлов сравнивать по именам (`grep -oE '^[A-Z_]+'`), значения не печатать — транскрипт.
- **Лечение:** менять секрет в ОБОИХ файлах одинаково (каждому бэкап с правами 600), затем rolling-рестарт с `/health`; проверка — `/auth/me` новым токеном.
- **Урок шире случая:** у секрета может быть больше одного дома. Перед ротацией выяснить, откуда его читает КАЖДЫЙ потребитель (systemd, dotenv, CI), а не только самый видимый файл.
- **Поймано:** 2026-08-28, отзыв засветившегося release-токена сменой JWT-секрета.

---

## M97 — запрос к системным каталогам PG возвращает строки в psql, а в коде массив «пустой»

- **Симптом:** SQL руками в `psql` отдаёт строки, тот же запрос из приложения — вроде бы отрабатывает, но потребитель получает пусто. Ошибок нет, лог чист. У нас так вышло, что снимок схемы годами приезжал клиентам БЕЗ единого уникального индекса, включая первичные ключи, и никто не замечал: пустой список — валидный ответ.
- **Корень:** `array_agg(a.attname)` по системным каталогам даёт тип **`name[]`**, а node-postgres такой массив **не разбирает** — значение приходит СТРОКОЙ `"{code}"`. Потребитель проверял `Array.isArray(row.columns)`, получал `false` и молча отбрасывал запись. Это касается всех каталожных полей типа `name`: `attname`, `relname`, `conname`, `nspname`.
- **Диагностика:** не сравнивать вывод `psql` с ожиданием — печатать `typeof` и `Array.isArray` того, что реально пришло в объекте. Разрыв «в psql есть, в коде нет» живёт ровно в разборе типа.
- **Лечение:** приводить к тексту в самом запросе — `array_agg(a.attname::text)`. Отдельно решить, что делать с потребителем: он мог годами работать на пустых данных, и оживление гейта — это изменение поведения, а не починка опечатки.
- **Урок шире случая:** результат, потерянный при разборе типа, **неотличим от «данных нет»**. Любой гейт, который считает пустой ответ нормой, обязан хоть раз быть проверен на непустых данных — иначе он мёртв с рождения и зеленеет вечно.
- **Поймано:** 2026-08-30, при подготовке релиза v3.17.0 — попутно сняло ложную находку, построенную на предположении, что этот путь работает.

## M98 — `unattended-upgrades` каждый день рапортует «нечего обновлять», а система отстала на месяцы

- **Симптом:** в `/var/log/unattended-upgrades/unattended-upgrades.log` изо дня в день `No packages found that can be upgraded unattended and no pending auto-removals`, служба зелёная. При этом ручной `apt update` + `apt list --upgradable` показывает сотни пакетов, включая security.
- **Корень:** обновляет **списки** пакетов не тот таймер, который ставит обновления. `apt-daily.timer` (он делает `apt update`) стоял `disabled` при `preset: enabled`, а `apt-daily-upgrade.timer` (он ставит) был `enabled` и честно отрабатывал каждое утро — по кэшу многомесячной давности. Установщик смотрел в протухшие списки и **правдиво** докладывал, что обновлять нечего.
- **Диагностика:** `systemctl list-unit-files 'apt-daily*'` — сверять состояние **обоих** таймеров, не только того, что в логе; `ls -la /var/lib/apt/lists` — дата файлов. Косвенный признак прямо в логе: предупреждение `Could not figure out development release: Distribution data outdated`.
- **Лечение:** `systemctl enable --now apt-daily.timer`, затем `apt update && apt list --upgradable` — проверять по факту, а не по логу.
- **Урок шире случая:** механизм из двух половин рапортует только по своей половине. Зелёный отчёт исполнителя — не доказательство, что он видел актуальные данные: проверять надо источник, которым он питается. Тот же класс, что «гейт, который не исполняет артефакт, который охраняет» (brain #228).
- **Поймано:** 2026-08-30, техобслуживание прод-бокса по мандату владельца: 261 пакет (218 security) не ставился с мая.

## M99 — таблица выглядит пустой/мёртвой в `pg_stat_user_tables`, а в ней десятки тысяч строк

- **Симптом:** обзорный запрос `select relname, n_live_tup from pg_stat_user_tables order by n_live_tup desc` на проде показывает `attribute_values` 489 строк (реально 45 116), `directory_parts` 0 (реально 1201), `change_log` 0 (реально 162 670) при размере таблицы 60 МБ. Вывод «таблица мёртвая, читатель висит на пустоте» строится на песке.
- **Корень:** `n_live_tup` — **оценка**, которую обновляют autovacuum/ANALYZE. После массовых правок (миграции EAV→строгие таблицы, бэкфиллы) она расходится с фактом на порядки и живёт так, пока не придёт autoanalyze. Размер (`pg_total_relation_size`) при этом честный — отсюда и характерная примета: «0 строк в таблице на 60 МБ».
- **Диагностика:** приметой служит расхождение размера и счётчика. Настоящие счётчики по всем таблицам разом:
  ```sql
  select c.relname,
         (xpath('/row/cnt/text()', query_to_xml(format('select count(*) as cnt from %I.%I', n.nspname, c.relname), false, true, '')))[1]::text::bigint as rows,
         pg_size_pretty(pg_total_relation_size(c.oid)) as sz
  from pg_class c join pg_namespace n on n.oid = c.relnamespace
  where n.nspname = 'public' and c.relkind = 'r'
  order by 2 desc;
  ```
  (Через `pg_stat_user_tables` тот же запрос падает на `__drizzle_migrations` — её нет в `public`.)
- **Лечение:** в аудитах «что живо, что мертво» брать `count(*)`; `n_live_tup` годится только для прикидки порядка **свежепроанализированной** таблицы.
- **Урок шире случая:** оценка, названная как факт (`n_live_tup` = «live tuples»), читается как факт. Прежде чем строить на числе вывод «источник мёртв», спроси, кто и когда его посчитал.
- **Поймано:** 2026-09-03, первый прогон проверки перекосов (пункт 14 владельца): по статистике половина `directory_*` выглядела пустой — при том, что нитка parts на них и стоит.

## M100 — после рестарта primary `/health` на :3001 не отвечает, а `is-active` говорит `active`

- **Симптом:** `sudo systemctl restart matricarmz-backend-primary` → `systemctl is-active` сразу печатает `active`, но `curl http://127.0.0.1:3001/health` даёт `Failed to connect … Couldn't connect to server`, а `ss -ltnp` не показывает слушателя на 3001. При этом `curl -fsk https://127.0.0.1/health` через nginx отвечает `200` — потому что в upstream жив secondary. Похоже на зависший primary.
- **Корень:** primary — singleton-инстанс (`runBackgroundJobs: true`), и между строкой `backend instance config` и строкой `listening on 127.0.0.1:3001` он поднимает фоновую обвязку (AI-воркер, планировщик дайджеста, торрент-трекер и раздачу инсталлятора). Замер 2026-09-04: **53 секунды** — 00:16:44 → 00:17:30. Secondary поднимается за секунды, отсюда обманчивая асимметрия. `systemd` рапортует `active` сразу после `fork`, до бинда, так что `is-active` про готовность не говорит **ничего**.
- **Диагностика:** смотреть не порт, а журнал — `sudo journalctl -u matricarmz-backend-primary --since "-5min"`; готовность — строка `listening on 127.0.0.1:3001`. Косвенно: `ps -o etime -p <pid>` покажет, сколько процесс реально живёт (легко принять «журнал молчит» за «висит», когда прошла минута, а не десять).
- **Лечение:** ждать бинд, а не гадать: `for i in $(seq 1 45); do sleep 2; curl -fsS -m 5 http://127.0.0.1:3001/health >/dev/null && break; done` — окно **не меньше 90 с**. Тридцати секунд не хватает, и это ровно тот случай, когда сессия объявляет ложную аварию на проде.
- **Урок шире случая:** `is-active` — это «systemd запустил», а не «сервис обслуживает». Приёмка рестарта — только по ответу порта или по строке `listening` в журнале.
- **Поймано:** 2026-09-04, при выкате порога `MATRICA_MAX_LOCAL_BYTES` — тридцатисекундного ожидания не хватило, primary успел забиндиться на 23-й секунде после окончания окна.
- **Амендмент 2026-09-06: пятидесяти секунд больше нет.** После снятия цепочки блоков (журнал переехал в PostgreSQL) primary биндится за **6 секунд** — замер при выкате #796. То есть основную часть той минуты съедала загрузка проекции цепочки в память на старте, а не «фоновая обвязка» вообще. Симптом и урок остаются в силе (`is-active` по-прежнему не про готовность), но широкое окно ожидания теперь нужно не всегда — судить по строке `listening`, а не по константе.

## M101 — `api.telegram.org` с прод-бокса «не отвечает», а на деле теряется половина SYN

- **Симптом:** `curl -m 8 https://api.telegram.org/` с бокса — `Connection timed out` (rc 28, http 000); DNS резолвит, пин в `/etc/hosts` на месте, CA-бандл цел, `cloud-api.yandex.net` с того же бокса отвечает. Выглядит как «Telegram заблокирован целиком», и `MATRICA_TELEGRAM_ENABLED=false` кажется единственным выходом.
- **Корень:** не блокировка, а потеря пакетов на сети хостера. Замер 04.09.2026 голым TCP-connect (`</dev/tcp/149.154.167.220/443`, 20 попыток): **11/20** с нашего бокса, **13/20** с бокса Сетки (тот же myjino, та же `10.100.0.0/16` за `venet0`); контроль `1.1.1.1:443` — 20/20, ICMP до того же адреса — 0 % потерь, порт 80 и все остальные DC-диапазоны Telegram (`149.154.16x`, `91.108.x`) режутся так же. С домашней машины владельца — 3/3 мгновенно. То есть режется именно TCP к диапазонам Telegram на стороне хостера или его аплинка, вероятностно; SNI ни при чём (TLS до отказа не доходит). **Установленное соединение живёт нормально**: по keep-alive три `getMe` подряд — 0,46 / 0,15 / 0,15 с.
- **Диагностика:** не один `curl`, а серия голых connect'ов и контроль к чужому хосту. Один таймаут при p≈0,5 — это не «канал мёртв», это монета. Смотреть три числа: доля успешных SYN к Telegram, к контролю, и ICMP до того же IP.
- **Лечение:** короткая попытка + повтор, не смена адреса и не прокси. curl: `--connect-timeout 4 --retry 6 --retry-delay 1 --retry-all-errors` (6 попыток при p=0,55 → 99,7 %); Node: `telegramFetch` в `telegramBotService.ts` — `AbortSignal.timeout` на попытку и до `MATRICA_TELEGRAM_ATTEMPTS` (6) повторов только на сетевой отказ, HTTP-ответ любого кода не повторяется. Undici держит keep-alive к одному origin, поэтому после первого удачного SYN опрос идёт без потерь.
- **Урок шире случая:** «Сарафан работает» означало «каждый второй запрос проходит, а неудачи никто не считает» — там те же потери, просто отправок много и `timeout=15`. Прежде чем строить обход (relay, прокси), измерить вероятность отказа: обход через бокс с той же сетью ничего бы не дал.
- **Поймано:** 2026-09-04 по мандату D-076; до этого канал считался мёртвым с 03.09, а critical-events молчали с ≤ 26.05.2026 (флаг `false` во всех сохранённых копиях env, начиная с копии от 26.05; тестовая доставка по `SECURITY.md` — 18.05).

## M102 — «Клиент пушит, но ни разу не пуллил»: `sync_state.client_id` при push — это id ПОЛЬЗОВАТЕЛЯ

- **Симптом:** в `sync_state` есть строки с голым uuid без префикса hostname (`0693eca7-…`, `b1aac85c-…`, `system`), у них `last_pushed_at` свежий, а `last_pulled_at` пуст и `last_pulled_server_seq = 0`; у настоящих машин (`PC19-…`) наоборот — `last_pushed_at` всегда `NULL`. Выглядит как «неизвестная машина, которая только шлёт» — и на неё уходит разбор.
- **Корень:** `syncWriteService.ts` передаёт в `applyPushBatch` `client_id: actor.id || actor.username || 'server'` — то есть **id аккаунта**, а pull пишет `sync_state` под настоящим `clientId` машины. Одна таблица, два разных ключа. `0693eca7-…` = аккаунт `mubvera`, машина — PC19 на 3.19.0.
- **Диагностика:** голый uuid в `sync_state` — сначала `select id, login from users where id = '<uuid>'`, и только потом искать машину. Машина — по `client_settings` того же логина (`last_seen_at`, `last_version`).
- **Следствие для отчётов:** колонка «Последняя отправка» (`reports.ts`, `syncState.lastPushedAt`) у машин **всегда пуста**, а у «аккаунтов» показывает время. Не чинить заменой ключа задним числом: `sync_state` читают диагностика и отчёты, а push-ключ виден ещё и в `logSkip` (`client_id`). Правка — отдельный PR: писать в push настоящий `clientId` из тела запроса и мигрировать строки.
- **Соседняя грабля того же дня:** строки, пропущенные сервером как `missing_dependency`, оставались `pending` и уходили в **каждый** push (PC19 — 8 нарядов на 4 несуществующих двигателя, раз в 22 с, неделю, без единой ошибки на экране). С 3.20.0 клиент возвращает в очередь зависимость, если она есть локально, иначе помечает строку `error` (`sync/dependencyRequeue.ts`).
- **Поймано:** 2026-09-04 при разборе первого critical-event, дошедшего в Telegram после починки канала.

## M103 — «в проекции этого нет» ничего не доказывает, пока не проверен заведомо присутствующий сосед

- **Симптом:** ищешь в `ledger_tx_index` семь пропавших из цепочки транзакций (`seq` 421759…421765), получаешь `count = 0` и делаешь вывод «потеряны и в базе тоже».
- **Почему вывод ложный:** `ledger_tx_index` — не копия цепочки, а проекция «последняя запись на строку». На 04.09.2026 в ней 845 801 строка против 1 507 914 транзакций в блоках, то есть **44 % записей отсутствуют штатно**. Ноль по любому старому `seq` — norma, а не находка.
- **Контроль, который решает:** спросить тем же запросом **соседей**, заведомо присутствующих в блоках. Для случая выше `seq 421758` и `421766` дали такой же ноль, как и весь диапазон 421700–421800 — значит молчание таблицы про этот участок ничего не говорит ни о чём.
- **Правило:** отрицательный результат поиска засчитывается, только если тот же поиск нашёл заведомо существующий контроль. Без непустого контроля «не найдено» означает «не искали там, где надо».
- **Родня:** та же семья, что «пустой вывод неотличим от несостоявшейся проверки» и `grep -c "Backup failed"` в логе, где алерт уходил в Telegram, а в лог писалось `ERROR:`. В этой же ревизии контроль спас второй раз: первая проверка хешей блоков объявила «не сходится» **все** блоки подряд — здоровые контрольные показали, что сломана моя формула (реальная — `prev|created_at|tx_id,tx_id`, а не JSON), а не данные.
- **Поймано:** 2026-09-04 при первой ревизии цепочки ledger.

## M104 — блок в цепочке ledger ≠ состоявшаяся транзакция: «блок-призрак» после обрыва в `appendBlock`

- **Симптом:** пересборка состояния из блоков (`ledger:rebuild-state`) даёт `DATA_DIVERGENCE` по `attribute_values`: 767 строк есть только в цепочке, 190 — в цепочке новее, чем в `state.json`. Все — из одного блока `386592` (13.08.2026 13:51 UTC, seq 1360011…1361010, 1000 транзакций массового push'а). В PG этих строк нет, в `ledger_tx_index` ровно этот диапазон seq отсутствует (по корзинам 1000: …1000, **588, 412**, 1000…), в проекции их тоже нет.
- **Корень:** порядок в `appendBlock` (`ledger/src/store.ts`): файл блока → `saveIndex` → `loadState`/`applyTxs`/`saveState` → и только потом, уже вне ledger'а, применение пакета в PG (`syncWriteService.ts`, шаг 2) и `ledger_tx_index` (шаг 3). Обрыв процесса между записью блока и `saveState` (тут — M79: `JSON.stringify` проекции на 194 МБ при массовом push'е) оставляет блок и продвинутый индекс, а транзакции не применяются **нигде**. Клиент получает ошибку и повторяет push уже с новыми seq — цепочка хранит и провалившуюся попытку, и удачную.
- **Чем опасно:** любая «пересборка из цепочки» или «доказательство по цепочке» воскресит транзакции, которые рабочее хранилище не принимало. Цепочка — **не надмножество истины**, а журнал попыток. Сверять пересборку надо с PG, а не только с проекцией; «блок есть» — ещё не «данные были».
- **Как ловится:** `ledger_tx_index` по корзинам seq — провал ровно в один блок; `last_server_seq` строк PG по тому же диапазону — тот же провал; проекция по `per-block` сверке — тот же блок. Три независимых свидетеля на один диапазон.
- **Родня:** зеркальный случай — записи в PG **мимо** ledger'а (скрипты миграций пишут прямо в таблицы: 1147 `operations` за 05.2026 с `last_server_seq IS NULL`, мягкие удаления при старом seq). Обе дыры смотрят в разные стороны, и ни одна не видна из самой цепочки.
- **Поймано:** 2026-09-04 вторым, полным прогоном `rebuild-state` на проде. Разбор — `PENDING_FOLLOWUPS.md` §«Второй прогон rebuild-state».
- **Закрыто (вариант А, 05.09.2026):** все 1000 `row_id` блока сверены с PG (767 нет, 190 старше блока, 43 обновлены клиентом позже другими seq; ни у одной `updated_at` не равен метке блока). Блок записан в `KNOWN_GHOST_BLOCKS` (`rebuildLedgerStatePlan.ts`) — пересборка его читает, называет и **не применяет**. Сам `rebuild-state` больше не судит о данных (вердикт только о читаемости цепочки); сверка «цепочка ↔ PG» по открытому тексту — `resnapshot-state --chain-rebuilt`.

## M105 — проекция `state.json` упирается в предел строки V8: `bad_alloc` на `JSON.stringify` при массовой записи в ledger

- **Симптом:** бэкфилл `engine-inventory:backfill-lines --apply` (291 тыс. строк блоками по 1000) умер после 49 блоков: `terminate called after throwing an instance of 'std::bad_alloc'`, код 134. Не «heap out of memory» — падает нативная аллокация строки. Свободной памяти на боксе при этом было достаточно.
- **Корень:** `appendBlock` на **каждый** блок делает `loadState` → `applyTxs` → `saveState`, а `saveState` — это `JSON.stringify(state, null, 2)` **всей** проекции одной строкой. Максимальная строка V8 — `buffer.constants.MAX_STRING_LENGTH` = 536 870 888 символов (~512 МБ). Проекция была 218 МБ; каждая строка списка деталей весит ~1 КБ с отступами (855 Б компактно) — 49 тыс. строк дали 287 МБ, полные 291 тыс. дали бы ~510 МБ. Компактная запись без отступов экономит ~20 % и предел лишь отодвигает.
- **Чем опасно:** предел не виден заранее — проекция растёт молча, а первый симптом — падение писателя ledger'а посреди пачки (здесь — скрипт; при росте от обычных push'ей это упал бы backend на каждом append). Рост проекции = рост стоимости **каждого** append'а: 287 МБ парсятся и пишутся заново на любую галочку оператора (та же семья, что M79).
- **Как ловится:** `ls -la state.json` до и после массовой записи; `node /tmp/state-size.js state.json` (потабличный вес, компактный vs с отступами, предел строки). Порог тревоги — 400 МБ.
- **Лечение:** таблицы, которые истина держит в PG и которые никто не читает из проекции, в проекцию не класть (список непроецируемых таблиц в `ledger/src/state.ts`), а `appendBlock` не должен переписывать проекцию, если блок её не изменил. Разбор — `PENDING` §«Список деталей → строгая таблица».
- **Поймано:** 2026-09-05 на первом прогоне бэкфилла на проде; 49 000 строк в PG/индексе/проекции сошлись, данные не пострадали.

## M106 — `pgrep -f` через ssh находит саму ssh-команду: «процесс жив» врёт часами

- **Симптом:** `ssh matricarmz 'pgrep -f backfillEngineInventoryLines'` печатает pid, счётчик строк в PG стоит на месте три часа, а процесс на самом деле умер через 6 минут после старта (`bad_alloc`, см. M105).
- **Корень:** удалённая команда исполняется через `bash -c '<вся строка>'`, и в строке есть имя искомого процесса — `pgrep -f` честно находит эту оболочку. То же с `ps aux | grep <имя>` без `grep -v grep`.
- **Чем опасно:** ожидание «пока процесс жив» никогда не кончается; итоговый отчёт строится на предположении, что работа идёт. Второй случай класса brain #284 «гейт без области»: проверка, условие которой не может стать ложным, — не проверка.
- **Как ловится:** в той же команде печатать **прогресс** (счётчик строк, размер лога) и сравнивать с прошлым замером; либо `pgrep -f '[b]ackfill…'` (скобки ломают самосовпадение), либо pid-файл от `nohup … & echo $! > /tmp/x.pid` и `kill -0 $(cat …)`.
- **Поймано:** 2026-09-05, тот же бэкфилл. Записать вместе с M105: одно падение, две грабли.

## M107 — два счётчика номеров против одного первичного ключа: смена генератора на живой системе

- **Симптом (поймано ДО прода, состязательной ревизией 06.09.2026):** при выкате журнала в PostgreSQL миграция ставила `setval('ledger_seq', max(server_seq), true)`, а порядок выката был «мигрировать → собрать → перезапустить инстансы». В этом окне старая сборка ещё работает и раздаёт номера из своего файлового счётчика (`index.json` цепочки), записывая их в ту же `ledger_tx_index`, где `server_seq` — первичный ключ.
- **Два исхода, и второй хуже первого.** (1) Новый инстанс берёт номер, который старый уже занял: `duplicate key value violates unique constraint`, вся транзакция журнала откатывается, `/ledger/tx/submit` отвечает 400. nginx балансирует между старым и новым инстансом, поэтому примерно половина пушей парка падает, вторая проходит. (2) Наоборот, старый инстанс берёт номер, который новый уже отдал: его вставка в журнал шла с `onConflictDoNothing`, то есть **молча** пропадает, а строку sync-таблицы он всё равно штампует этим номером. Клиент, чей курсор уже прошёл это число, не получит строку никогда — pull отбирает по `last_server_seq > since`.
- **Почему не ловится глазами:** первый исход громкий, второй бесшумный, и оба выглядят как «сеть барахлит». Метрика отставания журнала при этом показывает ноль: она сравнивает таблицу саму с собой.
- **Правило:** генератор номеров (последовательность, счётчик, id-сервер) меняется только при **остановленных писателях**. Ротацию «на живую» не спасает ни точный `setval` (он устаревает между `SELECT` и `COMMIT`), ни зазор в номерах (он закрывает отказ записи, но не невидимую строку).
- **Страховка в коде:** `ledgerService.ensureSequenceAheadOfJournal` поднимает последовательность до максимума таблицы и повторяет запись при `23505` — это делает занятый номер восстановимым состоянием, а не отказом. От второго исхода не спасает ничто, кроме останова писателей.
- **Родня:** M30 (тот же класс с другой стороны — писатель со своим, отставшим счётчиком), M104 (журнал и хранилище расходятся при обрыве).

## M108 — периодический опрос без флага «уже выполняется» конфликтует сам с собой

- **Симптом:** в логе primary около 30 раз в сутки `sync pipeline bot poll failed … telegram HTTP 409: Conflict: terminated by other getUpdates request; make sure that only one bot instance is running`. Текст ошибки прямым текстом говорит «запущено больше одного экземпляра бота», и разбор естественно уходит искать второй процесс, второй бокс, чужой проект с тем же токеном. Искать там нечего.
- **Корень:** опрос заведён как `setInterval(() => void processBotUpdates(), 15_000)` (`syncPipelineSupervisorService.ts:427-433`) — **без флага in-flight**. Один проход при этом живёт много дольше тика: `telegramFetch` (`telegramBotService.ts:34-47`) делает до `MATRICA_TELEGRAM_ATTEMPTS` (6) попыток по 8 с с паузами, а `processBotUpdates` внутри себя делает до трёх раундов (`:339/343/348`) — то есть до ~50–155 с. На сети, теряющей около половины SYN к Telegram (M101), медленные проходы штатны. Следующий тик уходит в `getUpdates`, пока предыдущий ещё висит, и Telegram обрывает **наш же** предыдущий опрос — 409 приходит от нас самих.
- **Как отличить самоконфликт от настоящего второго потребителя — по меткам времени, а не по коду.** `streak` инкрементируется один раз за проход и обнуляется только при успехе, поэтому два-три отказа **внутри одного тика** возможны лишь при нахлёсте. Наш замер: `07:19:32.021 streak=1` → `07:19:37.841 streak=3` (три завершённых прохода за 5,8 с при тике 15 с), плюс `14:04:38.209` и `14:04:41.598` (Δ 3,4 с). Внешний постоянный поллер дал бы, наоборот, сплошные серии и растущий streak, а не одиночные всплески (77 строк со `streak=1` на одну со `streak=3`).
- **Что затрагивает, а что нет:** ломается только **входящий** канал — команды боту. Исходящие `sendMessage` идут другим методом и работают, поэтому «алерты доходят» ничего не доказывает про целость канала.
- **Лечение:** флаг in-flight у тика (или `setTimeout`-цепочка вместо `setInterval`), а не смена токена и не поиск виноватого снаружи. Попутно: параллельные проходы делят `updateOffset` (`:385`) — одна команда может быть исполнена дважды.
- **Урок шире случая:** текст ошибки от чужого сервиса описывает **его** наблюдение («вижу два потребителя»), а не вашу топологию. Два потребителя — это могли быть вы дважды.
- **Поймано:** 2026-09-06. Первая версия этой записи утверждала обратное — что потребитель снаружи бокса; она стояла на отсутствии улик (в логе secondary нет строк `bot poll`), и это ровно M103. Опровергнута замером меток времени.
- **Флаг — половина лечения; вторая половина арифметическая (амендмент 2026-09-07).** Через сутки после выката флага 409 стало ноль, но строка «проход медленнее своего тика» шла **228 раз за 11 часов**: наложения больше нет, а проход по-прежнему не влезал в период — флаг честно выбрасывал каждый третий-четвёртый тик. То есть флаг убирает **следствие** (обрыв соседом), а причина — бюджет: попытки × таймаут × круги повторов против периода. Считать это надо в момент правки повторов, а не по логам через сутки: у нас было 6 × 8 с × 3 круга ≈ 155 с против 15 с.
- **Для задачи по расписанию расписание и есть повтор.** Повторы **внутри** прохода у периодической задачи дублируют её же период и почти всегда лишние: отказ прохода ничего не теряет, если состояние не сдвинулось (у нас — `updateOffset`). Лечение: свой, укороченный бюджет у опроса (2 × 5 с ≈ 10 с < 15 с), круги повторов сняты, а бюджет **отправки** оставлен широким — вот у неё повторить действительно некому. Арифметику держит тест, включая случай «переменные окружения вернули прежние значения»; строка про медленный проход оставлена растяжкой на будущее.
- **Родня:** M101 (тот же симптом «проходит примерно половина», другой корень; здесь именно повторы из M101 и растягивают проход), M103 (отрицательный результат без контроля ничего не значит).

## M109 — колбэк готовности ждёт все подсистемы, и один сломанный сокет прячет успех остальных

- **Симптом:** при каждом старте primary в логе `[WARN] tracker error {"error":"Error: bind EINVAL 0.0.0.0:6969"}` — и ни одной строки `tracker listening` за всё время (0 вхождений за 7 суток при 10 рестартах). Читается однозначно: «трекер не поднялся». На деле `ss -lntup` показывает и tcp, и udp на 6969 за pid'ом primary, а `curl` на `:6969/announce` отвечает корректным bencode. Сервис работает, лог утверждает обратное.
- **Корень, частный:** `bittorrent-tracker` при `{ udp: true }` поднимает **два** сокета — udp4 и udp6, — а `listen(port, '0.0.0.0')` отдаёт один и тот же адрес обоим. udp6-сокет на IPv4-литерале падает с `EINVAL` (не `EADDRNOTAVAIL` и не `EADDRINUSE` — отсюда ложный след «повторный бинд» / «конфликт портов»). Лечится адресом по протоколам: `listen(port, { http: '0.0.0.0', udp: '0.0.0.0', udp6: '::' })`.
- **Корень, общий и более полезный:** колбэк `listening` у этой библиотеки считает `!!http + !!udp4 + !!udp6` и срабатывает, **только когда готовы все три**. Один упавший сокет — и колбэк не вызывается никогда. То есть обработчик ошибки печатает отказ, а лог успеха молчит: **виден отказ подсистемы, не виден успех сервиса**. Этот перекос и есть грабля: по логу нельзя отличить «не поднялся вовсе» от «поднялся частично».
- **Как проверять такое за минуту:** не по логу, а по факту — слушает ли кто-то порт (`ss -lntup`) и отвечает ли эндпоинт. Лог здесь свидетель ненадёжный по построению.
- **Контроль перед правкой** (иначе чинишь вслепую): поднять ту же библиотеку локально обоими способами. Замер 06.09.2026 на свободных портах: `'0.0.0.0'` → `listened: false`, `bind EINVAL`; `{ http, udp, udp6: '::' }` → `listened: true`, ошибок нет. Прод-симптом воспроизводится на ноутбуке за две секунды.
- **Поймано:** 2026-09-06. Жило с 30.08 (раньше журнал не хранит) и никого не беспокоило ровно потому, что раздача обновлений от этого трекера не зависит — см. `PENDING` §«Торрент-раздача не имеет потребителя».

## M110 — `.ps1` с кириллицей без BOM: падение выглядит синтаксической ошибкой, а виновата кодировка

- **Симптом:** свежий скрипт `scripts/client-ops/*.ps1` не разбирается вовсе. Парсер сыплет «Непредвиденная лексема "…"», «Отсутствует закрывающий знак ")"», а при запуске — «не удалось найти позиционный параметр, принимающий аргумент "<мусор>"». Номера строк указывают на совершенно исправные места, и в аргументе — нечитаемая каша вместо русского текста.
- **Корень:** Windows PowerShell 5.1 (`powershell.exe`, не `pwsh`) файл **без BOM** читает в кодировке ANSI текущей системы. Русские строки распадаются на байты, часть которых для парсера — обычные символы, и он честно спотыкается на них как на синтаксисе. То есть сообщение об ошибке описывает следствие, а причина на два уровня ниже.
- **Как отличить за секунду:** `node -e "const b=require('fs').readFileSync(f); console.log(b[0]===0xEF&&b[1]===0xBB&&b[2]===0xBF)"`. Соседние рабочие скрипты в репозитории лежат с BOM — это и есть контроль.
- **Лечение:** сохранять `.ps1` в **UTF-8 с BOM**. Инструменты, пишущие файлы, BOM по умолчанию не ставят, поэтому его надо добавить явно: `fs.writeFileSync(f, Buffer.concat([Buffer.from([0xEF,0xBB,0xBF]), buf]))`.
- **Отдельно про временные файлы:** та же грабля бьёт по одноразовым `.ps1`, которые агент пишет через heredoc для проверки чего-нибудь. Если такой файл ASCII-only — проблемы нет; как только в нём появляется русская строка (хоть в комментарии), он ломается тем же способом. У нас это дважды за один заход увело разбор в сторону: сначала «сломан скрипт», потом «неверный набор параметров cmdlet», а сломана была кодировка обоих временных файлов.
- **Поймано:** 2026-09-06 при добавлении `lan-share-firewall.ps1`. Родня: G21 (Windows-инструменты по умолчанию пишут UTF-16 и превращают код-файл в `Bin` для git) — тот же класс «редактор молча выбрал не ту кодировку», другой конец.

## M111 — один фильтр, три трактовки: цепочка провенанса, сложенная в каждом отчёте заново

- **Симптом:** оператор выбирает одного и того же заказчика в двух отчётах по двигателям и получает разные наборы. Ошибки нет, оба отчёта «работают», расхождение видно только если сверять построчно. В третьем отчёте — третий ответ.
- **Корень:** заказчик у двигателя в карточке обычно не заполнен, он выводится по цепочке `двигатель → его договор → заказчик договора`. Цепочку каждый отчёт складывал у себя: «Движение по заказчикам» — с фолбэком на договор и с разбором `contract_sections`; «Двигатели» — только поле карточки; «Наряды» — без `counterparty_id` у двигателя и без разделов у договора, по одному легаси-`customer_id`. Три места, три ответа, ни одного признака поломки: тесты у каждого свои и каждый зелёный относительно собственной трактовки.
- **Почему не ловится гейтами:** отличается не результат вычисления, а **область** — какие данные вообще попадают в отбор. Типы одинаковые, значения валидные, счёт сходится. Такое расхождение видит только сверка двух отчётов на одних данных, и её никто не делает, пока не спросит владелец.
- **Диагностика:** грепать по обоим концам цепочки, а не по имени поля — `counterparty_id`, `customer_id`, `parseContractSections` — и смотреть, сколько раз рядом стоит фолбэк. Число разных написаний = число трактовок. Проверять на данных, где промежуточное звено пустое (карточка без заказчика при известном договоре): на полностью заполненных строках все варианты дают одно и то же и расхождение не проявляется.
- **Лечение:** одна функция на цепочку, все читатели через неё (`resolveEngineCounterpartyId` + `buildContractCounterpartyIndex` в `electron-app/src/main/services/reports/context.ts`), и тест, который прогоняет **разные отчёты на одной фикстуре** и требует одинакового отбора — иначе четвёртый отчёт снова сложит цепочку сам.
- **Поймано:** 2026-09-07 при постановке каскада фильтров отбора. Решение владельца: заказчик ищется всеми доступными путями, и одинаково везде, где по смыслу это одно и то же понятие.

## M112 — фильтр знает одно представление значения, а данные хранят два

- **Симптом:** отчёт по фильтру отдаёт меньше, чем должен, а иногда — разное при одинаковом запросе. Ошибки нет, счёт по видимым строкам сходится.
- **Корень (форма 1, «ссылка против текста»):** марка двигателя живёт и ссылкой `engine_brand_id` на справочник, и текстом `engine_brand` в карточке. Фильтр отдаёт идентификаторы, поэтому карточка с одним текстом молча выпадает. Хуже, если код кладёт оба значения в **одну** ячейку (`Map<engineId, string>`): побеждает то, что встретилось в EAV последним, и результат зависит от порядка строк — фильтр не узкий, а **недетерминированный**. На проде 07.09.2026: 1936 двигателей несут оба поля, 2 — только текст.
- **Корень (форма 2, «фолбэк в никуда»):** запасной путь читает поле, которого в источнике нет. Журнал движений и выработка цехов откатывались на `raw.warehouseId`, а в клиентской реплике `erp_reg_stock_movements` такой колонки нет вовсе. Следствие: при недоступном справочнике складов подпись схлопывалась в прочерк у **всех** строк, а разрез «по цехам» отдавал **ноль строк**, неотличимый от «за период ничего не ремонтировали».
- **Почему держалось годами:** фикстура сторожа `humanLabels.guard.test.ts` подавала движению поле `warehouseId` — то есть тест кормил код данными, которых в реальной таблице не бывает, и мёртвый путь оставался зелёным. **Фикстура, расходящаяся со схемой источника, не проверяет код — она его консервирует.**
- **Диагностика:** для каждого фильтра выписать, сколькими способами значение попадает в данные, и сравнить с тем, сколько способов читает код. Проверять на строках, где заполнено **не то** представление (только текст, только легаси-поле): на полностью заполненных все варианты дают одно и то же. Отдельно сверять поля фикстур со схемой таблицы — грепом по имени колонки в `database/schema.ts`.
- **Лечение:** одно место, читающее оба представления (`resolveEngineBrandRef`), и один матчер, сравнивающий по любому из них (`buildBrandFilterMatcher` в `reports/context.ts`). Там, где второго источника у клиента действительно нет (справочник складов приходит по сети, локальной реплики нет), — не угадывать, а **назвать причину пустоты** в отчёте: пустая таблица обязана отличаться от «не смог определить».
- **Поймано:** 2026-09-07 при сплошной ревизии фильтров отчётов, начатой после M111. Родня: M111 (та же болезнь на цепочке связей, а не на представлениях значения), M103 (отрицательный результат без контроля).

## M113 — «всего» больше суммы своих подколонок: разбивка не покрывает всё множество

- **Симптом:** владелец читает бумагу и спрашивает, куда делись строки. «Движение двигателей по заказчикам»: утиль всего **5**, утиль на заводе **3**, утиль отправлен **0** — двух двигателей нет нигде, при том что общий итог сходится (`пришло 20 = отгружено 11 + утиль отправлен 0 + на заводе 9`) и подпись под таблицей уверенно печатает «из них ещё на заводе 3, возвращено заказчику 0».
- **Корень:** «утиль» — это признак, а состояний у двигателя три (уехал утилем · уехал не утилем · на заводе), и разбивка накрывала только два из них. Утильным двигатель признаётся флагом `status_scrap_confirmed` **или** `status_rework_sent`, а колонка «утиль отправлен» читала **только второй** флаг. Двигатель, признанный утильным и уехавший заказчику обычной отгрузкой, попадал в «отгружено заказчику» и выпадал из разбивки утиля. На проде 07.09.2026 таких **39** из 231 утильного (188 на заводе, 5 по флагу возврата, 39 потерянных).
- **Почему держалось:** проверялись инварианты `пришло = отгружено + утиль отправлен + на заводе` и `на заводе = утиль на заводе + в ремонте` — оба **истинны** и при дыре, потому что потерянные строки честно лежат в «отгружено». Третьего инварианта — `утиль всего = утиль на заводе + утиль отправлен` — не было ни в коде, ни в тестах, а фикстура не содержала ни одного двигателя этого класса: тест покрывал ровно те два состояния, которые код и умел различать.
- **Диагностика:** для каждой колонки «всего» выписать её подколонки и проверить, что их условия **разбивают** множество, а не выбирают из него два удобных случая. Быстрая проба на данных: `count(*) FILTER (WHERE признак) - сумма count(*) FILTER (WHERE подколонка)` — ненулевая разница и есть класс строк, которого нет в бумаге.
- **Лечение:** состояния строить взаимоисключающими от одного признака (`leftFactory` → `scrapSent = утиль && ушёл`, `shipped = не утиль && ушёл`, `atFactory = остался`), а сумму частей печатать в пояснении рядом с «всего» (`Утиль всего = утиль на заводе + утиль отправлен (5 = 3 + 2)`) — тогда следующая дыра видна на бумаге сразу, без вопроса владельца. Инвариант — в тест, на каждую строку отчёта.
- **Поймано:** 2026-09-07 по вопросу владельца «а куда делись два утильных двигателя?». Родня: M111 и M112 (та же семья «отчёт уверенно печатает неполную правду»), M103 (вывод из отсутствия улик).

## M114 — `Boolean('false')` — это истина: булев флаг из EAV имеет три представления

- **Симптом:** двигатель числится утильным / отремонтированным / забракованным, хотя в карточке галочка снята. Ошибок нет, у большинства строк всё верно, воспроизводится только на «тех самых» записях.
- **Корень:** в EAV одно и то же «нет» лежит **тремя** способами — настоящим булевым `false` (так пишет карточка), строкой `'false'` (импорт 11.06.2026) и отсутствием строки вовсе. Код снимал флаг как `Boolean(attrs[code])`, а `Boolean('false') === true`. На проде 23 значения-строки, из них 14 — `'false'`: `status_rejected` ×4, `status_repaired` ×3, `status_repair_started` ×3, `status_rework_sent` ×3, `status_customer_sent` ×1.
- **Зеркальная форма, не менее тихая:** строгое `value === true` роняет строковое `'true'` из того же импорта. Так читал гейт выдачи наряда сборки: утильный двигатель со строковым флагом гейт бы пропустил. На проде утильных флагов строкой `'true'` не оказалось — повезло, а не защитило.
- **Третья форма, худшая:** `normalizeText(attrs.flag, '')` — он возвращает **строку**, и `'false'` в условии истинна. Так селектор «Комплектование двигателя» прятал 147 живых двигателей из 378, у которых метку утиля когда-либо ставили и снимали.
- **Почему держалось:** на подавляющем большинстве записей все три разбора дают одно и то же — расходятся они ровно на строках-исключениях, которых в фикстурах не было. Плюс разнобой внутри одного файла: `resolveEngineShippingState` читал те же поля строгой проверкой и отвечал правильно, а `statusFlags` рядом — нет.
- **Диагностика:** `SELECT ad.code, av.value_json, count(*) … WHERE av.value_json LIKE '"%'` — сколько булевых атрибутов лежит строками. Если больше нуля, любой `Boolean(...)` и любой `=== true` по этому полю подозрителен.
- **Лечение:** один разбор на весь проект — `isEavFlagSet` (`shared/src/domain/contract.ts`), плюс `statusFlagsFromAttrs` для снимка всех статусов. Ручных `Boolean(attrs[flag])` по EAV в коде не остаётся. Данные не чинили: чинить представление в БД можно, но читатель обязан быть устойчив и без этого.
- **Поймано:** 2026-09-07 сплошной ревизией отчётов после M113. Родня: M112 (то же «два представления одного значения», но на ссылке против текста).

## M115 — сырой байт NUL в исходнике делает файл невидимым для поиска по коду

- **Симптом:** файл не находится грепом ни по одной строке, которая в нём точно есть. Никакой ошибки: `grep` печатает `Binary file … matches` (или вовсе молчит под `-l`), `ripgrep` пропускает такой файл по умолчанию.
- **Корень:** в файле есть байт NUL — обычно как разделитель составного ключа, вписанный **самим символом** вместо escape-последовательности (`` `${a}<NUL>${b}` ``). Инструменты определяют бинарность по наличию NUL в начале файла, а не по расширению.
- **Чем опасно:** файл выпадает из **каждого** сплошного поиска — ревизии, аудита секретов, сторожей человекочитаемых подписей, любого «проверил весь код грепом». Аудит при этом рапортует «чисто»: отрицательный результат неотличим от «не смотрел». Найдено 07.09.2026 ровно так — своим же аудитом, который эти файлы не увидел: `shared/src/domain/payments.ts`, `backend-api/src/services/engineInventoryLinesService.ts` (там NUL внутри ключа детерминированного UUID) и `backend-api/src/services/snapshotCrypto.test.ts`.
- **Диагностика и защита:** сторож `shared/src/domain/sourcesAreText.guard.test.ts` обходит `*/src` и падает на любом файле с NUL. Разово: `grep -rlI '' <dir>` печатает только текстовые файлы — разница со списком всех и есть невидимые.
- **Лечение:** заменить байт на escape-последовательность. Строка получается **побайтово та же**, поэтому хеши и детерминированные идентификаторы не меняются — правка безопасна для уже существующих данных.
- **Поймано:** 2026-09-07. Родня: M103 (отрицательный результат без контроля — «не нашёл» ≠ «нет»), письмо brain'а 01.09 «ваш чистый grep-аудит может быть слепым» (G286–G288).

## M116 — «уходит в error, чтобы выйти из вечного цикла» не работает: строку воскрешает соседняя функция

- **Симптом:** сервер сутками пишет `sync dependency rows skipped` про ОДНИ И ТЕ ЖЕ строки одного клиента, каждые 40 секунд. На клиенте — ни ошибки на экране, ни записи в диагностике. Работа оператора (у нас — четыре листа деталей двигателя за 20.08–31.08) не доезжает до сервера и не видна нигде, кроме чужого счётчика.
- **Корень:** лечение было написано (`planDependencyRequeue` → `markPendingError`), но двадцатью строками ниже в том же файле `recoverErroredRows` возвращает в `pending` любую `error`-строку, которая **проходит по схеме**. А сломана не схема — сломано отсутствие родителя. Цикл замыкается: push → skip → error → recover → push.
- **Почему держалось:** обе функции по отдельности правильны и обе покрыты смыслом («не терять строки из-за разовой невалидности» против «не гонять безнадёжные вечно»). Тест был на первую, вторая её отменяла — и ни один гейт не спрашивает «а не отменяет ли соседний код это лечение».
- **Побочный эффект, дороже самого цикла:** журнал пишется ДО применения (`syncWriteService`: `signAndAppendDetailed` → `applyPushBatch`), поэтому в журнал ложится КАЖДАЯ отвергнутая попытка. 29 таких строк дали 65 069 записей — 5,6% журнала (861 МБ), +6 000 записей в сутки от одного клиента.
- **Диагностика:** `journalctl -u matricarmz-backend-primary --since '24 hours ago' | grep 'sync dependency rows skipped'` → свернуть по `table|dependency|user` и сравнить **сумму `missing`** с числом различных `row_ids`. Если различных единицы, а сумма тысячи — это цикл, а не потеря данных. Проверка «строка вообще доезжала?»: `select count(*) from ledger_tx_index where row_id=…` против наличия строки в проекции.
- **Лечение:** карантин (`electron-app/src/main/services/sync/blockedRows.ts`) — состояние, из которого `recoverErroredRows` не воскрешает; выход один — зависимость появилась локально либо строка перестала быть `error`. Плюс доклад на сервер один раз на строку (`client.sync.blocked_rows`), чтобы застрявшая работа была видна на своей машине, а не только по чужому симптому.
- **Поймано:** 2026-09-08 разбором ночного алерта. Родня: M103 (отсутствие улики — не улика), M52 (строка `synced` без подтверждения сервера).

## M117 — гейт считает попытки, а называет их строками: один клиент держит статус `critical` вечно

- **Симптом:** здоровье конвейера синхронизации — `critical`, причина «skipped dependency rows 24h=10076». Отставаний нет, `23505` нет, парк работает. Ощущение массовой потери данных, которой не было: различных строк — восемь.
- **Корень:** снимок пропусков писал только СЧЁТЧИК (`count`) без id, и порог (`> 500 за сутки`) сравнивался с суммой попыток. Клиент, повторяющий 8 строк каждые 40 секунд, даёт 6 264 «пропущенных строки» в сутки. Порог перекрыт в 12 раз навсегда — а вместе с ним перекрыт и сигнал: за те же сутки 220 из 224 критических событий были этим же циклом.
- **Общая форма:** величина, по которой стоит порог, обязана отвечать на вопрос, которым порог назван. «Сколько данных не доехало» — это различные строки; «крутится ли цикл» — это попытки. Одно число не отвечает на оба вопроса, и подмена не видна, пока цикла нет.
- **Лечение:** снимок несёт `rowIds` (с потолком), свёртка считает различные строки (`foldSkippedRowsSnapshots`), попытки остаются отдельной величиной и печатаются в причине как `attempts=N`. Старые снимки без id считаются попытками — иначе сутки после выката гейт слепнет.
- **Поймано:** 2026-09-08. Родня: brain #284 «гейт без области» (порог, условие которого не может стать ложным), M113 (итог, не покрывающий свои части).

## M118 — сравнение «список тот же» по подписи, в которую забыли добавить новое поле

- **Симптом:** оператор записывает событие в карточке, список перечитывается — и показывает старое. Кнопка общего обновления не помогает, перезаход в раздел не помогает; помогает только перезапуск. При этом мост отдаёт правильные данные: `engines.list()` в консоли содержит новое поле, а на экране его нет.
- **Корень:** `refreshEngines` не заменяет состояние вслепую — сначала спрашивает `sameEngineList`, а та сравнивает строки по `engineRowSignature`. Подпись перечисляет поля **поимённо**, и новые (`hasDefectAct`, `defectDate`, `workshopId`, `lastHistoryAction`, `lastHistoryAt`) в неё не попали. Свежий список признавался тем же самым и **отбрасывался**.
- **Почему держалось:** поля, добавленные раньше, менялись вместе с самой сущностью, и `updatedAt` в подписи их выдавал. Новые меняются **без правки двигателя**: акт — это операция, история ремонта и переезд в цех — тоже. `updatedAt` двигателя при этом не двигается, и подпись честно совпадает.
- **Чем опасно:** ни типы, ни линт, ни юнит-тесты этого не видят — данные правильные на всём пути, теряются они в оптимизации сравнения. Симптом выглядит как «фильтр не работает», хотя фильтр исправен.
- **Диагностика:** сверить то, что отдаёт мост (`await window.matrica.engines.list()`), с тем, что на экране. Разошлись — виновата подпись, а не источник.
- **Лечение:** новое поле строки списка добавляется в `engineRowSignature` (`App.tsx`) в том же шаге, что и в саму строку. Сторож — `EngineRepairHistoryPanel.guard.test.ts`.
- **Поймано:** 2026-09-08 CDP-смоуком на dev-стенде; 2574 юнит-теста были зелёными. Родня: **M116** (лечение, отменённое соседней функцией) — тот же класс «две правильные по отдельности половины, вторая молча гасит первую»; brain #299.

## M119 — гейт прав возвращает УСПЕХ, а его читают как «отказ»

- **Симптом:** новый IPC-канал отвечает `{ ok: true }` на любой запрос — и на несуществующий идентификатор, и на пустой аргумент. Ошибок нет, типы сходятся, тесты зелёные; просто в базе ничего не появляется, а список всегда пуст. На экране — «комнат нет», хотя комната только что создана.
- **Корень:** `requirePermOrResult(ctx, 'chat.use')` возвращает `{ ok: true }`, когда право ЕСТЬ, и `{ ok: false, error }`, когда его нет. Написано было `const denied = requireChatUse(); if (denied) return denied;` — то есть возвращался успех гейта вместо результата операции. Хуже: функция асинхронная, `await` забыт, и `denied` — это Promise, всегда истинный. Обработчик до сервиса не доходил НИКОГДА.
- **Почему не поймали раньше:** возвращаемый тип обработчика — объединение, в которое `{ ok: true }` укладывается; `typecheck` доволен. Сторожа по исходному тексту проверяют наличие канала, а не то, что он делает. Юнит-тесты сервиса зелёные — сервис исправен, до него просто не доезжают.
- **Как отличить от «не зарегистрирован»:** незарегистрированный канал даёт исключение в `invoke`. Ровный `{ ok: true }` без полезной нагрузки — признак именно этого класса.
- **Лечение:** единственная форма вызова в проекте — `const gate = await requireChatUse(); if (!gate.ok) return gate;`. Имя переменной `denied` для помощника, возвращающего успех, — половина ошибки.
- **Поймано:** 2026-09-08 CDP-смоуком (мост отвечал `{"ok":true}` без `rooms`). Родня: M116/M118 — «две половины по отдельности правильные, стык молчит».

## M120 — замер ширины в скрытой вкладке и признак, потерянный горячей перезагрузкой

Одна и та же правка (ряд кнопок в одну строку) принесла два независимых промаха; оба видны только в живом окне.

- **Нуль вместо ширины.** Закрытые вкладки остаются в DOM скрытыми, и `getBoundingClientRect()` в них даёт нули. Кэш «ширины на состав ряда» пишется один раз, поэтому замер, случившийся до показа вкладки, навсегда объявлял ряд шириной 0 — переполнение не срабатывало ни при какой ширине окна. Лечение: пока `row.clientWidth <= 0`, замер не запоминается; эффект перезапустится, когда ряд появится на экране.
- **`child.type === Component` не переживает Fast Refresh.** Признак «не убирать в меню» определялся сравнением типа обёртки. После горячей перезагрузки React подменяет функцию, сравнение перестаёт совпадать — и в меню уезжает всё подряд, включая поле поиска. То же будет и от минификации имён. Лечение: признак ставится РАЗМЕТКОЙ (`data-toolbar-pin` + `display: contents`) и читается из DOM при замере.
- **Следствие для смоуков:** любой запрос вида `document.querySelector(...)` на этом экране может попасть в копию со скрытой вкладки — щелчок «сработает» и ничего не сделает. Все запросы драйвера фильтруются по видимости (`getClientRects().length > 0`), а тулбар и панель ищутся ОТ таблицы своего списка: на экране бывает две панели с двумя списками сразу.
- **Поймано:** 2026-09-08 на dev-стенде; 2654 юнит-теста и сторожа по исходному тексту были зелёными.

## M121 — счётчик без следа в журнале, и дофиксовые снимки, врущие ещё сутки после лечения

Одна тревога 09.09.2026 (`fatal`, «8194 пропущенных dependency-строки из 8258 попыток, ~99% неудач») стоила часа разбора и не содержала ни одного верного числа. Замер: пропущено **8 различных строк**, повторённых 18 838 раз, и все — до 08.09 14:02.

- **Дофиксовые снимки врут ещё целое окно.** Лечение M117 (считать различные `rowIds`, а не попытки) выкачено 08.09 13:54. Снимки, записанные ДО него, `rowIds` не несут, и `foldSkippedRowsSnapshots` осознанно засчитывает их `count` как строки — иначе первые сутки после выката гейт видел бы ноль там, где пропуски есть. Следствие, которое никто не назвал вслух: **сутки после лечения гейт продолжает выдавать старую ложь**, и выглядит она как новая беда. Проверять лечение счётчика — не раньше, чем окно целиком состоит из снимков нового формата (`jsonb_array_length(e->'rowIds') > 0`).
- **Счётчик без записи в журнал.** У шести путей пропуска рядом с `addSkipMetric('dependency', …)` стоял `logSkip`, у чата и общих заметок — не стоял. Такой пропуск виден только числом в тревоге: `grep` по журналу даёт ноль, и разбор уходит доказывать, что число не выдумано, вместо того чтобы искать строки. Сторож — `backend-api/src/tests/syncSkipLogging.guard.test.ts` (проверен на дофиксовом файле: ловит ровно два места).
- **AI-пересказ добавил своё.** «8258 попыток» и «~99% неудач» — трактовка отчёта, а не измерение: обе величины считают ПРОПУСКИ, доли отказов в данных нет вовсе. Третий случай подряд, когда число из AI-отчёта приняли за замер (06.09, 08.09, 09.09) — цитируемую человеку величину проверяем вопросом «отвечает ли она ровно на то, чем названа».
- **Поймано:** 2026-09-09, тревога прода. Родня: **M117** (тот же счётчик, прошлый слой лжи), **M103** (тишина в своём логе — не улика).

## M122 — отложенное создание родителя + автосохранение ребёнка: ребёнок раньше родителя, сироты и двойники

- **Симптом:** `client.sync.blocked_rows` («критично») на каждый новый двигатель, на сервере — `sync dependency rows skipped` для `operations → engine_entity`; через 5–40 секунд всё сходится само. За утро 10.09 — 22 доклада на 22 новых двигателя, ни одной потерянной строки. Заодно у двигателей по два списка деталей, созданных с разницей в 12 мс: один пустой, второй заполняется.
- **Корень, половина первая:** `createEngine` выдаёт id без строки в базе (deferred create, Phase 2) — строка появляется с первым записанным полем. Панель списка деталей при открытии сама сохраняет автозаполненный лист — и операция ссылается на двигатель, которого нет ни локально, ни на сервере. Клиент честно отвечает «зависимости нет и у меня», кладёт строку в карантин и докладывает. Брошенная карточка оставляет лист-сироту навсегда.
- **Корень, половина вторая:** `save()` берёт `operationId` из замыкания React, два автосохранения подряд оба видят `null`, а `saveRepairChecklistForEngine` без номера листа всегда вставлял новый. На 10.09 двигателей с двумя живыми листами — 206, из них 133 пары созданы в пределах 5 секунд; в сентябре так созданы все новые пары.
- **Почему держалось:** отложенное создание и автосохранение по отдельности правильны, стык виден только в синхронизации, и то чужим симптомом. Пункт «PC19 держит единственную копию работы» (09.09) стоял на этом же классе: восемь «строк работы» оказались четырьмя брошенными карточками по два пустых листа.
- **Диагностика:** для каждой строки из пропуска — есть ли двигатель на сервере сейчас и когда он создан (клиентский `created_at`) против `created_at` листа. Двигатель моложе листа на секунды — этот класс. Содержимое отвергнутых строк лежит в `ledger_tx_index` (журнал пишется до применения, M116): «единственная копия на машине» проверяется там, а не догадкой.
- **Лечение (3.25):** main откладывает автозаполненный лист, пока нет строки двигателя (`auto: true` → ответ `deferred`), правка оператора сначала создаёт двигатель (`ensureEngineRow`); без номера листа сервис берёт существующий лист двигателя. Панель дописывает отложенный лист, когда карточка сохранила двигатель, и при закрытии карточки. Сторожа — `checklistService.test.ts`, `RepairChecklistPanel.guard.test.ts`.
- **Общая форма:** всё, что пишется САМО под id отложенно создаваемого родителя, обязано ждать родителя; ручная правка ребёнка — уже содержимое, она родителя материализует.
- **Поймано:** 2026-09-10, разбор тревоги прода. Родня: **M116** (тот же PC19, прошлый слой), **M121** (число, не отвечающее на свой вопрос).

## M123 — префиксный гейт раздела захватил запись телеметрии вместе с чтением журнала

- **Симптом:** у оператора вне «Администрирования» в логе сервера на каждый визит вкладки — `unhandledrejection: … 'audit:add': Нет доступа к разделу «Администрирование»`. В журнале действий телеметрия за неделю есть у пятерых из десяти активных пользователей.
- **Корень:** в `sectionGate.ts` правило `['audit:', 'administration']` — самый длинный префикс отдал разделу весь namespace, а `audit:add` пишет каждый (`uiUsageLog`). `void promise` без `.catch` превращал отказ в ошибку лога.
- **Почему держалось:** гейт fail-open для незасеянных и суперадмина — владелец и тестовые стенды отказа не видят; телеметрия best-effort, её пропажа ничего не ломает на экране. С 3.1.0 (02.07) по 10.09.
- **Лечение:** правило сужено до `audit:list`; `.catch` в `uiUsageLog`. Сторож — `sectionGatePrefix.test.ts`.
- **Общая форма:** префиксное правило гейта годится, только если ВСЕ каналы namespace принадлежат одному разделу. Сквозной канал (запись телеметрии, lookup из чужих карточек) в таком namespace закрывается у всех, кто в раздел не входит.
- **Поймано:** 2026-09-10, обзор клиентских логов за двое суток.
