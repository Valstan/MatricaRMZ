...
## M128 — подключение скрипта точкой исполняет его `param()` и молча затирает переменные вызывающего
...
## M129 — `,$array` из функции PowerShell не разворачивается в конвейере, и отчёт печатается одной строкой
