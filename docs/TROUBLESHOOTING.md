...
## 1.3) Временные сетевые/БД сбои: как теперь ведёт себя sync/auth

### Что изменилось в устойчивости
- Клиент больше не сбрасывает локальную сессию при временном провале `POST /auth/refresh` (например `500`, timeout, сетевой сбой).
- Сессия очищается только при подтверждённом auth-отклонении (`401/403`).
- При восстановлении сети клиент запускает sync сразу (без ожидания полного backoff-окна).
- Backend для `POST /auth/refresh` делает повторную попытку при транзиентной DB-ошибке подключения/таймаута.

### Как интерпретировать логи после обновления
- `sync failed: Error: offline` — запрос не ушёл из клиента (локально офлайн по `net.isOnline`/probe), это не обязательно падение backend.
- `auth refresh failed` на сервере — обычно временная проблема доступа backend к PostgreSQL; после единичного пика может восстановиться без разлогина клиента.
- `sync blocked: auth required` теперь должен появляться в основном при реальной потере валидной сессии (просроченный/отозванный refresh, `401/403`).

### Быстрый чеклист
1. Проверить сеть с рабочего места: `curl.exe -I http://<domain>/health`.
2. Проверить backend health на VPS: `curl -sS http://127.0.0.1:3001/health`.
3. Если в серверных логах повторяется `auth refresh failed` с timeout/connect — проверить доступность PostgreSQL и pool (`PGPOOL_*`).
4. Если клиент стабильно пишет `auth required`, проверить ответы `/auth/refresh` (ожидаемо искать `401/403`, а не только `500`).

---

## 1.3.1) `TypeError: The database connection is not open` после schema mismatch

### Симптом
В Electron на экране входа или сразу после старта:
- `Ошибка: TypeError: The database connection is not open`

### Причина
Обычно это не падение прод-сервера, а клиентский сценарий:
- сервер отдал новый schema hash;
- клиент решил пересобрать локальную SQLite-базу (`schema_mismatch`);
- старая база была закрыта;
- старый клиент продолжил работать без полного перезапуска и попытался снова обратиться к уже закрытому соединению.

### Актуальное поведение
- Начиная с исправленного клиента после локального DB reset по `schema_mismatch` приложение должно автоматически выполнить `relaunch` и открыть уже свежую базу.

### Что проверить
1. `GET /health` с рабочего места и на VPS, чтобы исключить реальный серверный инцидент.
2. Логи клиента: искать рядом `schema rebuild`, `local db reset requested reason=schema_mismatch`, `sqlite.close()`.
3. Версию клиента: если используется релиз до фикса, проблема может повторяться до обновления клиента.

### Практический вывод
- Если `/health` отвечает, а в клиентском логе виден `schema_mismatch`, сначала подозревать устаревший Windows-клиент, а не недоступность backend.
...
- Имя и пароль пользователя БД: в `backend-api/.env` (`PGUSER`, `PGPASSWORD`). Этот же файл подхватывается systemd через `EnvironmentFile`. См. `docs/OPERATIONS.md` (раздел ключевых ENV).
...
Описание путей, файлов и механизмов логирования находится в `docs/OPERATIONS.md`
