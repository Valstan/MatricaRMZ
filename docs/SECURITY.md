...
- **`MATRICA_JWT_SECRET` живёт в ДВУХ файлах — менять синхронно.** Сервисы читают systemd `EnvironmentFile` (`/etc/matricarmz/matricarmz.env`) — он авторитетен для рантайма; `backend-api/.env` читают только CLI-скрипты (dotenv не перекрывает переменные, уже выставленные systemd). Ротация только в `.env` для сервисов невидима: свежевыпущенный токен получает `token_invalid`, а старые продолжают работать (инцидент 2026-08-28, GOTCHAS M96). Порядок: бэкап обоих файлов (600) → одинаковый новый секрет в оба → rolling-рестарт с `/health` → перевыпуск release-токена → `/auth/me`.
...
1. **DNS хостера режет Telegram CDN.** Из проверенных IP `api.telegram.org` с прод-сервера достижим один; он закреплён в `/etc/hosts` с комментарием `# MatricaRMZ: pinned working CDN IP` (значение — только на сервере, в репо не пишем). Если этот IP станет недоступен — повторить проверку диапазона и обновить запись.
   > Уточнение 04.09.2026 (GOTCHAS **M101**): «достижим один» было артефактом одиночных проверок. Сеть хостера теряет около половины TCP-SYN ко **всем** диапазонам Telegram, ICMP и остальной интернет целы, установленное соединение живёт нормально. Поэтому все отправители (backend `telegramBotService`, ops-скрипты `backup-encrypted` / `watch-failed-auth` / `audit-deps`) ходят с коротким таймаутом попытки и повторами; пин в `/etc/hosts` не вреден, но и не лечит.
