...
2. ✅ (21.09, G1) Чтение: `getRepairChecklistForEngine` собирает `answers.engine_inventory_items.rows` из реплики таблицы, если у листа там есть живые строки; иначе — из `meta_json`. Хелпер — `electron-app/src/main/services/engineInventoryLinesReplica.ts` (батч по листам, ключ `operation_id`, чанки IN по M66; сбой чтения реплики = fallback на `meta_json` с одним предупреждением — до E3 это честно, после E3 обязано стать ошибкой). Переведены `engineService` (флаги) и оба пресета «Двигатели»; **поиск по клейму (`cardContentSearchService`) остался на `meta_json`** — его префильтр по тексту, перевод вместе с E3. Кэш флагов ключуется `updated_at` листа — после E3 ключ обязан учитывать и строки.
3. ✅ (21.09, G2) Запись: панель сохраняет как раньше (весь `answers`), а `saveRepairChecklistForEngine` на клиенте (а) пишет строки в реплику с `sync_status='pending'` (`writeInventoryLinesForSheet`: diff по `line_key`, изменённые/новые/удалённые → тумстоуны; id — `engineInventoryLineId` из shared, общий с сервером), (б) **пока** продолжает писать `rows` и в `meta_json`. Push — своя секция в `syncService` (лимит 1500 строк на пуш, чтобы лист и его строки уезжали вместе), `markAllSynced`, recovery. **Не сделано осознанно:** защита pending-строк от перезаписи pull'ом — её нет и у `operations`, пуш идёт раньше pull'а в том же цикле, класс общий.
