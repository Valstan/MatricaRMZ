...
- UFW активен, default DROP; наружу только нестандартный SSH-порт, 80/443 и торрент-порты.
...
- 2026-06-26: **H8 — ✅ ЗАКРЫТО.** При выполнении выяснилось, что утечка шире, чем «подписной ключ»: репо был **публичным**, в HEAD лежал `server-key.json` (прод подписывал им же — sha совпал), в **истории** — `data-key.json` + `state.json` (2026-01-27→2026-06-07, #252 убрал из трекинга, не из истории), 119 блоков с **552 `enc:v1:`** полями реальных ПДн (chat_messages/attribute_values сотрудников; data-key + шифротекст оба были публичны → расшифровываемы). Решение: live-ledger **вынесен из git-checkout** (`MATRICA_LEDGER_DIR=~/matricarmz-ledger`, `mv` 341k блоков — root-cause); подписной ключ **ротирован** (старый retired, бэкап); data-key уже на не-утёкшем `k-mq3wacgz` (майская ротация); ledger untracked из HEAD (#614); **репозиторий сделан приватным** (forks/stars/watchers=0 → закрывает доступ к истории). Прод healthy, новый ключ активен. **Найденный капкан:** live-ledger трекался внутри checkout'а → наивный `git rm`/`reset --hard` удалил бы живые прод-данные; обязателен relocate+бэкап до любых git-операций (→ GOTCHAS). **Опционально/low:** полная вычистка истории `filter-repo`+force-push (1276 коммитов/579 тегов, реклон всех машин) — деприоритизировано (репо приватный + ключ ротирован).
- 2026-06-30: **Доаудит ротации ledger-ключей (next-step H8) — ✅ активные ключи чисты, ротация не нужна.** После того как в эту сессию репо снова сделали публичным (fresh-snapshot, история с ключами → приватный `MatricaRMZ-archive`), сверили **живые прод-ключи** с тем, что реально лежало в трекаемой истории архива (печать только id/отпечатков, без секретов). **Подписной** `server-key.json`: историч. pub-отпечаток `1563d94f…` (Jan 27 → untrack #614) ≠ живой `d2b91b5e…` → ротирован H8, утёкший выведен. **Data-key**: в истории трекалась только **legacy** `{keyBase64}` (=`v1-legacy`, удержан для replay `enc:v1`, НЕ активен); активный `k-mq3wacgz-6d7aa9` создан 2026-06-07 17:46 MSK — на 11 мин **позже** untrack #252 (14:34Z) → в трекинг не попадал. Живой backend (PID, env `/etc/matricarmz/matricarmz.env`) подтверждённо читает `MATRICA_LEDGER_DIR=~/matricarmz-ledger` (релокейт H8 цел); ни один live-ключ не в архиве и не в публичном snapshot. **Вердикт: H8 полон, добивать нечего.** Осознанная граница (необратимо): `v1-legacy` навсегда в keyring для replay, историч.-публичные данные экспонированы навсегда — защищается только будущее. **Побочно убран** orphan-ledger в git-checkout `backend-api/ledger/` на проде (150 МБ, gitignored, живым процессом не использовался; пересоздавался деплоем при cwd=`backend-api` без `MATRICA_LEDGER_DIR` → `DEFAULT_LEDGER_DIR=cwd/ledger`). PENDING §6 закрыт → COMPLETED.
