...
**Решения владельца (2026-08-19):** баг первым; «Рабочий стол» = стартовый экран после входа; отчёты по двигателям объединить + убрать мёртвый; доступ к журналу — выдаваемое право (сразу выдать названному сотруднику).
...
| 5 | Рабочий стол + чат одним экраном (папки, корзина, подвижные границы) | 4, 10 |
...
> **Этап 1 закрыт кодом 2026-08-19** (PR [#632](https://github.com/Valstan/MatricaRMZ/pull/632), [#633](https://github.com/Valstan/MatricaRMZ/pull/633), [#634](https://github.com/Valstan/MatricaRMZ/pull/634), [#635](https://github.com/Valstan/MatricaRMZ/pull/635), [#636](https://github.com/Valstan/MatricaRMZ/pull/636)) — выпущены релизом **v3.5.0** и раскатаны на прод 2026-08-19. Ниже исходный план этапа сохранён как история решений.
...
> **Этап 3 закрыт кодом 2026-08-19** ([#641](https://github.com/Valstan/MatricaRMZ/pull/641)): единый `AttachmentsModule` — общая выборка фото и списка, один тулбар (объединение прежних наборов) + «Сохранить все (N)»; карточка двигателя наконец передаёт `scope`. Дочерние компоненты приняли управляемую выборку, остальные места использования не тронуты.
...
> **Этап 4 закрыт кодом 2026-08-20** ([#642](https://github.com/Valstan/MatricaRMZ/pull/642)): право `audit.view` (выдаваемое; у супер-админа по умолчанию, с ролью admin НЕ приезжает), `ui.card_open` пишет entityId (кто какую карточку смотрел), endpoint `/admin/audit/document`, панель «История изменений этого документа» в карточках двигателя и контракта. **Остались хвосты этапа:** запись правок для остальных типов карточек (сейчас развёрнуто у двигателя и заявки снабжения) и `ui.report_build` с картой фильтров — он же сырьё для этапа 7.
...
2. **Право** `audit.view` (superadmin всегда; выдать названному владельцем сотруднику при релизе; выдаётся из админки как остальные права).
...
## Этап 5. Рабочий стол — единый экран с чатом (правка владельца 2026-08-19)

> **Этап 5 закрыт кодом 2026-08-20**: вкладка чата стала экраном **«Рабочий стол»** (id `chat` сохранён — сессии/deep-link'и живы), открывается с фокусом после входа; чат слева (⅓), стол справа, **обе границы тянутся** и роумятся; ярлыки (кнопка «На рабочий стол» в МЕНЮ→Сервис), папки с окном ¾, корзина (drag туда/обратно, ПКМ→«Очистить» с подтверждением), удаление папки уводит содержимое в корзину. Хранение — секция `desktop` в `ui_profile_json` (LWW секцией), sanitize терпит незнакомые link'и. CDP-смоук 10/10 + persist-проба (ярлык и chatPct пережили reload через серверный профиль). Отступление от плана: перетаскивание ярлыков между позициями стола не делалось (порядок = порядок добавления) — HTML5 DnD покрывает корзину/папки/возврат.

**Чат и Рабочий стол — одно целое, а не две вкладки.** Один экран, разделённый на две зоны:

```
┌───────────────────────────────┬──────────────────────────────────────────┐
│ ЧАТ (по умолчанию ⅓ ширины)   │ РАБОЧИЙ СТОЛ                             │
│ ┌──────────┬────────────────┐ │   [ярлык] [ярлык] [папка]                │
│ │участники │  переписка     │ │   [ярлык] [ярлык]                        │
│ │  ↕ гран. │                │ │                                          │
│ └──────────┴────────────────┘ │                              🗑 Корзина  │
└───────────────────────────────┴──────────────────────────────────────────┘
                                ↕ обе границы тянутся мышкой
```

1. **Экран**: singleton-вкладка `desktop` (паттерн `chat`/`settings` в `tabsModel.ts`), **открывается первой после входа**. Кнопка/вкладка «Чат» ведёт на этот же экран — отдельной вкладки чата больше нет, чат живёт в левой зоне в нынешнем виде (колонка собеседников + лента + композер из этапа 2).
2. **Две подвижные границы**, обе тянутся мышкой: (а) чат ↔ рабочий стол (дефолт ⅓ / ⅔); (б) внутри чата — участники ↔ переписка. Проценты хранятся в roaming-настройках пользователя (`shellPrefs`-канал этапа 1) — раскладка едет за человеком, как ширины сплитов v3.
3. **Ярлык** = `{id, appLink: ChatDeepLinkPayload, label, icon, folderId: string|null, deletedAt: number|null}`. Реюз меток `TAB_SHORTCUT_META` (`HistoryPage.tsx:85-112`) и `buildChatBreadcrumbs`. Кнопка «На рабочий стол» рядом с «Ссылка в заметки» (`App.tsx`, образец `saveCurrentPositionToNotes`) + в карточках/списках/отчётах + из МЕНЮ.
4. **Корзина** — постоянный элемент рабочего стола (правый нижний угол). Ярлык можно **перетащить мышкой в корзину**; удаление ярлыка через контекстное меню тоже кладёт его **сначала в корзину**, не стирая. ПКМ по корзине → «Очистить корзину» (после подтверждения — окончательное удаление всего содержимого). Из корзины ярлык можно вернуть перетаскиванием обратно на стол.
5. **Папки**: создаются на рабочем столе (ПКМ по свободному месту → «Создать папку», имя правится по месту). Ярлыки складываются в папку перетаскиванием. **Папка открывается окном поверх рабочего стола**, по центру, размером ≈¾ зоны рабочего стола, с кнопкой «Закрыть». Из открытой папки ярлык перетаскивается **обратно на стол**. ПКМ по папке → «Удалить папку» — вместе с содержимым (содержимое уезжает в корзину, сама папка исчезает; подтверждение обязательно).
6. **Хранение** — ключ `desktop` в `ui_profile_json` (merge-LWW этапа 1 обязателен): `{shortcuts[], folders[], trash[], layout{chatPct, peoplePct}}`. Синк между машинами бесплатно. Толерантность к неизвестным id — как у пинов меню: неизвестное храним, не рендерим (id-чурн релизов не должен стирать чужой рабочий стол).
7. **Гейты приёмки**: перетаскивание ярлыка в корзину и обратно; очистка корзины через ПКМ; создание папки, укладка ярлыка, открытие окна папки, возврат ярлыка на стол, удаление папки с содержимым; обе границы тянутся и переживают перезапуск и переход на другой компьютер.

## Этап 6. Отчёты по двигателям + фильтры

> **Пункты 1–2 этапа закрыты кодом 2026-08-20**: `engines_list` + `engines_contracts_overview` слиты в один пресет **`engines` («Двигатели»)** — суперсет фильтров (periodBasis получил «по дате создания», диапазоны дат прихода/ремонта/отгрузки, трёхпозиционный «Утиль», «Начат ремонт», «Акт комплектности») и колонок детального разреза; groupBy как был. Старые id — **алиасы** (`REPORT_PRESET_ALIASES`): сохранённые ссылки/вкладки, избранное, история и шаблоны фильтров резолвятся в новый (шаблоны обеих корзин показываются вместе). Каталог показывает один пресет; на «Двигателях» одна кнопка «Отчёт „Двигатели“». **`engine_movements` удалён** (каталог + диспетчер + источник «Моих отчётов»); **`engine_stages` починен** — двигатели без единой операции больше не выпадают (стадия из статусов карточки, на стенде 0 → 1602 строки). **Пункт 3 закрыт следом (2026-08-20)**: строка «Выбрано: …» под контролами (селекты, диапазоны дат; дефолтный «последний месяц» не считается отклонением), блок «Применённые фильтры» над результатом (только отклонения от дефолта), шаблоны фильтров зеркалируются в секцию `reportFilterTemplates` профиля (export/import-мост main↔профиль, LWW секцией) — едут за пользователем. **Этап 6 закрыт целиком.**
