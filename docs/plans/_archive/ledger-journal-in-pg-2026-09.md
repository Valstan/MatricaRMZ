...
**Статус:** DONE (2026-09-12). J1 слит (#788, #789), J2 выкачен 06.09, шаг 7 выполнен 12.09: постоянная копия последнего архива с деревом цепочки — Я.Диск `/matricarmz/ledger-final/matricarmz-backup-20260912-001701-LAST-WITH-LEDGER-TREE.tar.zst.gpg` (1 876 758 272 байт, sha256 `339e9d53…` — совпал с проверенным ночным архивом), каталог `~/matricarmz-ledger` удалён (диск 60 % → 35 %), `MATRICA_LEDGER_DIR` снят из обоих env (бэкапы `*.env.bak-20260912-114735`), приёмочный прогон бэкапа — `verified: db.dump only listed`. Хвосты J3 — в `PENDING_FOLLOWUPS.md`.

**Прежний статус:** ACTIVE (2026-09-05). Решение владельца 05.09 (вечер): убрать цепочку блоков из пути записи. Истина — PostgreSQL (вариант А от 05.09 утра), история — журнал в PG (`ledger_tx_index`, он уже есть и уже полный), нумерация seq — последовательность PG. Подписи, шифрование строк на диске сервера, проекция `state.json`, чекпоинты и блоки — уходят. «Нормальный обмен через реестр» если понадобится — в Матрице 4 с чистого листа.
