...
**Статус: ЗАКРЫТ 2026-07-25.** Ф0 ✅ · Ф1 ✅ (v2026.712.1818) · Ф2 ✅ целиком: `reconcile-code-name` 2026-07-12, `blank-synthetic-codes --apply` 2026-07-25 (blanked=145+3, идемпотентно 0, гейт клиентов обойдён `--skip-client-gate` по решению владельца; force_full_pull 25 клиентам, 5 старых машин — переустановка). Хвосты данных — PENDING_FOLLOWUPS §де-дупликация. Ф3 — опционально, не начата.
...
2. ✅ **`reconcile-code-name --apply` выполнен на проде 2026-07-12** (по подтверждению владельца): names=13, codes=0, collisions skipped=1 (картер). Контрольный dry после: NAME fixes = 0 (идемпотентно); остаток 96 расхождений — чисто кодовые (DET-строки, уйдут шагом 3). Бэкап: `~/backup-reconcile-code-name-20260712.sql` (2.2M). `/health` после — ok.
3. ✅ **`blank-synthetic-codes --apply` выполнен на проде 2026-07-25** (по решению владельца — без ожидания раската, `--skip-client-gate`): blanked=145 зеркал + 3 карточки, promoted=0, ghosts=0, SUSPICIOUS=5 (живые SRV-коды услуг, подтверждены `--allow-suspicious=5`). Повторный dry = 0 (идемпотентно). Бэкап `~/backup-blank-synth-20260725.sql`. Доставка: force_full_pull проставлен 25 клиентам ≥ v2026.712.1818 напрямую в `client_settings` (admin-роут ловит 504 nginx: снапшот на каждый вызов); 5 машин ниже порога — без команды, ждут переустановки.
