...
**Схема ключей (директива brain `2026-08-19-three-zero-cost-fixes-before-the-onprem-move`).** На сервере лежит **только публичный** ключ (`BACKUP_ENCRYPTION_PUBLIC_KEY_FILE=/opt/matricarmz/secrets/backup-public.pem`): сервер умеет зашифровать и не умеет расшифровать. Приватная половина — **вне сервера**, и где именно она сейчас — **не подтверждено** (см. `PENDING_FOLLOWUPS` §«Расшифровка ночного бэкапа не проверена»). Сверено 2026-09-22: на боксе лежит только `backup-public.pem`, приватного ключа там нет нигде (так и задумано); на `PC79` в `%USERPROFILE%\.matricarmz-keys\` его тоже нет — там только Android-keystore и его пароль. Прежняя запись «на PC40» устарела: машина упразднена 2026-09, каталог стёрт вместе с ней. Файл `/etc/matricarmz/backup.passphrase` (45 байт) — это **пароль**, а не ключ, и приватную половину он не заменяет. Украденный `YANDEX_DISK_TOKEN` без приватного ключа даёт только шифротекст.
