...
**Схема ключей (директива brain `2026-08-19-three-zero-cost-fixes-before-the-onprem-move`).** На сервере лежит **только публичный** ключ (`BACKUP_ENCRYPTION_PUBLIC_KEY_FILE=/opt/matricarmz/secrets/backup-public.pem`): сервер умеет зашифровать и не умеет расшифровать. Приватная половина — **вне сервера**, на `PC79` в `%USERPROFILE%\.matricarmz-keys\backup-private.pem`.

> **Пара перевыпущена 2026-09-22 — прежняя была утрачена.** Приватный ключ от 19.08 жил на `PC40`; машину владелец вычистил 13.09, каталог ключей исчез вместе с ней, и это заметили только 22.09. То есть месяц ночные дампы исправно шифровались ключом, к которому не было половины: **дампы за 19.08–22.09 не восстановимы** (они уходят сами — хранение 10 дней). Проверять восстановимость надо расшифровкой, а не зелёным логом задания: лог был зелёным всё это время.
>
> **Новая пара проверена на настоящем дампе, а не на тесте:** RSA-4096 (OAEP-SHA256 + AES-256-GCM, как и прежде), публичная половина установлена на бокс 22.09, ночное задание запущено вручную, из полученного `2026-09-22.dump.enc` приватным ключом снята обёртка и первые байты дали `PGDMP`. Файл `/etc/matricarmz/backup.passphrase` (45 байт) — это **пароль** для другого назначения, приватную половину он не заменяет.
>
> ⚠️ **Копия ключа пока одна.** Пока приватная половина лежит только на `PC79`, мы стоим ровно там же, откуда пришли. Вторая и третья копии — на владельце: свёрток `node scripts/seal-key-bundle.mjs seal` (теперь он соберётся: все три участника на месте) на офлайн-носитель в сейф + копия вне здания, пароль отдельно от носителей. Украденный `YANDEX_DISK_TOKEN` без приватного ключа даёт только шифротекст.
