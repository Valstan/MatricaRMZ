...
## Склады (места хранения): один справочник и куда зайти в UI

### Откуда берётся список складов

- **Единый справочник для склада:** тип masterdata **`warehouse_ref`** (записи в общей модели `entityTypes` / `entities` / `attributeValues`, синхронизируются как остальные справочники).
- Все экраны склада (остатки, документы, номенклатура, фильтры) подтягивают склады **из одного агрегата** `lookups.warehouses` → это ровно активные записи `warehouse_ref` (плюс подписи из атрибутов `name` / `title` / `label` / `code` — см. `listMasterdataLookup('warehouse_ref')` в `warehouseService.ts`).
- **Не путать с `storage_zone`:** в справочниках есть отдельный тип **`storage_zone`** (зоны хранения внутри логики снабжения/склада). Это **не** дубль списка складов в `warehouse_ref`, но визуально оба сидят в группе «Снабжение и склад» в **Справочниках** — при уборке смотрите код типа сущности.
- В коде и старых данных часто встречается идентификатор **`default`** — это **не строка из справочника**, а **условный плейсхолдер склада** (например в черновиках документов и в отчётах, пока не выбран реальный склад). На сервере в ответ `GET /warehouse/lookups` в начало списка складов **подмешивается** синтетическая строка `{ id: 'default', label: 'Склад по умолчанию', code: null }` (`ensureDefaultWarehouse` в `warehouseService.ts`), если в данных ещё нет id `default`. Это **не** запись `warehouse_ref` с вашим UUID «Основной склад». Его нельзя удалить в «Справочниках»: это не сущность в БД. Для учёта по реальному складу в документе/номенклатуре выберите строку с **настоящим UUID** из `warehouse_ref`.

### Куда зайти, чтобы создавать и удалять склады

1. В Electron: раздел **«Контроль и аналитика»** → вкладка **«Справочники»** (`masterdata`).
2. Блок **«Снабжение и склад»** → тип сущности **`warehouse_ref`** (по названию типа в дереве, обычно про склады/склад).
3. Кнопка добавления записи ведёт на создание новой позиции справочника (подпись кнопки в UI: **«Добавить склад»**).
4. Удаление неиспользуемых складов: открыть карточку записи и воспользоваться удалением в **Справочниках** (нужны права **редактирования masterdata**; опасные действия могут требовать **расширенного режима** в этом экране — см. подсказки на форме). Перед удалением система проверяет ссылки с других сущностей.

Дополнительно: в **«Склад» → «Номенклатура»** в карточке позиции в полях «Склад по умолчанию» / серийные экземпляры есть быстрое создание через **«Новый склад»** — оно создаёт **ту же** сущность `warehouse_ref`, т.е. это второй вход в тот же справочник, а не отдельный источник.

### Почему кажется «несколько источников»

- Один физический справочник **`warehouse_ref`**, но **два UX-входа**: Справочники и quick-create из номенклатуры.
- Отдельно в интерфейсе фигурирует **`default`** и опция в отчётах — это **не дубль справочника**, а технический идентификатор по умолчанию.
- Устаревшие или тестовые записи `warehouse_ref` остаются в базе, пока их не удалить или не пометить удалёнными — они продолжают попадать в `lookups`, отсюда «лишние» склады.

### Уборка дублей

- В UI **Справочники → warehouse_ref**: найти дубли по названию/коду, оставить одну каноническую запись, лишние — удалить (если нет блокирующих ссылок) или не использовать в новых документах.
- Если дубли уже «разъехались» по остаткам/документам, смена склада в проведённых документах **не делается правкой справочника** — нужна осмысленная корректировка движений/остатков (отдельная учётная процедура). Для диагностики пересечений учёта см. отчёт **`warehouse_stock_path_audit`**.
...
- **Phase 2 Stage F (2026-06-02): зеркало деталь→номенклатура удалено.** Сняты `MATRICA_WAREHOUSE_PART_MIRROR_MODE`, `syncPartsToWarehouseNomenclature`, `refreshPartWarehouseNomenclatureLinks` — directory-first стал единственной моделью. `erp_nomenclature` + spec-колонки `directory_parts` — единственный источник истины для деталей. Карточка детали редактируется внутри карточки номенклатуры (embedded `PartDetailsPage`); `/parts/*` остаются как EAV-CRUD под ней (НЕ депрецированы — пересмотр отдельной ниткой). Новые orphan-строки (деталь без `erp_nomenclature`) чинятся `warehouse:backfill-orphan-part-nomenclature`.

### Инструмент: позиция ≠ экземпляр (2026-08-13)

**Граница, которую нельзя размывать:** номенклатура отвечает на вопрос «что это за инструмент»,
EAV-`tool` — «вот эта конкретная штука».

```
tool_catalog (наименование, EAV) → directory_tools → erp_nomenclature (позиция)
                                                            ↑
                                   tool (экземпляр, EAV) ────┘  через атрибут tool_catalog_id
```

- **Позиция** несёт то, что общее для всех единиц: артикул, группу, единицу измерения, поставщика,
  остатки, шаблон свойств, норматив (напр. период поверки).
- **Экземпляр** несёт то, что своё у каждой единицы: табельный и серийный номер, подразделение-
  владельца, даты поступления/снятия, фото, выдачу сотруднику (`operations` `tool_movement`),
  факт последней поверки.
- **Не класть экземплярные поля в `spec_json` позиции.** До 2026-08-13 было наоборот: миграция
  `0045` набила `directory_tools` **экземплярами**, и в номенклатуру уехали слепки конкретных
  единиц. Разбор и починка — [`plans/tools-catalog-unify-2026-08-13.md`](plans/tools-catalog-unify-2026-08-13.md).
- **Выдаётся экземпляр, а не наименование** («Снабжение → Учёт инструментов»): выдать «Шуруповерт»
  бессмысленно, если их пять. Товары, наоборот, выдаются позицией.
- Скрипты: `warehouse:migrate-tool-catalog` (сведение), `warehouse:cleanup-tool-mirrors` (чистка).

**`directoryRefName` в `nomenclatureUpsert`.** Клиент заводит карточку-источник **локально**
(sqlite + ledger-push), а позицию создаёт REST'ом — сервер источник ещё не видит и раньше отвечал
«Источник … не найден», оставляя карточку сиротой без позиции. Поэтому вместе с `directoryRefId`
шлём имя: сервер заводит строку `directory_*` сам, а приехавшая синхронизацией запись ложится
в неё по тому же id. Касается **всех** видов с карточкой-источником, не только инструмента.
