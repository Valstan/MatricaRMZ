...
| **Ведомости работ — узлы** | `services/workSheetTypeService.ts`, `routes/workSheetTypes.ts` (`/work-sheet-types`), миграция `0097`; клиент — `electron-app/src/main/ipc/register/workSheets.ts` (`workSheets:types:*`, REST через `httpAuthed`), кэш `ui/utils/workSheetTypesCache.ts` | Справочник видов ведомостей (укладка, вал, обкатка, сборка, …) с `columns_json` и `completes_repair`. Вне синка: строки ведомостей самоописываемы и живут в `operations`. Читать — `operations.view`, править — `erp.dictionary.edit`. Код узла после создания не меняется (на него ссылаются строки); удаление = архив |
...
| **Drizzle schema** | `backend-api/src/database/schema.ts` | Поля колонок, индексы, FK; миграции в `backend-api/drizzle/` (последняя — `0097`) |
...
| **Производство → Ведомости работ** | `pages/WorkSheetsPage.tsx` (CardTabs «Все» + узел, фасеты по колонкам узла, `ListCount`/`rowNumbers`), `components/WorkSheetRowDialog.tsx` (строка: двигатель, дата, цех, поля узла; id даёт клиент), `components/WorkSheetTypeEditorDialog.tsx` (узлы и их колонки), main — `services/workSheetService.ts` (upsert строки по id, `completesRepair` → `advanceEngineStatusForWorkOrder` + автозапись стадии датой строки), `operationService.ts` (`listOperationsByType`, `upsertOperation`, `softDeleteOperation`), IPC `workSheets:rows:*`; сторож — `pages/WorkSheetsPage.guard.test.ts` | Строка = запись истории ремонта (`operations`), правится только здесь; узел — REST-справочник (см. Backend). Панели вкладок смонтированы, скрыты `hidden` (M78). Правка/удаление строки статус ремонта не трогают |
...
| **Верстак / ярлыки** | `components/DesktopPane.tsx` (Верстак на экране чата: сетка, выделение, лассо, перенос), `shellV3/V3TabShell.tsx` (галстук `.v3-tab-desktop` на вкладке — тумблер ярлыка), `shellV2/ButtonPanel.tsx` (пункт «Добавить на Верстак», закреп = закреп + ярлык), `components/TieIcon.tsx`, `HistoryPage.tsx` («Мой круг» = те же ярлыки) | Одна модель закладок — секция `desktop` в `ui_profile_json`; легаси «Быстрый запуск» (`pinnedShortcuts`) только читается ради одноразового переезда (`desktopMigrateQuickStart`). Раскладку считает домен (`desktopLayoutGrid`), не вёрстка; координата хранится в ячейках и при сужении Верстака НЕ переписывается. Размер плитки — место в личном распределении (`desktopUsageSteps`), счёт копится локально и уезжает в секцию `desktopUsage` свёрткой раз в 5 минут своим таймером. Файл с Верстака — ярлык со ссылкой `{kind:'file', fileId}`; Верстак убирает только СВОЙ ярлык и никогда не удаляет файл (дедуп по sha256, счётчика ссылок нет). Во вложения карточки файл переносит **сама карточка** («Взять с Верстака» в `AttachmentsPanel`, список ярлыков — из `components/DesktopFilesContext.tsx`): писать в карточку снаружи нельзя, у трёх из девяти список вложений живёт в её памяти до сохранения |
| **Обвязка списков: счётчик и «№»** | `components/ListCount.tsx` («Всего: N · Показано: M» над таблицей), `components/RowNumberCell.tsx` (`RowNumberHeaderCell` / `RowNumberCell`, `data-col-kind="rownum"`), `components/VirtualTable.tsx` (проп `rowNumbers`, для страничных — `{offset}`), `utils/listCount.ts`, сторож — `components/ListChrome.guard.test.ts` §«счётчик и нумерация» | Колонка «№» живёт ВНЕ `useColumnLayout` (не скрыть, не переставить) и нумеруется по индексу отображаемого массива при каждом построении; хук ширин `useAdaptiveListTables` её пропускает. `total` — до поиска/фасетов, `shown` — что реально на экране; `TwoColumnList` отдаёт `startIndex` для сквозной нумерации; печать (`ListPrintDialog`) ведёт «№» первой колонкой |
| **Ступенчатый фильтр списка двигателей** | домен — `shared/domain/engineListFacets.ts` (описания ступеней, `applyEngineFacets`, `engineFacetOptions`), интерфейс — `components/EngineFacetFilter.tsx`, состояние списка — `EnginesPage.tsx` (`facetFields` / `facets`) | Первая ступень — какие столбцы участвуют, вторая — значения (пусто = все). **Варианты каждой ступени считаются по строкам, прошедшим ОСТАЛЬНЫЕ ступени**, поэтому отсечение работает в обе стороны и порядок щелчков не важен; своя ступень из подсчёта исключена, иначе выбранное значение схлопнуло бы свой же список. Старый одиночный выбор контрагента переехал сюда |
| **Передача файлов между Верстаками** | `main/services/desktopTransferService.ts`, `ipc/register/desktopTransfer.ts`, `components/SendFileToColleagueDialog.tsx` (выбор коллеги), `components/DesktopInboxBar.tsx` (полоса «Принять N файлов от …»), домен — `desktopMarkInboxHandled` / `desktopPendingInbox` | Транспорт — **личное сообщение чата с файлом**: оно уже синхронизируется получателю и уже выдаёт ему доступ (`chatPayloadGrantsFileAccess` на сервере). Своей таблицы нет намеренно — она потребовала бы правки контракта синхронизации и второго источника прав. Ярлык кладётся только после согласия получателя, в его СВОЙ профиль; отметка разобранных роумится (`inboxHandledIds`), иначе на второй машине те же файлы предложились бы снова |
| **Скрипты обслуживания машины** | `main/services/clientOpsService.ts` (архив из `extraResources`: распаковка в `<userData>/Обслуживание` и запуск окна; показ в проводнике и копия в «Загрузки» — запасной путь), `main/services/clientOpsArchive.ts` (свой читатель ZipCrypto-архива), сами утилиты — `scripts/client-ops/` (`matrica-ops.ps1` — окно с вкладками, см. его README), `ipc/register/clientOps.ts`, `components/ClientOpsDialog.tsx`, встроенная плитка Верстака — `shared/domain/desktop.ts` (`withBuiltinDesktopShortcuts` / `stripBuiltinDesktopShortcuts`) | Плитка есть у ВСЕХ и **не хранится в профиле**: секция Верстака LWW, разложенная копия жила бы до первого пуша клиента. Подмешивается при отрисовке, вырезается при записи, санитайзер не пускает встроенный id внутрь. Скрипты отдаём архивом под паролем `111` — распакованный `.ps1` съедает антивирус до появления исключения (M94) |
| **Карточка файла по id** | мост `files.meta` → `main/services/fileService.ts` → `GET /files/:id/meta` | Полный `FileRef` и **проверка доступа** одним вызовом. Брать этим всё, что собирается положить file id в чужой контейнер: ярлык стола прав не даёт, а вложение карточки — даёт. У компонента нет CSS-классов вовсе — зацепки смоуков только `data-desktop-*`, их держит `DesktopPane.guard.test.ts`. Обработчики читают стол из `desktopUiRef` (GOTCHAS M90) |
...
| `desktop.ts`, `deepLinkRoute.ts` | Верстак: санитайзер секции (`pos`, `shortcutsMigratedAt`, `desktopUsage` — «прививка»), тумблер/`put` с явным исходом, ключ ссылки для дедупа (`file:`/роут/`tab:`), переезд «Быстрого запуска». Сетка: `desktopTileMetrics` (шесть шагов размера) и `desktopLayoutGrid` (кто в какой ячейке при N колонках). Рейтинг: `desktopUsageBump`/`Add`/`Score`/`Steps` — затухание `0.5^(возраст/7)`, окно 30 дней, шаг по месту в распределении с правилом ничьих. Файловый ярлык: `desktopFileLink`/`desktopFileFromLink`/`desktopFileIcon` — нагрузка живёт ВНУТРИ `link`, потому что новое поле ярлыка срезал бы санитайзер. Разбор ссылки приложения (`resolveDeepLinkRoute`) — здесь же, renderer реэкспортирует |
...
- **AI на проде работает через DeepSeek** (D-024, с 2026-08-10); geo-block Anthropic остался только у ревью PR в CI.
