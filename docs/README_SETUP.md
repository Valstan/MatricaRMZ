...
- Положите последнюю сборку **Setup** (NSIS installer, `*Setup*.exe`) в `MATRICA_UPDATES_DIR`.
- Если рядом есть другие `.exe`, сервер всегда предпочтёт Setup‑инсталлятор.
- Backend автоматически создаст `latest.torrent` и будет сидировать файл.
- Клиенты раздают скачанные блоки сразу во время загрузки.
- Установка происходит **при следующем запуске** клиента (после скачивания).
...
### Важно про модель синхронизации
- Доменные данные (справочники, операции, заявки, детали, контракты, сотрудники) синхронизируются в режиме полной репликации.
- Чат остаётся приватным: клиент получает только свои диалоги (и админ видит всё).
- При ошибке `sync_dependency_missing` клиент автоматически делает полный pull и повторяет push один раз.
