...
## 8. ИИ-агент: предпочтительный канал к VPS (SSH vs MCP)

**Рекомендация для нейросетевого разработчика в Cursor:** по возможности выполнять прод-операции через **обычный SSH в терминале**, используя **Host-алиас из OpenSSH config** (например `matricarmz` в `%USERPROFILE%\.ssh\config`), а не только MCP.

Почему так удобнее агенту:

- произвольные команды, пайплайны (`|`), heredoc, циклы — без ограничений схемы MCP-инструмента;
- длинный вывод (`journalctl`, логи nginx, `git diff`) читается из stdout терминала целиком;
- нет жёсткого потолка одного HTTP/MCP-таймаута на всю операцию — тайминг задаётся оболочкой и повторными вызовами;
- те же шаги, что у человека в runbook (`docs/OPERATIONS.md`, `docs/TROUBLESHOOTING.md`), копируются буквально.

Пример (PowerShell / cmd, из машины разработчика):

```powershell
ssh matricarmz "cd /home/valstan/MatricaRMZ && git status --short && git rev-parse HEAD"
```

Для неинтерактивных сценариев полезно:

```powershell
ssh -o BatchMode=yes -o ConnectTimeout=15 matricarmz "systemctl is-active matricarmz-backend-primary.service matricarmz-backend-secondary.service"
```

**Когда оставить MCP:** MCP `vps-matricarmz` остаётся полезным запасным каналом (быстрая команда без открытия shell, или если SSH из среды агента временно недоступен). Настройка и формат аргументов — по разделам 1–7 этого файла.

**Важно:** не коммитить приватные ключи и не дублировать секреты из `.ssh` в репозиторий; в документации достаточно имени Host и путей к профилям на машине разработчика.
