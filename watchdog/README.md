...
Сторож показывает маленькое окно «Матрица РМЗ» с текущим этапом — и только когда
начал **реальное действие**: переустановку, скачивание установщика или починку
ярлыков. Здоровый плановый проход (раз в 15 минут, клиент цел) окна не открывает
вовсе, иначе оно мигало бы на всём парке каждые четверть часа. `--repair`
поднимает окно сразу: человек нажал кнопку и должен видеть ответ, а не пустой
экран.

Этапы: «Проверяю установку Матрицы» → «Скачиваю установщик Матрицы… N%» →
«Переустанавливаю Матрицу. До 15 минут, не выключайте компьютер» →
«Восстанавливаю ярлыки» → «Готово. Матрица восстановлена» либо «Не удалось
восстановить. Сообщено администратору». Итоговая строка висит четыре секунды,
потом окно закрывается вместе с процессом.

Код — [`statuswin_windows.go`](statuswin_windows.go): голый Win32 через `syscall`
(зависимостей в `go.mod` по-прежнему нет), окно живёт в своей горутине с
`runtime.LockOSThread` и собственным циклом сообщений, главная горутина общается
с ним только через `PostMessageW`. `statusShow` / `statusSet` / `statusFinal` /
`statusClose` безопасны и когда окна нет — тогда это пустышки.
